...
> **定位宣言(founder 原话,2026-07-07 口述,总审查员整理格式 —— v2.5 入宪)**
> 1. **FIKIRTIVE 是一个 ALL-IN-ONE 的 MARKETING POWER HOUSE PLATFORM** —— "意思就是基本上都有全部 feature,像之前我给的那些 reference products(Salesforce 那些)"(全量对标 = 第六章对标地图的 18 家)。
> 2. **FIKIRTIVE 上每一个 feature 都可以 100% 人操作,OTTO 也能操作 100% 全部。**(结构保证 = 宪法第 7 条)
>
> **解读边界(防过度发挥)**:"全部 feature"是**终局承诺,不是当下要求** —— feature 清单只来自对标研究 + founder 判决(WHAT-pass),agent 永不自行发明 feature;逐项动工顺序与验收以 `docs/MASTERPLAN.md` 为准。

**FIKIRTIVE = 给中小商家的世界级 ALL-IN-ONE 营销与营收增长(revenue-growth)OS —— 全球为底、本地为皮;每一批为具体真实商家优化到极致(第一批 = 顾客一号,v2.11)。**
(措辞纪律:不用裸的"营收 OS" —— 那会诱导商家预期开票/收款/客服;revenue-growth 与下方边界四层表一致。原 v2.1a 句"从东南亚起步,终点是全球的世界级平台"含义并入本句,不废除。)
...
内容创作(图/视频/分镜)→ 投放(Meta/TikTok/Lazada/Shopee)→ 数据分析 → 排期发布 → **CRM + campaign 管理 + 售前对话与自动回复 + 口碑与复购/唤回** → 资产管理。每一环都人工可操作,每一环 Otto 都能 100% 代劳。今天建成的是创作区和广告区的地基;CRM/campaign 管理/回复/更多平台是已规划的新区(见第六章,功能清单以 Salesforce/HubSpot 全量分析为底稿)。

**边界四层表(v2.11 入宪 —— 按"拥有哪类责任"划界,取代按区名划界;源自 R5 双脑裁定 + founder 逐条批复 2026-07-10)**:

| 层 | 范围 |
|---|---|
| **本体负责** | 研究 / 内容创作 / Campaign / 排期发布与广告 / 本地发现(搜索&地图) / **售前对话与成交促进**(答价、数量价、定制 proof、订金与售前付款链接 —— 钱进商家自己的账户) / 线索与 CRM / 请评 / 推荐 / 复购与唤回 / 增长实验 |
| **读取并验证** | 订单 / 付款 / 退款 / 库存 / 履约 / 积分等经营事实 —— **只读**,用于状态、归因与回执(类比 pixel tracking),永不代管、永不自建账本 |
| **明确交接** | 售后客服工单 / 退换货处理 / 物流 / 会计正式报价单 / 商家对顾客开票 / 催收 / 税务 —— 按 read→trigger→handoff→receipt 契约交接,不拥有这些系统 |
| **永久不做** | 代持商家资金 / 税务工资系统 / 第 8 条全部 |

释义三句(防歧义):① 上文"自动回复(客服)"旧措辞自 v2.11 起正式改义 —— 自动回复属**售前/营销面**(本体);**售后 service desk 属"明确交接"**,第六章"自动回复/客服区"的对标与建设范围按此收窄;② 售前订金的付款链接由 Otto 代发**不触犯**宪法 7 的 money-in 豁免 —— 该豁免管"用户给 FIKIRTIVE 充值",商家的顾客向**商家**付款是商家的生意动作,照常走审批闸(涉钱=必审批);③ FIKIRTIVE 自己向商家收订阅/用量费属自身经营,与本表无关。
...
8. **明确不盖的楼**(拍板过,别再提案):Build 终端 coding agent、独立通用 Chat、Spicy/18+ NSFW、**开放第三方 skill 生态**(skill 永久 BELCORT 内部编写)、**对外 MCP/API 让外部 agent 操作 FIKIRTIVE**(founder:"如果会用其他 LLM,代表我们的 Otto harness 不够好,就代表 FIKIRTIVE 不好。" —— 操作这座城的 agent 永远只有 Otto)、**白标**(founder 2026-07-03:"我要的就是 FIKIRTIVE 变成世界级别的平台" —— 平台不贴别人的牌,Otto 永不改名换脸)、**任何 unlimited 类报价**(与 Otto 自动化互斥,成本敞口)、**Slack/Notion 类工作工具 connectors**(O-13 拍板:SEA SMB 主场在 WhatsApp/Meta/TikTok/Shopee,不在欧美知识工作者工具)、**"评价×奖励"合体流**(v2.11,founder 2026-07-10 二裁:Google 明令禁止激励换评价,合体按钮 = 拿全体商家的平台接入资格冒险 —— 请评与奖励两线**永久分离**,各自都做)。
9. **(已移除,v2.13)** 原「语言约定」三条(spec/skill 文档华语;生成 prompt 一律英文;UI 文案 sentence case)自本版起移出宪法:前两类工作约定降级至项目法(`.claude/CLAUDE.md` 项目约定节);**生成 prompt 语言改由各引擎的 prompt 权威模块按实测最优决定**(例:Seedance 2.0 实测中文提示词优于英文),不再作宪法约束。移除理由(founder 2026-07-23):宪法只装永不漂移的原则,引擎语言偏好是会随引擎换代变化的工程事实。编号 9 保留为墓碑,10/11 不重排,既有引用不受影响。
...
11. **UIUX 是第二支柱(2026-07-03 入宪)。** 除"Otto 全操控"外,UIUX 是留住与吸引用户的核心卖点。质感标杆 = **Apple**;交互趣味参考 Duolingo 但克制 —— 面向专业用户只做 **minimal gamification**(GM 卷已拍 2026-07-03:**GM-02 里程碑/GM-03 目标进度/GM-04 周报语气/GM-05 开店完成度 = 要;GM-01 streak = 不要**;三条边界与方案见 `docs/design/2026-07-03-harmony-06-uiux-gamification.md`)。落地机制:单一设计系统(.gb)不许分叉;每个用户可感的面必须过设计审(不只 runtime QA);设计基准 = Analytics 屏(已有 gold standard)。**Agent-native UI 与 live reflection(v2.6 入宪,founder 与总审查员共同定调 2026-07-07)**:①界面必须实时反映后台与 Otto 的动作(推送/即时刷新;"后台已完成而界面不知"按缺陷处理);②Otto 永远经动作层操作,不做 computer-use 式像素操作(它住在家里,用门不爬窗);③Otto 的动作在界面上对用户可见:coral 高亮 + 简短叙述(与 O-12 同源 —— coral 是 Otto 动作的专属视觉语言);④Otto 随时一步可唤起、其工作永远可见,但**永不抢占主场** —— 画布/工作台是主场,Otto 以边栏/角标等安静形态常驻(v2.7 措辞澄清:与 canvas-first 是一体两面,不是竞争关系)。**零学习曲线定律(v2.9 入宪,founder 2026-07-09 定调)**:用户永远不需要"学会 FIKIRTIVE" —— 他只需要会两件事:说出他要什么,和点"批准"。凡是需要用户学习的地方,按我们的缺陷处理,不是用户的功课。落地三查:①每个专业功能必有"Otto 替我做"的一键入口 ②每个人工面自解释(术语必带人话,界面自己教自己) ③从注册到第一个看得见的成果 ≤ 一次会话。**信任阶梯注**:学习曲线归零,信任曲线不抹除 —— 审批闸是信任的阶梯(看提案→批准一次→routine 放手),明确且短;**实力是信任的引擎**(founder 原话:"我们要做的,就是完美,全部东西。像 Apple,完美他们的 product"),钱路闸门是它的安全带,两者同属"完美"。

**解读边界附则(v2.8 —— 六句解药,不改变任何决定,只灭歧义;源自 2026-07-07 全库措辞审计)**:
1. 第 7 条"(a) 人工可完整操作**无例外**"与"豁免四类"不冲突:豁免管的是 (b) **Otto 对等**那一半;(a) 人工面真的无例外。
2. 第 6 条"跨租户读一个字节 = 事故"管的是**租户之间**;市政厅(平台侧)按其权限矩阵读平台数据不属此列(admin 永久豁免见第 7 条)。
3. 第 11 条 live reflection 的"实时" = **秒级感知**(推送优先、短轮询兜底),不是毫秒神话;"后台完成而界面不知"仍按缺陷处理。
4. 第 2 条"问就是上限、没有代码上限"管**开发/验证期的真实供应商花费**;产品内 routine 的预算上限(第 4 条例外②)是另一个世界的用户侧授权,两者并行不悖。
5. 第 8 条"对外 MCP/API 永久不做"禁的是**外部 agent 获得操作面**;Stripe/Meta/X 等 **inbound webhook 与 OAuth 回调**是收信与授权回执,不是给外人钥匙。
6. 第 3 条铁律①"spend 面只显示 credits 永不显示美元"管**用户可见面**;市政厅成本页(founder 的账房)显示美元属平台侧,永久豁免。
7. 第 8 条同条(v2.10 释宪,founder 2026-07-10):禁的是**外人拿到操作这座城的接口**;FIKIRTIVE 自己**作为消费方**、经自家动作层与审批闸调用**平台官方协议**(含平台提供的 MCP,如 TikTok 官方 agent 通道)属渠道接入实现细节,不在禁区 —— 外部行为体仍永远不得指挥本城、不得绕过动作层。
...
任何新功能都必须走这九条缝之一;绕缝直连 = 审查一票否决。缝 1-8 的**完整施工配方**在 `docs/review/EXPANSION-SEAMS.md`;第九缝配方在 `docs/design/2026-07-03-harmony-02-parity-manifest.md`(EXPANSION-SEAMS 有指针;CI 拦截已上线为**硬闸** —— `scripts/check-parity.mjs` 在 check job 内强制,缺登记/过期/未知豁免/TODO_SKILL 债增长均直接红;2026-07-23 体检核实,较本句旧文「warn→hard 在建」已超额兑现)。
...
| 2026-07-07 | v2.5 定位宣言入宪(第一章开头,founder 原话):ALL-IN-ONE marketing power house(全部 feature,对标第六章 18 家)+ 双 100% 操作宣言 + 解读边界(feature 只来自对标+判决,agent 不发明;动工顺序以 MASTERPLAN 为准);同修订回填 v2.4 批准列(founder 授权"B",总审查员起草) | 已定稿(#187 合并) |
| 2026-07-07 | v2.6 第 11 条增补:Agent-native UI 与 live reflection 四原则入宪(界面实时反映/动作层操作/coral 可见性/常驻陪伴)(founder 口述定调,总审查员起草) | 已定稿(#192 合并) |
| 2026-07-07 | v2.7 措辞修正:第 11 条原则④防误读改写("常驻陪伴"明确为"随时可唤起、永不抢占主场",消除与 canvas-first 的表面冲突;founder 选项 A)+ 回填 v2.5/v2.6 批准列 | 已定稿(#195 合并) |
...
| 2026-07-16 | v2.12 第一期可售产品纵切修订(founder 于 #334 逐项重确认,经 #336 统一批准起草):澄清和声图 P1→P4 是施工波次而非商业分期;第一期须同时完成品类一流内容、真正可用发布、完整 Customer Engagement CRM 三支柱及各自验收门;CRM 第一期对标 respond.io、不进入 Salesforce 深度;WhatsApp 为一期唯一必须真实上线的顾客渠道;更新首发叙事 + 回填 v2.11 批准列 | 已定稿(#337 合并) |
| 2026-07-23 | v2.13 语言约定移出宪法(founder 裁定「认同移除」,总审查员起草):第 9 条整条降级 —— spec 华语与 UI sentence case 归项目法,生成 prompt 语言归各引擎 prompt 权威模块按实测定(Seedance 2.0 中文更优为触发案例);编号留墓碑不重排。同 PR:第四章第九缝旧句「warn→hard 在建」更正为已上线硬闸(2026-07-23 体检核实)+ 回填 v2.12 批准列 | 待 founder 终审 |
