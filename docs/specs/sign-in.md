...
| 2026-09-11 | **登录门⑤（SIGNIN-A12）：Google 门在 E2E 跑道上必须有替身，规格 §1 九问没写它长什么样**。A12 的旅程要求真的走一次 Google 门，而跑道上没有 Google 也永远不该有（`e2e/support/env.ts` 逐条挡掉一切能打出这台机器的凭据），Better Auth 的 Google 供应商又把 `accounts.google.com` 与 `oauth2.googleapis.com/token` 写死在库里、换不掉。落地方式：走库自己的第二条 Google 入口（`/sign-in/social` 带 `idToken`，Google One Tap 那条），**只替掉 Google 自己那个签名**——`verifyIdToken` 在武装时收一个用 `BETTER_AUTH_SECRET` 算 HMAC 的替身 token（`packages/core/src/e2e-google-door-stub.ts`）。之后的账号合并、`user.create.before` 三步判定与 A13 断言、`session.create.before` 复查、身份收敛、会话 cookie 全程照跑。两把锁：`E2E_GOOGLE_DOOR_STUB` 逐字 `"1"`（不用 `BETA_*`／`*_ENABLED` 命名，它不是等着上线的功能开关；契约 `productionValues` 空数组 **加 `warnImmune`**——生产上任何值都被开机检查拒绝，而且 `FIKIRTIVE_ENV_CONTRACT=warn` 的逃生门也降不了它；唯一豁免是 `productionExemptWhen = pointsAtThrowawayTestDatabase`，即**指着一个 `_test` 库的进程**，跑道正是靠这个结构性事实起得来，而不是靠逃生门），以及那个 HMAC。围栏的口径按判官 #1349 P2 校准过：武装之后 `BETTER_AUTH_SECRET` **单独一把**就能换到任意邮箱的会话，所以围栏＝**生产禁武装 + 密钥保密**，HMAC 那把锁在跑道上的作用是「别的进程与页面脚本递不出能过的 token」，不是「误开也无害」。反向围栏在旅程里（同一份载荷只把签名改掉 → 401 `INVALID_TOKEN` 且不留行，换回对的签名 → 200）与 `apps/web/lib/__tests__/e2e-google-door-stub.test.ts`（五条）、`packages/core/src/env-contract.test.ts`（不可降级四条）。留给 S5 看一眼：这是产品代码里唯一一段「在对的条件下接受一个不是 Google 签的身份断言」。 |  |
| 2026-09-11 | **登录门⑤：A12 里那句「生成一张图」在 E2E 跑道上是种出来的，不是 worker 跑出来的**。这台跑道没有 worker，也没有任何供应商凭据，所以旅程里那张图与那笔账由套件按产品的形状写下（素材 `seedLibraryMedia`，钱 `seedSettledJob` 的 RESERVE＋SETTLE，键与 `packages/db/src/credits.ts` 逐字同形）。A12 要问的是「换一扇门回来之后它们还在不在、有没有变成两份」，那一段是**真的**：同一个 org、同一个用户、一笔赠金、一笔 SETTLE、名单只有码门写的那一行。要让「生成」那一步也端到端为真，得先让 E2E 跑道有一个离线 worker（`GENERATION_PROVIDER` 的 mock 那条路），那是另一张票。 |  |
