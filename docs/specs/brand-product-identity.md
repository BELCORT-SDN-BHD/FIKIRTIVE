...
| 2026-09-11 | **落地（PR #1345）**票 #1323「编辑与删除同步 + 自动化旅程」：PRODID-A4 与 A6 收口 —— ①A4 补上四个写入口逐条核对之后剩的最后一格：Brand 页那张产品表单**根本没有主图栏**（`ProdForm` 只有 Name / Price / Description / Selling angle / Link / Tags / Category），上一版 `OttoMemory.prodSave` 却替它把 `data.imageAssetId` 当主图意图递下去，而那一格是读路 `withProductIdentity` 补进 `data` 的客户端快照 —— 商家在 Library 换过封面之后，回 Brand 页改一次价就把旧封面写回权威。改成由 `ProductShowcase` 自己交 `identity`（它是唯一知道自己有哪几栏的地方），这一趟只交名字。②A6 补上「已生成的成片不动」那半句的真测试（此前只有一行注释担保）。③新增旅程 `e2e/journeys/23-brand-product-identity.spec.ts` | |
| 2026-09-11 | 票 #1323 报出的**缺口，待裁**：A6 与 A4 说的两个商家动作今天在界面上没有入口。Brand 页的产品卡只有 Archive，**没有 Remove**（`ProductShowcase` 不接 `onDelete`）；Library 的元素只有 Remove，**没有 Restore**（`restoreBrandRecord` 今天只有 Otto 的撤销条走得到）；Library 的元素详情也**没有改名栏**（`updateEntity` 今天只有已退役的 `OttoStuff` 在调）。动作层三条路都在、都有真库测试，缺的只是入口。所以 A6 的「在 Brand 页删除产品；再在 Library 恢复它」与 A4 的「在 Library 改名」在 S5 当场演示不出来。本票不补入口（新面 = 重挡），照原样报出 | |
