...
| 2026-09-11 | 票 #1323 报出的**缺口，待裁**（判官第 2 轮更正过一次，下面每条都逐条核过当前代码）：A6 与 A4 各有半边今天在界面上演示不出来。①**删产品做得到**：`/brand`（`app/brand/BrandWorkspace.tsx:668` 的 `Remove this context`）对产品价签调 `deleteBrandRecord`，产品在这一面是知识库分区里的一条（`lib/brand-context-data.ts:147` 的 `brandSectionForRecordKind`）；`/brand/records` 的产品卡只有 Archive、没有 Remove（`ProductShowcase` 不接 `onDelete`）。②**恢复入口的方向与规格写的相反**：`restoreBrandRecord` 有两个人手入口 —— `/brand` 的 Removed 列表（`BrandWorkspace.tsx:582`）与 Otto 的撤销条；**Library 没有元素的恢复键**（`LibraryView.tsx` 只有 `softDeleteEntity` 与恢复成片的 `restoreGeneration`）。所以 A6 那句「在 Brand 页删除产品；再在 Library 恢复它」演示不出来，反方向（Library 删 → `/brand` 恢复）整条演示得出来。③**Library 元素详情没有改名栏**：`updateEntity` 今天的调用方是 `components/otto/OttoStuff.tsx:260`（聊天壳时代的旧件，已无路由渲染）与 `lib/otto-entities-port.ts:37`（Otto 改名技能），Library 那一面一个字都没接，所以 A4 那句「在 Library 改名」演示不出来。三条动作层的路都在、都有真库测试，缺的只是入口；补入口 = 新面（重挡），本票不做，照原样报出 | |
