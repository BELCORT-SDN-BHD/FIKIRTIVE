...
| 2026-09-15 | **未钉封面时的显示规则**：产品身份（`Entity`）今天没有规定「没有人手挑过封面时显示哪张图」的默认行为（`Entity.baseAssetId` 可空，`packages/db/prisma/schema.prisma:100`）。提案：挂上该产品的第一张参考图时自动设为封面并写回 `Entity.baseAssetId`，商家随时可在 Library 或 Brand 换掉；存量已有产品、尚未有 `baseAssetId` 但已挂参考图的行，一次性补齐迁移；Library 与 Brand 从此读同一张图，不再各显各的。 | 2026-09-15 Founder（对谈）：采用 |
