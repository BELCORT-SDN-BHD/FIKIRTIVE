...
> 状态: 已交付 · 归档（生产 `NODE_ENV` 核证在 #1480）
...
| 日期 | 想法 | 裁决（留空待 S5） |
|---|---|---|
| 2026-09-12 | **§4 要求的 staging 现场核证结果（#1383 前置，只读）**。查法：`railway status` 确认 project `FIKIRTIVE` / environment `staging`；`railway variables -e staging -s web --kv` 与 `-s worker --kv` 各重定向到临时文件，只 grep `NODE_ENV` 与 `GENERATION_PROVIDER` 两键，读完即删，其余变量一律不回显。结果：① 两个 service 的 Railway 变量里**都没有 `NODE_ENV`**（web 50 个变量、worker 26 个变量，均无此键）——因此「staging 按生产标记运行」在变量层**不成立**，但进程运行时取值（`next start` 自己会把 web 设成 production；worker 镜像的启动命令未核）**未证**，要断言需另查运行进程；② staging **worker 的 `GENERATION_PROVIDER=byteplus`**——也就是说 staging 今天跑的是**真付费引擎**，不是 `docs/runbooks/staging.md` 两级意图表里写的 `mock`/$0。对本规格的影响：本票落地不会拒掉 staging worker 的启动（它已是唯一合法值），§4 担心的「staging 的 $0 mock 演示当天失效」这条退路**在别处已经先失效了**——staging 今天根本不是 $0。离线演示与 staging 花费边界要怎么摆，呈 Founder 另裁（不属本票）。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：**§4 那句「离线演示走 staging 的 $0 mock」改读为「演示走 staging 真引擎，按次授权，预算见走查计划」**。理由是事实已经如此——staging worker 的 `GENERATION_PROVIDER=byteplus`，今天跑的就是真付费引擎，本规格落地之后 `mock` 更不再是生产合法值；第三轮走查从 2026-09-15 起的每一次真实生成都是这么办的（Founder 逐次授权、US$20 封顶／US$16 暂停，累计与逐笔见 `docs/audits/fullstack-staging-2026-09-14/run-ledger.md`「第三轮真实付费与演练花费累计」一节）。**§4 正文不改**（冻结），本行即它的现行读法。 |
| 2026-09-12 | 『送达』出现两套定义：§1.4 写『至少一条通道 sent（含 Sentry）』，RELY-A7 要求『邮件与 Telegram 双双失败⇒下一趟再试人工渠道』；实现按 A7 优先（gen 告警的送达判定排除 Sentry），Stripe 购买告警回执沿用 §1.4 含 Sentry 口径——后果：生产配了有效 SENTRY_DSN 时购买告警几乎总算已送达，主动找回只在连 Sentry 也挂时触发。两套口径待 S5 统一裁。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：**不统一成一套，分层，各有测试钉住**。① **生成类告警（「商家付了钱什么都没拿到」这一族）按 RELY-A7**：只有人工渠道（邮件／Telegram）真 sent 才算送达，**Sentry 不算**——下一趟巡检还要再试，不许因为 Sentry 收到了就当这条已经有人看见。② **Stripe 购买告警按 §1.4**：含 Sentry 算送达（`alertDelivered`），因为这一族的下游是对账重试（RELY-A9），台账本身会把没送出去的那条追回来。两条分层写在这里，**§1.4 与 RELY-A7 的正文都不动**（冻结）；读 §1.4 时以本行为准。为什么不合成一套：合成「一律含 Sentry」会让生成类告警在配了 DSN 的生产上几乎永远算已送达，等于把 A7 那句「下一趟再试人工渠道」废掉；合成「一律不含 Sentry」又会让 Stripe 那一族在只有 Sentry 可用时永远重试不停。 |
| 2026-09-12 | RELY-A10 生产现场核证（只读，railway variables -e production -s worker 键名）：DATABASE_URL/STORAGE_DRIVER/SENTRY_DSN/GENERATION_PROVIDER/R2_ACCESS_KEY_ID/R2_SECRET_ACCESS_KEY/R2_BUCKET/R2_ENDPOINT/BYTEPLUS_API_KEY 全部在位；变量层无 NODE_ENV 键（与 A 段核证一致）——生产判定依赖运行时/启动命令，backup-cron 容器的 NODE_ENV 未核，若运行时非 production 则 A10 的 exit(1) 不会触发，S5 演示前需另核。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：**PRODUCTION-GATED——这一核属生产部署门 #1480，不在本版**。要核的那一句是「生产 backup-cron 容器运行时的 `NODE_ENV` 到底是不是 production」（变量层两边都没有这个键，只能查运行进程），而查它要碰生产，按项目「外部边界」一节非经 Founder 对该次动作授权不得进行。已列进 #1480 的清单；在它核完之前，A10 前半句（缺 env 即退出非 0）在**生产**上仍是**未核**，本规格归档不掩盖这一点。 |
| 2026-09-14 | **R3-F08**（`docs/audits/fullstack-staging-2026-09-14/findings-catalog.md` 环境漂移登记：CI 服务容器 postgres:16、staging 实测 18.6）：RELY-A10 第二句「后者照常完成当日备份」自 2026-09-04 19:01Z 起在 staging 从未成立——BackupRun 累计 1360 条 failed、0 条 succeeded；根因＝worker 镜像 `pg_dump` 17（`apps/worker/Dockerfile:18` 固定 postgresql-client-17）拒绝 dump 已是 PostgreSQL 18.6 的 app DB（大版本不兼容），且失败时 stderr 被丢弃（`apps/worker/src/db-backup.ts` `stderr:"ignore"`），原始报错从未落盘，此前无法诊断——CI 固定 16 对 16，从未测过这条大版本落差。修复见 PR #1442（客户端 `pg_dump` 升至 18、stderr 截尾按错误类别分类入库、恢复侧加版本闸）。关闭条件：Founder 授权 staging worker 按 #1442 重建部署后，出现第一条 `status=succeeded` 的 BackupRun，且 `/api/health` 的 backup 字段离开 `missing`。 | 2026-09-15 Founder：批准登记；授权合并 #1442 后重建部署 staging worker，以第一条 succeeded BackupRun 关闭。**2026-09-19 关闭条件已达（staging 只读回执）**：`BackupRun` 已有 **5 条 succeeded**，最新一条 `startedAt 2026-09-18 19:04:31.991Z`／`finishedAt 19:04:33.935Z`（1944 ms），键 `backups/db/fikirtive-2026-09-19.dump.gz`、**3,070,337 字节**、trigger `worker-timer`、credentialMode `shared`，首条成功不晚于 2026-09-17 19:04:59Z；1627 条 failed 止于 2026-09-15 06:12:34Z（修法生效后再无新增）；`/api/health` 的 backup 字段已由 `missing` 变成 **`fresh`**。**RELY-A10 后半句「后者照常完成当日备份」据此在真环境上第一次成立**；前半句（缺必需 env 启动即退出非 0 并点名缺项）本轮没在真环境演示过，且这条读数取自 staging 而非生产，所以 `coverage-matrix.md` 的 RELY-A10 行记 **PARTIAL** 而不是 PASS，生产那一半挂 #1480。 |
...
| 2026-09-15 | **后台可丢弃单条死信并留审计记录（已做，中挡 mini spec）**。归宿说明：全仓没有任何已冻结规格定义 `/api/ops/dlq` 探针或 `/admin/queue` 的验收（`#793`、`#779` 都早于规格树；`docs/specs/` 里 `dlq`／死信 的命中只有 `tenant-isolation.md:85` 那条队列点名之争），故照上一行 R3-F16 的先例按 fail-closed 精神登记于本规格 —— **上游缺口如实报告，未当作已完成**。<br>**问题**：七（今天八）条队列都配了 `deadLetter` 而**没有任何东西消费它们**（`packages/core/src/dead-letters.ts:4-10` 逐字写明这是设计）；放弃一件活没有错，错的是放弃之后没有出口 —— staging 今天卡着一条 `gen.dlq`（payload `{genJobId}`，2026-09-11 进队，该单账本 RESERVE −110／+110 后 REFUND +110／−110 净 0），探针因此长期 503，而产品里唯一的处理手段是对生产库跑裸 SQL（本仓禁止）。**Founder 2026-09-15 裁决（对谈）**：做一个后台小控件，一次丢弃一条并留一行审计，让下一条卡住的死信变成十秒钟的事。<br>**做法**：`/admin/queue` 新增 “Dead letters” 一节（`apps/web/lib/dead-letters-admin.ts` 读、`apps/web/components/admin/DeadLetterPanel.tsx` 画）＋ server action `discardDeadLetter`（`apps/web/lib/dlq-actions.ts`）。权限 **`system.mutate`**（不是看板今天的 `system.read` —— `viewer` 持有 read，把不可逆的写挂在读权限下等于让只读角色销毁队列状态）。丢弃＝pg-boss 的 `cancel`（state → `cancelled`，行留在库里可查），不是物理删除；探针的 `state <= 'active'` 因此不再数它。审计行写 `ActionEvent{ ownerId: founder, type: "dlq.discard" }`，payload 只留标识符与操作者邮箱，其余键**只留键名**（同 `dlq-watch.ts` 对 Sentry 的纪律）。`/api/ops/dlq` 那条免鉴权只读路径**一个字未改**。<br>**验收（DLQ-A1…A5，逐条有测试）**：A1 清单列出队列名／job id／进队时间／payload 标识符／该单账本净额，读不到时说「读不到」而不画空清单；A2 确认后丢弃一条 ⇒ 该行消失、探针不再数它、屏幕上那句话点名刚写的审计行 id；A3 再丢一次（或那条已不在）⇒ 答「已经不在了」、不报错、审计行仍只有一行；A4 丢弃前后 `CreditLedger` 一行未变（丢弃从不碰钱）；A5 不持 `system.mutate` 的会话被拒、死信仍在、探针仍数它，非死信队列一律拒绝。 | 2026-09-15 Founder：批准并已做（对谈裁决）；PR `claude/admin-dlq-discard` |
| 2026-09-16 | **复核修正（承上一行「后台可丢弃单条死信」；跨厂对抗复审三条 P2，PR #1461）**。三条都不改 DLQ-A1…A5 的判据，只把它们说到位 —— A4「丢弃从不碰钱」原样成立。<br>① **清单上账本那一行改报两个数**：真花掉的（`−Σ(balanceDelta + reservedDelta)`）与还押着的（`Σ reservedDelta`），外加该单出现过的账本动作种类。上一版只求和 `balanceDelta`，而 RESERVE 写 `-cost / +cost` —— 一条**从没释放的预留**会被报成「扣了 110」，与「结算掉 110」印出同一句话，而那正是运维按下 Discard 之前最需要分清的两种局面。<br>② **取消成功、审计行写不下去时多一种结果 `discarded-unaudited`**。取消不可逆：上一版这条路整个 server action reject，对话框说「The action could not finish…」，运维重试时那一条已经不在队列里，于是读到「已经不在了，什么都没记」—— 一次**零审计的丢弃**被一句假话盖住。现在「活丢掉了」与「痕迹没留下」两件事一起说出口，屏幕上请运维手工补一条并告知团队，另发一条**只带失败分类、不带 payload** 的 Sentry 告警（同 `dlq-watch.ts` 的纪律；形状照 `lib/tenant-actions.ts` 的 `auditFailed` 先例）。顺序没动：审计仍写在取消之后 —— 反过来会审计一次从没发生的丢弃。确认对话框那句「会留一行审计」也随之改成在每一条路上都为真。<br>③ **账本读失败改报 `"unreadable"`**，不再与「payload 没点名任何生成单」一起收敛成「这一条不涉及钱」—— 后者会让运维丢掉一条预留还悬着的活。这就是本规格「读不到 vs 读到空是两句话」在账本这一层的执行。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：**追认——三条复核修正已落地**，PR #1461 合并 commit `f0835808`（2026-09-17T03:52Z，见 `docs/audits/fullstack-staging-2026-09-14/report-round3.md` 合并台账第 23 行）。DLQ-A1…A5 的判据一字未改，本行只是把「说到位」的那三处补齐。 |
| 2026-09-19 | **归档时照实登记的缺口：本规格十一行验收只有一行拿到真环境回执**。2026-09-19 的逐条扫描（`docs/audits/fullstack-staging-2026-09-14/local-logs/spec-acceptance-sweep-2026-09-19.json`）判 §2 十一行为 **10 PROVEN ＋ A10 PARTIAL**，但那是**代码与自动测试**的证据（并以 `origin/main` 当时的绿 CI 为准）。走查矩阵（`coverage-matrix.md` RELY-A1…A11）今天是 **A10 PARTIAL、其余十行 NOT RUN**，因为它的表头写死「历史报告与已有自动测试不能替代本轮执行回执」——本轮只有 A10 的备份读数是真环境回执，A1…A9／A11 一条都没在真环境上演示过（A4／A6 要故意把进程起坏，A7／A8／A12 那类要真的打挂告警通道，都不是只读边界内做得了的事）。两者不矛盾：**代码层面已证，真环境回执欠着**。补它属于第三轮走查剩下那一摞（见 `report-round3.md` 2026-09-19（六）节的缺口表与两条路）。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：**本规格状态改「已交付 · 归档」**——生产 `NODE_ENV` 的核证挂在生产部署门 #1480；十一行的 staging 回执缺口按本行如实留账，不用测试冒充回执。 |
