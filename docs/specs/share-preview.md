...
| 2026-09-19 | **服务器端 Sentry 不洗分享 token（R3-F35 —— SHARE-A6 遥测那一句只关了一半）**。<br>意图：SHARE-A6 后半句「遥测（Sentry `beforeSend` 抓到的事件）里的任何 URL 都不含 token」今天只在**浏览器**上、且只在**地址字段**上成立。服务端 `apps/web/instrumentation.ts:41-45` 的 `Sentry.init` 压根没有 `beforeSend`（缺口已记在上一行 2026-09-17 R3-F31／F32 的「残余」段），于是一条服务端错误只要请求地址是 `/s/<token>`、`/api/media/pub/<token>`、`?t=<token>`，或请求头／cookie 里带着那颗 `__Secure-sp_t`，token 就原样送去第三方——拿到它就等于拿到商家发给客户的那条链接本身。修根不修表、关类不补例：要关的是「分享／媒体 token 进遥测」这**一类**，不是「服务端那一例」——洗法与形状搬进运行时中立的 `apps/web/lib/sentry-scrub.ts`，**浏览器与服务器两个 init 的 `beforeSend` 从此是同一只函数** `scrubSentryEventTokens`；浏览器那边原本也漏着同一类的另外两处（错误正文 `exception.values[].value`、fetch 面包屑 `data.url`），一并关上。<br>可当场验的验收：`/s/<token>`、`/api/media/pub/<token>`、`?t=`、`__Secure-sp_t` 四种形态在 Sentry 事件的 `request.url`／`query_string`／`headers`／`cookies`／`breadcrumbs`／`exception.values[].value`／`message` 中一律 `[redacted]`（`Authorization` 一类的凭据回声整条不要），**两个 init 用的是同一只函数**，且 `instrumentation.ts` 真的把它交给了 `Sentry.init`——服务端由 `apps/web/lib/__tests__/sentry-server-scrub.test.ts` 钉住（含「`Sentry.init` 收到的 beforeSend 就是 `scrubSentryEventTokens`」一条）；浏览器端由 `apps/web/lib/__tests__/sentry-browser.test.ts` 新增的两条钉住——「SHARE-A6 —— init 参数带的 beforeSend 也洗异常正文里的 token(旧的 scrubShareTokens 碰不到)」与「SHARE-A6 —— init 参数带的 beforeSend 也洗 fetch 面包屑的 `data.url`」，该文件既有断言一字未改、仍全绿（浏览器端只会洗得更多，不会洗得更少）。脱敏字面量沿用既有的 `[redacted]`，不是 `<redacted>`——那是 #1317 起就被浏览器端测试钉住的字面量。<br>非目标：①不改 `apps/worker` 的两处 `Sentry.init`（`src/index.ts:135`、`src/backup-cron.ts:48`）——worker 全仓不出现 `sp_t`／`/s/<token>`／`SHARE_PREVIEW*`，它只自签 2 小时 TTL 的**发布**媒体 token 交给 Meta（`src/jobs/publish.ts:298`），分享 token 到不了它那里；那条「Meta 的报错回声可能带出发布媒体 URL」的路是**未验证**的另一条线，且跨 app 边界（要么让 worker import apps/web，要么把洗法搬去 `packages/core`），另立一行再议；②不接 `beforeBreadcrumb`——面包屑在 `beforeSend` 之前就已挂进事件，这道 `beforeSend` 已经逐条洗过。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：本版修。2026-09-19 追加裁决（关类不补例）：浏览器端不得只关一半，两个 init 用同一只函数，「浏览器端行为不变」放宽为「浏览器端只会洗得更多」。 |
