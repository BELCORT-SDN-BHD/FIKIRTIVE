...
2. **入口在哪里？（列全，含深链）**
   ...
   - 撤销：**撤销面＝Otto**——与铸链同一处，Otto 技能 `sharePreviewRevoke` 调 `revokeSharePreview`（`apps/web/lib/otto-actions.ts:918` 是今天唯一的调用点）；**排期页的撤销入口另案**，本规格不建那个页面（2026-09-19 措辞订正，理由与出处见 §5 同日行）。
...
| 日期 | 想法 | 裁决（留空待 S5） |
|---|---|---|
| 2026-09-17 | **staging 无法验证本规格任何一行（第三轮付费旅程第四组）**：staging `web` 与 `worker` 两个服务均未设置 `MEDIA_PROXY_SECRET`／`SHARE_PREVIEW_SECRET`（各查 53／31 个变量名，逐一确认缺失），`sharePostPreview`／`getPublicMediaLink`／`/api/media/pub/<token>` 三处全部在密钥判断这一步就 fail closed 返回；同时全部 8 个组织在 staging 数据库里零 `ScheduledPost` 行，且本 build 没有商家排期入口（`/schedule` 307 回 Home，唯一路径是 Otto 的 `sharePostPreview` 技能），铸链本身就无从发起。独立核证员额外发现第三个阻断：即便补齐两把密钥且真有一条 `ScheduledPost`，客户面入口 `apps/web/app/s/[token]/route.ts:55` 用 `new URL(SHARE_PREVIEW_COOKIE_PATH, req.nextUrl.origin)` 构造跳转目标，在 Railway 代理之后 `req.nextUrl.origin` 解析成内部 socket——`GET /s/<token>` 在 staging 实测 303 到 `https://localhost:8080/schedule/share-preview`，任何商家发出的分享链接客户端到端必坏（登记 `findings-catalog.md` R3-F31，修复分支 `claude/share-entry-redirect-origin-r3-f31` 施工中）。旧式 `?t=` 链接另有一处一秒级的令牌驻留与登录壳短暂闪现（R3-F32）。已脱离分享链验证的条款：账本无新增（SHARE-A9 的精神，虽非 100 次拉取）、匿名读其他租户资源统一 404（SHARE-A7 附近的一般 fail-closed 行为）、fail-closed 页面文案统一（SHARE-A8）。**未能验证、且观察到的 404 不能代替验证的**：SHARE-A1（Range 分段传）、SHARE-A2（普通打开）、SHARE-A5（「内容可能已更新」提示行）、SHARE-A6（干净地址端到端、遥测不含 token）、SHARE-A9（100 次匿名拉取）、**SHARE-A10（签名跨租户改指）**——密钥未配置时 `verifyMediaToken` 在 `keyOwnerMatches` 归属复核之前就返回 null，本轮观察到的 404 证明的是「密钥未配置」分支被命中，不是签名跨租户改指被专门挡下，SHARE-A10 在 staging 事实上从未被真正测过。**待 Founder 裁**：staging 何时补齐两把密钥、创建一条测试 `ScheduledPost`，以便下一轮付费旅程能真正跑通本规格。出处：本次第三轮收官（四）走查报告，`docs/audits/fullstack-staging-2026-09-14/report-round3.md` 2026-09-17（三）节、`local-logs/staging-r3-paid/real-10-share-anon.json`。 | **已落地（2026-09-18）**：Founder 自己跑不回显命令把两把密钥设上了——`web` 的 `MEDIA_PROXY_SECRET` 与 `SHARE_PREVIEW_SECRET` 各 64 字符，`worker`／`worker-compute` 的 `MEDIA_PROXY_SECRET` 各 64 字符，三处 `MEDIA_PROXY_SECRET` 的 sha256 前缀同为 `0a19139e0c22`（**只核名字与长度，值从头到尾没显示过**）；`web` 13:47:49Z 重启后 `/admin/system` 答「Deploy identity In sync」。出处 `report-round3.md` 2026-09-18（五）节第④条。**那条测试 `ScheduledPost` 也有了**：2026-09-18 经 Otto `sharePostPreview` 铸出一条真链接（旅程自造，范围事先定死；撤销授权由 Founder 2026-09-19 对谈授权编排者代裁追认），REAL-10／REAL-11 当天真跑，SHARE-A1／A2／A5／A6／A7／A8／A9／A11 八行拿到回执（回填见 `coverage-matrix.md`）。**仍未验证的三行照实留着**：A3／A4（要把限流计数存储打挂，超出本轮边界）、A10（签名跨租户改指——手上那把媒体 token 在造出改指之前就被撤销了，撤销行答的 404 分不出是归属复核挡的还是撤销挡的；批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：staging 结构上不可测，**接受 CI 单元测试覆盖**，REAL-10 记 PARTIAL、不挡 GO）、A12（限流存储不可用的告警送达回执没观察到）。 |
| 2026-09-17 | **客户面入口两处在真服务器上不兑现（R3-F31／R3-F32，第三轮 staging 匿名只读核证，构建 c0d25917）**。<br>意图：第 1 节第 2 条（客户面「入口路由 `/s/<token>` → 303 到干净的 `/schedule/share-preview`；旧的 `?t=` 形式继续被受理，受理后同样换 cookie 再跳干净地址」）与验收 SHARE-A6（「地址栏是不含 token 的干净地址」）今天只在代码形状上成立，在代理后面与流式渲染下都不成立：①`/s/<token>` 用 `req.nextUrl.origin` 拼绝对地址，Railway 容器里那就是 `localhost:8080`，实测答 `location: https://localhost:8080/schedule/share-preview` —— 顾客点真链接落在自己电脑上；②旧式 `?t=` 实测答 HTTP 200（约 25 KB）+ `<meta id="__next-page-redirect" http-equiv="refresh">` + 一屏已冲出的外壳（含 `href="/login"` 的「Go to sign in」），token 在地址栏停约一秒，且这张自述「无登录、无回工作区的路」的页面先闪了登录面。<br>可当场验的验收：①`GET /s/<任意 token>` 的 `Location` 是相对路径 `/schedule/share-preview`，不含任何主机名，且换一个对外主机名答案一字不变；②`GET /schedule/share-preview?t=<token>` 答 30x，正文不含 `__next-page-redirect`、不含 `/login`、长度 < 200 字节；③干净地址 `GET /schedule/share-preview` 仍答 200 并画那张唯一的拒绝牌。三条都由 e2e journey 30（`e2e/journeys/30-share-link-entry.spec.ts`）在真服务器上判定。<br>残余（本票不修，留给后续；跨厂复审 2026-09-17 点名）：同一形状的潜伏点还有两处 —— `apps/web/app/api/meta/data-deletion/route.ts:84` 用 `process.env.APP_ORIGIN ?? new URL(req.url).origin` 拼对外状态页地址，而 `APP_ORIGIN` 在契约里是 optional（条目在 `packages/core/src/env-contract.ts:608-617`，`requirement: "optional"` 那一行是 :612；2026-09-18 订正：原文写的 :609 是这条目的 `name:` 行，不是 optional 那一行），没配就退回请求 origin，代理后同样是容器自己；`apps/web/app/files/[...key]/route.ts:61` 与 `:66` 用 `new URL("/login", req.url)` 建登录跳转（墙内面，商家侧）。另记一条早于本票的缺口：服务端 Sentry init（`apps/web/instrumentation.ts:41-45`）没有浏览器侧那道 `beforeSend` 脱敏（`apps/web/lib/sentry-browser.ts:64` 的 `scrubShareTokens`），SHARE-A6 后半句只在浏览器遥测上成立（该缺口已由 R3-F35／PR #1482、合并 `bf8dbe96` 关掉，登记见本表 2026-09-19 行）。 | 批准: 2026-09-18 Founder 对谈追认（原话「这四条追认」；编排者依 2026-09-15「新发现全部本版修」裁决与 2026-09-16「要我拍板的直接决定」授权先行施工，2026-09-17）（**2026-09-19 搬格说明**：这句批准原先写在左边「想法」格的末尾，本次原封不动搬进「裁决」格——一个字没改，只是放回它该在的那一列，免得后来人以为这一行还没批。） |
| 2026-09-19 | **遥测脱敏从「浏览器」扩到「服务器」，并补上事务事件那只钩子（R3-F35）**。<br>先把话说准：§2 冻结的 SHARE-A6 原文是「地址栏是不含 token 的干净地址；**浏览器遥测**（Sentry `beforeSend` 抓到的事件）里的任何 URL 都不含 token」——它只承诺**浏览器**那一侧，服务端本来就不在它的字面之内。所以本行不是「SHARE-A6 有一半没做到」，而是**在 §5 里把这条判定扩到服务器遥测**；§2 那张冻结表一个字不动，扩张只住在这里，下次 S5 连同本行一起裁。<br>意图：服务端 `apps/web/instrumentation.ts` 的 `Sentry.init` 压根没有 `beforeSend`（缺口已记在上一行 2026-09-17 R3-F31／F32 的「残余」段），于是一条服务端错误只要请求地址是 `/s/<token>`、`/api/media/pub/<token>`、`?t=<token>`，或请求头／cookie 里带着那颗 `__Secure-sp_t`，token 就原样送去第三方——拿到它就等于拿到商家发给客户的那条链接本身。顺带两件同类的事：①浏览器那道门当时只咬地址字段，错误正文 `exception.values[].value` 与 fetch 面包屑的 `data.url` 一直没洗（Founder 2026-09-19「关类不补例」追加裁决要求一并关上）；②`beforeSend` 在 SDK 里只作用于**错误**事件，事务事件走 `beforeSendTransaction`，两个 init 都没接——今天 `tracesSampleRate: 0` 没有事务事件被采样，所以不是正在漏，但那是个随时会被改掉的设置值，不是一道门。修根不修表：洗法与形状搬进运行时中立的 `apps/web/lib/sentry-scrub.ts`，**两个 init 的 `beforeSend` 与 `beforeSendTransaction` 都是同一只函数** `scrubSentryEventTokens`。<br>可当场验的验收：`/s/<token>`、`/api/media/pub/<token>`、`?t=`、`__Secure-sp_t` 四种形态，在 Sentry 事件的 `request.url`／`query_string`（裸串、字典、二元组三种形状同口径）／`headers`／`cookies`／`breadcrumbs`／`exception.values[].value`／`message`／以及事务事件的 `transaction`／`contexts.trace`／`spans[]` 中一律 `[redacted]`（`authorization`、`proxy-authorization`、`x-api-key`、`x-auth-token`、`x-amz-security-token` 这份短名单整条值不要），**服务器与浏览器两侧接的是同一只函数，且两只钩子都接上**——服务端由 `apps/web/lib/__tests__/sentry-server-scrub.test.ts`（14 条）钉住，含「`Sentry.init` 收到的 beforeSend／beforeSendTransaction 就是 `scrubSentryEventTokens`」；浏览器端由 `apps/web/lib/__tests__/sentry-browser.test.ts`（18 条）钉住，新增三条——异常正文、fetch 面包屑 `data.url`、`beforeSendTransaction` 接线；该文件既有断言一字未改、仍全绿（浏览器端只会洗得更多，不会洗得更少）。脱敏字面量沿用既有的 `[redacted]`，不是 `<redacted>`——那是 #1317 起就被浏览器端测试钉住的字面量。<br>非目标与已知边界：①不改 `apps/worker` 的两处 `Sentry.init`（`src/index.ts:135`、`src/backup-cron.ts:48`）——worker 全仓不出现 `sp_t`／`/s/<token>`／`SHARE_PREVIEW*`，它只自签 2 小时 TTL 的**发布**媒体 token 交给 Meta（`src/jobs/publish.ts:298`），分享 token 到不了它那里；那条「Meta 的报错回声可能带出发布媒体 URL」的路是**未验证**的另一条线，且跨 app 边界（要么让 worker import apps/web，要么把洗法搬去 `packages/core`），另立一行再议；②不接 `beforeBreadcrumb`——面包屑在 `beforeSend` 之前就已挂进事件，这道 `beforeSend` 已经逐条洗过；③trace context 与 span 只洗**一层**字符串（外加它们各自的 `data` 一层），不递归——更深的结构今天不存在，真出现了应当是一条新的登记，而不是一次悄悄加深的遍历；④`event.extra`／`event.tags`／用户自己 `setContext` 的任意上下文不在本行写集内（**未验证**是否会被写进 token，今天仓内没有这样的调用点）。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：本版修。2026-09-19 追加裁决（关类不补例）：浏览器端不得只关一半，两个 init 用同一只函数，「浏览器端行为不变」放宽为「浏览器端只会洗得更多」。2026-09-19 两镜头复审后补：事务事件那只钩子（`beforeSendTransaction`）一并接上，本行按「扩张 SHARE-A6 判定面」而非「SHARE-A6 未达成」记账。 |
| 2026-09-19 | **§1 问 2 的撤销那一句改了措辞（代裁 3）**。原文写「撤销：排期页现有的 Revoke（`revokeSharePreview`）」——**「排期页现有的」这半句不成立**：staging 实测 `/schedule` 307 回 Home，仓内没有 `apps/web/app/schedule/page.tsx`，`revokeSharePreview` 今天**唯一的调用点**是 Otto 技能 `sharePreviewRevoke`（`apps/web/lib/otto-actions.ts:918`，全仓 grep 除测试外只此一处）。Founder 2026-09-19 裁：**改措辞，不建页面**——撤销面就是 Otto，排期页的撤销入口另案。这是本规格 §1–§4 冻结正文里**唯一**的一处改动，改的是一句与现码不符的描述，不动任何验收判据（SHARE-A7／A8 一字未改，REAL-11 也正是照 Otto 这条路真跑通的）。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：改规格措辞（撤销面＝Otto；排期页撤销入口另案），不建页面。 |
| 2026-09-19 | **归档时照实登记的缺口：十二行验收里八行有回执、四行没有**。2026-09-19 的逐条扫描（`local-logs/spec-acceptance-sweep-2026-09-19.json`）判 §2 十二行为 **11 PROVEN ＋ A6 PARTIAL**（A6 的服务器遥测半句当时还没修，现已由 #1482 关掉），那是代码与测试的证据；**真环境回执**这一侧，2026-09-18 那条客户面旅程给了八行（A1 PARTIAL、A2 PASS、A5 PARTIAL、A6 PARTIAL、A7 PASS、A8 PASS、A9 PARTIAL、A11 PASS），**A3／A4／A10／A12 四行仍是 NOT RUN**（前两行要打挂限流存储、A10 结构上不可测已代裁接受 CI 覆盖、A12 要观察告警送达回执）。回填与逐行理由见 `docs/audits/fullstack-staging-2026-09-14/coverage-matrix.md`。 | 批准: Founder 2026-09-19 对谈授权编排者代裁（原话「我要你现在给我做任何需要我做的决定」）：**本规格状态改「已交付 · 归档」**；四行回执缺口按本行如实留账，不用测试冒充回执。 |
