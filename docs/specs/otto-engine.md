...
| 2026-09-05 | **尾巴组十一落地登记（`otto-actions.ts` 两门的钱话判据 ＋ 确认卡三格的并发，ENGINE-A4 / ENGINE-A3）**。（1）**钱话统一判据**：`ottoApprove` 截断那一支从前只凭 `onRefundedFailure` 点亮的那面旗就对商家说「没收钱」，与同一个 catch 第 5 支「PROVEN, not assumed」的规定正好相反——恢复轮是先跑完被批准的那件工具再撞上步数上限的，那笔生成可能已经付过钱。现在两门共用一个新的只读判据 `chargedNothingProven`（`apps/web/lib/otto-actions.ts`）：要账本上真有一行 `refund:<refId>`（退款函数会返回 `already-settled` / `no-reservation`，而钩子是 `void` 的，那个返回值到不了入口）＋ `otherHoldsSince === "none"`，读不出来就 fail closed 不说这句话；两门的旗因此从布尔换成 `refundedRefId`（钥匙才能回账本对证），钱路一行未动（零 reserve/settle/refund 改动、零幂等键、零迁移）。（2）**两门此前零测试的两处补上**：meter 包装里 `budgetArgs.onRefundedFailure?.()` 的转调（今天引擎不产出这个钩子，故由测试给一份带钩子的 budgetArgs 走一趟；删掉转调 2 红）、以及传给错误文案的 `{ chargedNothing }`（删掉传参 2 红）。判据本身的变异（改回只看那面旗）8 红。（3）**确认卡三格的窄 TOCTOU 合上**：`ottoUpdateGenCardOptions` 从前是先查账本任务行、回到 Node、再改卡，缝里正好是 `startCoworkGen` 建任务行的地方；现在查与写进同一个 `$transaction`，写的那一句是条件更新（`updateMany` 的 `where` 带 id/threadId/ownerId/kind/deletedAt），零行受影响时如实说读不到；改回先查后写 3 红。（4）**跨租户补一条拆闸即红的见证**：原来那条跨租户测试拆掉应用层两道闸仍绿（底下 Prisma 租户守卫会注 `ownerId`），新增一条把身份帧换成无租户系统帧、让守卫放行，只剩应用层闸拦——拆掉它当场红。（5）**追认上一轮的写集溢出**：Founder 裁决③落修轮给六个既有测试文件各加了一行 `onOptionsChanged`（`front-a12-canvas-turn-failure` / `creation-otto-canvas-seam` / `otto-plan-card-detail` / `creation-reference-receipt-card` / `otto-turn-cost` / `otto-narrow-cards`），当时只写在 PR 描述里，此行补记。（6）**已知未做**：`apps/web/components/otto/PackCard.tsx` 是第三处确认位，今天没有三格控件（本组写集不含画布/卡片组件，未动），留待下一刀判是否要一致。 |
