...
| 2026-09-05 | **尾巴组十二落地登记（画布状态卡与错误面，ENGINE-A4 那一半）**：(1) **[#1224 P2-2 已修]** 供应商侧不可恢复的那一档从前顶着 `error` 这个 kind，于是诚实句「Otto is unavailable right now on our side.… Please try again later.」旁边照旧长出一颗「Edit and retry」——一句「等一会儿再说」配一颗「马上再送一次」，每按一次都重走一遍预扣/退款。`OttoErrorData` 沿现有形状加一档 `provider_unavailable`（`apps/web/lib/otto-stream-bridge.ts`），路由按同一份 `isProviderSideFailure` 分类（只动 kind 字段），三处白名单一并更新（`lib/dto.ts`、`lib/otto-status-helpers.ts` 的 `persistedStreamErrorOf`——漏一处就等于刷新之后那一档退回 `error`、重试键自己长回来；渲染层 `OttoStreamErrorNotice` 本来就只给 `error` 出键）。(2) **[#1224 P2-3 已修]** 瞬时那一句从前三份互抄，收成单源 `OTTO_TRANSIENT_FAILURE_SENTENCE`；**它落在 `lib/otto-stream-bridge.ts` 而不是 `lib/otto-error-copy.ts`**，因为后者第一行是 `import "server-only"`（要读 `@fikirtive/db` 的 typed 错误），而这一句客户端组件也要读——文案与它的 kind 住在同一个 `data-error` 契约模块里。#699 的破折号围栏跟着收到那一份上。(3) **[#1225 P2-5 已修]** `canvasTurnStatus` 里 `workingCardCount` 排在失败前面，于是更早一轮的生成还在跑时这一轮的失败被「Generating」盖掉；改成失败优先（`pendingConfirmCount` 与失败的先后一个字没动，钱路那一颗确认键的位置不变）。(4) **[#1225 残留 已修]** 状态卡在充值／抬上限两档补出 `Top up` 与 `Open Billing & credits` 两条出路，字与地址复用抽屉里那张告示导出的同一份常量（地址仍来自导航 registry）。(5) **[#1225 P2-2／P2-4 已修]** 接线层补齐变异可证的断言：两档不给重试键、失败正文是失败色。(6) **[#1225 残留 已修]** 参考取不到那种 400 从前在卡上是通用兜底句、在输入框旁是具体句，现在两处读同一次判定。(7) **写集溢出五处**（清单未列，各改最小一处）：`lib/otto-stream-bridge.ts`（新 kind 与单源文案的家）、`lib/otto-stream-errors.ts`（一行改读单源）、`lib/dto.ts` 与 `lib/otto-status-helpers.ts`（两处 kind 白名单，不改就等于没修）、`lib/__tests__/out-of-credits-copy.test.ts`（破折号围栏原本盯着被删掉的那两个抄写点）。钱路、租户、预扣/退款一行未动。 | |
| 2026-09-05 | **⑥ 段 ENGINE-A7 对照登记**：main 9a1f5292 上 `evals:check`（engine 线 10 题，型号 claude-sonnet-4-6，subject 与 judge 同一型号）总分 25.0/40=62.5% 对基线 26/40=65%（2cb54e2d，即 ENGINE-A1 那份骨架首跑）：**不回退**——差 -2.5 个百分点，落在 runner 自己的 ±5 个百分点噪声容差内（`packages/otto/evals/core.ts` 的 `REGRESSION_TOLERANCE_POINTS`），退出码 0，原文判词「不低于基线。」。ENGINE-A7 那条验收行至此有了真比较对象（此前⑥段是带着敞口交付的）。花费 $0.3014（单次上限 $10；开跑前账本已记 $0.4617，本次最坏估 $0.7833），账本 `evals/baselines/spend.jsonl` 追加一行；`--check` **不覆盖** `baselines/engine.json`，基线仍是 2cb54e2d 那一份。跑法 `env -u ANTHROPIC_BASE_URL pnpm --filter @fikirtive/otto run evals:check`。逐题分（满分各 4）：engine-1:1.5 engine-2:1.0 engine-3:1.5 engine-4:4.0 engine-5:2.5 engine-6:4.0 engine-7:4.0 engine-8:2.5 engine-9:3.0 engine-10:1.0。未过的机械检查与基线同族、不是⑥段引入的新病：engine-1 `mentions-all:seedreamPrompt,propose`、engine-2 `mentions-all:seedancePrompt` 与 `mentions-any:t2v,text-to-video`、engine-3 `mentions-all:proposeStoryboard`、engine-9 `uses-shot-vocabulary`、engine-10 `mentions-all:desiredAspect,9:16`。本次只跑对照、未改⑥段任何代码。 | |
