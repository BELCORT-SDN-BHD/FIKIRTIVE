...
| 2026-09-05 | **尾巴组七落地登记（评测骨架，#1213 判官五条 P2 全落修）**：①**[P2-1] 「本段累计 $20」从前名不副实**——`recordedSegmentUsd` 读的是 `baselines/<line>.json`，而那份档案每跑一次就被整份覆盖，求和只算得到「每条线最后那一趟」，跑三趟只记一趟的钱。改成真累计：新增只追加不覆盖的 `packages/otto/evals/baselines/spend.jsonl`，每成功跑一趟（写档案或 `--check`，两者都真的花钱）追加一行 `{line,date,commit,costUsd}`，累计闸读它求和；读不懂的一行当场炸，不静默按 0 计（fail closed 的闸不许把「读不懂」算成没花钱）。账本首行按 2026-09-05 首跑的 $0.46167 从 `engine.json` 补记——账本后建，但那笔钱真的花了，不该凭空归零。`evals/README.md`「预算」节与 `core.ts` 的 `SEGMENT_BUDGET_USD` 措辞同步改口。②**[P2-2] 「守卫在花钱前」的回归从前只钉住 `guardedRun` 的契约**，判官变异实证：只把 main() 改成先跑后判，当时 22 条测试全绿。把 main() 的接线抽成 `runMain({ preflight, runEvals })`，两件事都是闭包、谁先谁后由这一个函数说了算；回归测试断言守卫不过时 `runEvals` 一次都没被调用（变异实证：改成先跑后判当场红）。③**[P2-3] `forbids` 从前是裸子串**，会冤枉人（`extend` 撞上 “the extended cut”、`Inbox` 撞上 “inboxes”）→ 改按词边界匹配（词组只看两端，大小写仍不敏感）；`mentions-all` / `mentions-any` **不动**，它们判的是「有没有提到」，宁可宽一点。本次改动不动任何题目、不动判分算式，对已入档的基线是纯收窄（原本零命中，收窄后仍零命中）。④**[P2-4] 判分噪声污染「不低于基线」判定**（engine-6 判词自相矛盾，同一份产物两次跑可能差一档；一题 4 分、共 10 题，一个维度抖一档＝总分 1.25 个百分点）→ `evals:check` 的比对留 **±5 个百分点**噪声容差（`REGRESSION_TOLERANCE_POINTS`），可由 `--tolerance=<百分点>` 覆盖，给 `0` 即旧口径；`README.md` 新增一节写明。**不做多跑取中位**（要花好几倍的钱）——登记为后续，等真有必要再议。⑤**[P2-5] `packages/otto/src/otto-acceptance-map.test.ts` 的 ENGINE-A1 由 `it.todo` 转真身**：断言 `evals/baselines/engine.json` 存在且带日期（可解析）、40 位 commit sha、被测与判分两个型号、总分 ∈ (0,1]、真实花费 > 0。**本轮不跑任何真实评测、不发一次模型调用、零花费**；五条各配一次变异实证（改回旧行为当场红）。 | |
| 2026-09-05 | **尾巴组五落地登记（Otto runtime 尾巴，ENGINE-A4 / ENGINE-A6，五条）**：(1) **[⑤ P2-a] 纯读动作不再算交付**：六个只查不写的动作住在 `effect:"write"` 的技能里（`manageCanvas.view`、`manageLibrary.history` / `.detail`、`manageMedia.list` / `.load_more`、`draftWorkflows.validateWorkflowRules`），成功时同样返回 `{ ok:true, … }`，落修前一律被 `countingDeliveryTool` 记成一次落盘 —— 于是「只反复看板、列清单直到跑满步数」的死胡同照收钱，与 ENGINE-A4 相反。**修法（现码最小改法）**：判据留在技能自己家里 —— `packages/otto/src/skill.ts` 的 `OttoSkillSpec` 新增可选 `readOnlyActions: { field, actions }`（`field` 必须是 `parameters` 的 key，工厂在定义期 fail-loud），四个技能各自声明；`packages/otto/src/runtime.ts` 的包装层改收整个 skill，调用时从 SDK 边界那串 JSON 实参里读出动作名比对。**方向**：只有确凿读出判别键、且逐字在名单里才算纯读，读不出来一律退回原行为（记一笔）—— 这一刀只把已证明的纯读从账上摘掉，不新造「该收没收」的口子。**`manageProjects.get_default` 刻意不列入**：它的端口是 `getOrCreateDefaultProject()`，真的可能新建一行 Project，不满足「商家手里什么都没多」。测试：`packages/otto/src/runtime.test.ts` 四条（纯读一轮 ⇒ 判词 null ⇒ 退款分支／同一把工具的真写动作 ⇒ 仍计交付／三次看板＋一次真写 ⇒ 仍计交付／生产六个动作由技能自己声明），`packages/otto/src/skill.test.ts` 两条（判别键校验、字段原样挂上）；变异实证：把 `isPureReadCall` 改成恒 false，只有那一条纯读用例变红。(2) **[④ P2-3] CJK token 估算从每字 2 改成 1.3**（`packages/otto/src/run-input.ts` 新增导出常量 `CJK_TOKENS_PER_CHAR`）：旧值约高估一倍，华语对话因此在 12,000 预算里只装得下英文对话一半的字数就开始折叠丢上下文。**取值依据（是假设，不是实测，注释里写明）**：常用汉字/假名/谚文在 BPE 词表里各收成 1 个 token，生僻字回退成 2–3 个字节片，故真值在 1.0 上下；1.3 是在 1.0 之上留的高估余量（方向仍是早裁）。**仍未实测**，钉死它要一次真的 `count_tokens` 调用，与 ENGINE-A1 基线同一把钥匙、同一趟。测试：`packages/otto/src/history-budget.test.ts` 两条（6,000 字华语历史零折叠、12,000 字触发裁剪并交给摘要）＋ 单位换算那一条改口。(3) **[④ P2-1] 折叠型号从调用处写死改成 manifest 上的一个决定**：`packages/otto/src/model.ts` 新增 `OTTO_SUMMARY_MODEL` 常量（沿用现有型号常量的来源，并进 `assertOttoModelsPriced()` 的组合期查价名单）与 `ottoSummaryModel` 绑定，`OttoModelRuntime` 新增可选 `summaryBinding`（缺省＝主绑定，夹具零改动），`foldRollingSummary` 改从 manifest 取。**今天取值仍等于主力型号，不是偷懒而是没有更便宜的一档**：价目表 `PRICED_MODEL_IDS` 只有 `claude-opus-4-8` / `claude-sonnet-4-6` / `claude-sonnet-4-5`，后两个同价。**真的换小型号要 Founder 两个决定，本轮不自决**：①往 `packages/core/src/llm-prices.ts` 加一行 haiku 档（型号 id 与四个真实单价 —— 猜价已被删掉，查不到就拒绝启动，施工方不得凭记忆填价）；②折叠那条腿的**计价**怎么算 —— 它烧的 token 今天并进本轮 usage、按 `billableModelId`（sonnet）结算，跑小型号却按 sonnet 收对商家是多收，不能默默决定。§7.2④「一次便宜的小调用」今天仍由两端硬顶（输入 24,000 字符、输出 400 token）承担。测试：`packages/otto/src/runtime-history-budget.test.ts` 一条（折叠落在 summaryBinding、主轮落在主绑定，各一次不串）＋ `packages/otto/src/model.test.ts` 一条（组合期查价名单含它、manifest 带下去）；变异实证：把折叠改回 `runtime.modelRuntime.binding`，前一条当场红。(4) **[⑥ P2-5] `packages/otto/src/model.ts` 的提示词缓存注释按现码改口**（该登记原话就是「请下一次碰 model.ts 的那一段顺手改」）：删去「Otto 的**恒定**前缀」与 ~4.7k／~7.7k／~12.4k 三个单体时代的 token 数，改成实话 —— 断点 1（工具块）仍跨轮恒定；断点 2（leading system message）⑥段之后**不再跨轮恒定**（说明书每轮现装，拉进一份新柜文那一轮就付 cache write 而不是 cache read；恢复轮整柜装载是例外），④段之后装载集**不单调**（匹配输入是这一轮此刻带着的上下文，摘要每折一次整段重写），故「一场对话最多 12 次 cache write」不成立；缓存净账与 §7.7 那句 $0.17/轮**都没有数**，等同一趟真跑。(5) **[⑦ P2-2] `packages/otto/src/otto-acceptance-map.test.ts` 抬头名单补上 ENGINE-A3（⑦段 §7.2⑦，真身见 `apps/web/lib/__tests__/engine-a3-canvas-conversation.test.tsx`）**：四条 → 五条；ENGINE-A1 那一条不是本 PR 转正的，是主干 #1221 把它由 `it.todo` 转成了真身断言。**本组三条已在主干做掉、据实跳过，零改动**：⑥ P2-1（`priorState` 为 null 时仍带摘要端口）已落在 `apps/web/app/api/otto/stream/route.ts:307-313` 与 `apps/web/lib/otto-actions.ts:1956`；⑥ P2-2（§5 那句「装载集单调增…12 份上限」）已在本表 ⑥段判官落修轮那一行改成实话；#1198 P2-6（`packages/otto/AGENTS.md` 快查节）已由⑥段更新（`src/skills/CATALOG.md` 57 行与 CATALOG 实测行数一致、两份 golden 与 `evals/` 均在位）。**写集零溢出**：全部落在 `packages/otto/**` 与本行 | |
| 2026-09-05 | **尾巴组五判官落修轮登记（P1 一条已修，P2 四条当场修、一条只是回执措辞）**：**[P1-1] 已修**——`packages/otto/AGENTS.md:37` 那句「基线档案 `evals/baselines/engine.json` 待 Founder 换一把有效 Anthropic 钥匙后才跑得出来」是**过期假话**：该档案早已在库（commit `003cebbe`，10 题、总分 65.0%、花费 $0.4617、对应代码 `2cb54e2d`，20,718 字节），`evals:check` 因此**已有比较对象**（没有基线时 `evals/runner.ts:124-125` 会开跑前就拒绝）。上一轮回执自称「逐条核过现状为真」不实：它核的是目录与文件**存在性**，唯独没核这句话本身的真假。改后写实话，并点明真正未转正的是 ENGINE-A7 那一句「⑥段之后重跑、总分不低于基线」的**对比跑**（跑分仍要一把有效钥匙）。**[P2-1] 不再需要**——主干 #1221 已把 `packages/otto/src/otto-acceptance-map.test.ts` 里 ENGINE-A1 那一行由 `it.todo` 转成真身断言，「`packages/otto/evals/` 今天还不存在」那句过期话随之消失；合并时该文件整份取主干，本 PR 只补抬头名单（见本表尾巴组五那一行的 (5)）。**[P2-2] 已修（口径不变，注释改成实话）**——`packages/otto/src/skills/manage-canvas.ts:211` 从前写「没有任何新东西因这一轮而生」，偏乐观：`view` 前置的 `syncOttoCanvasNodes`（`apps/web/lib/otto-canvas-bridge.ts:152`）**确实会落 `CanvasNode` 行**——商家刚从聊天开工、结算故意不投影的那批**在飞占位卡**。但那是**显示层**节点，只映射已开工／已在 hold 里的批次，不调 `startGen`、不碰供应商、不碰账本，一分钱不会因这一轮多花。故仍不算交付：把它算成交付＝该退不退，与 ENGINE-A4「宁退不收」的方向相反；摘出交付只会让退款更多，钱路对商家只宽不紧。**[P2-3] 已修**——SDK 出处路径错一段：`invoke` 的类型在 `@openai/agents-core@0.11.8` 的 `dist/tool.d.ts:183`，不是 `dist/types/tool.d.ts:183`（该目录下没有 `tool.d.ts`）。签名本身与主张一致，只是引用指不到；`packages/otto/src/runtime.ts:431` 已改。**[P2-5] 已修**——`packages/otto/src/history-budget.test.ts:49` 的上界从 `toBeLessThanOrEqual(1.3)` 改成 `toBeLessThan(2)`：护栏原意是「别退回旧的 2.0（把华语可容纳字数腰斩一半）」，钉死 1.3 会连**向上的实测修正**一起挡红——将来做真 `count_tokens` 实测量出 1.5 也不该是红。**[P2-4] 无需改码**——只是上一轮回执的措辞不准（自述本规格 §5 那行「只出现 A2/A3/A4/A6/A7」，实为 A1/A3/A4/A6）；M2 不受影响，四个都是本规格自己的编号。**仍登记待办（本轮不做）**：① CJK 每字 token 的**实测**——今天 1.3 是假设不是实测，钉死它要一次真的 `count_tokens` 调用；② ENGINE-A7 的对比跑（`evals:check` 重跑、总分不低于基线）待有效钥匙；③ ENGINE-A1 在两份验收映射表里的转正与两表合并。**写集零溢出**：全部落在 `packages/otto/**` 与本行。 | |
| 2026-09-05 | **Founder 2026-09-05 七裁决（AskUserQuestion，逐条原话）回写本表两条**（另两条①②与⑥⑦回写 `docs/specs/frontend-baseline.md` §5 同日行；⑤草稿规格 #1212 见该 PR 评论，不改任何规格正文）。**③精修 pro 档加进确认卡**：「加进确认卡」——下一刀把张数、形状、精修三格加进 Otto 的确认卡，商家批准前可改，精修按 pro 档报价；本行只记裁决与前置条件，验收编号与施工细则由承接该刀的 PR 自行定，行内不写别家规格（`frontend-baseline.md`）的验收编号。**④折叠摘要型号换 Haiku**：「换 Haiku，按 Haiku 实价计入商家账单」——折叠那一步（本表 2026-09-05「[④ P2-1] 折叠型号从调用处写死改成 manifest 上的一个决定」那一行点名的 `OTTO_SUMMARY_MODEL`）改用 Haiku，费用按 Haiku 实价计入本轮；**前置条件**：Founder 本人须先把 Haiku 官方单价贴进 `packages/core/src/llm-prices.ts` 价目表（施工方不得凭记忆填价，同该行既有纪律），本条**待 Founder 贴价后再排期施工**，本行不视为已批准开工。 | Founder 2026-09-05：③加进确认卡；④换 Haiku 且按实价计费，待贴价后施工 |
| 2026-09-05 | **尾巴组走查修复三落地登记（ENGINE-A4，供应商侧失败的诚实文案）**：(1) **病灶**——3310 走查实证（截图 09、13-15）：我们这边的 Anthropic 账户余额不足，服务端拿到 `AI_APICallError` status=400「Your credit balance is too low…」，商家读到的却是 `Otto hit a snag — please try again. Reference: OTTO-…`。那句话在**瞬时**错误上成立、在「我们这边坏了」上是误导：再试永远失败，而每试一次都重新走一遍预扣/退款。(2) **分类与文案同源**——新判词 `isProviderSideFailure`（`apps/web/lib/otto-error-copy.ts:58`）按**结构化**判据分档：401/403 鉴权、404 型号不存在、429 配额、≥500 供应商故障（含 529），400 **只有**报文点名计费/额度才算（其余 400 留给瞬时句——那一档里有「这条消息本身有问题」的可能，改了再试是能成的）；沿 `lastError`/`cause` 递归并带 `seen` 防环，与 `packages/otto/src/model.ts` 的 `isOverloadError` 同一种形状。诚实句＝`Otto is unavailable right now on our side.`＋（属实时）`This turn wasn't charged.`＋`Please try again later.`＋`Reference: OTTO-…`：不点名供应商、不露技术栈、把手照旧。(3) **三门共用一处**：流式门通用失败分支（`apps/web/app/api/otto/stream/route.ts:534`）与 `createUIMessageStream` 的 onError 兜底（:331）、`ottoTurn`（`apps/web/lib/otto-actions.ts:2054`）、`ottoApprove`（:2743）都走同一个 `ottoFailureMessage`。(4) **扣费状态属实**：「没收钱」只在 ENGINE-A4 那个只读钩子 `onRefundedFailure` 真的点亮时才说；为此把 `chargedNothing` 的声明从内层 try 提到外层 try 之前（`otto-actions.ts:1827`、:2110），因为供应商侧失败正是被文末那个 catch 接住的，声明留在里面等于那句话在唯一需要它的地方读不到。`ottoApprove` 更严一档：只在**没有任何一张卡被消费**时才说（`claimedPayload === null`，:2114 同步提到外层；理由同 #524 r6——`chargedNothing` 只证明这一轮的预扣退了，恢复轮可能已先跑完并付过被批准的那件工具），卡被消费那一支照旧由第 5 支自己的句子负责。(5) **⑤段登记里留的单源化这次做了**：`This turn wasn't charged.` 与 `I got a bit tangled up — try asking again.` 此前三门各抄一份，现在全仓只有 `otto-error-copy.ts` 一处（`TURN_NOT_CHARGED_SENTENCE` / `OTTO_DEGRADE_SENTENCE` / `ottoDegradeText`），文案一个字未变。(6) **测试**：新增 `apps/web/lib/__tests__/otto-provider-failure-copy.test.ts`（分类五档＋瞬时对照＋包装/环＋文案＋按 `git ls-files` 全仓钉「字面量只有一处」）与 `otto-stream-route.test.ts` 一条真路径行为测试（400 计费错误 → 诚实句＋「没收钱」＋Reference，落盘的 TURN_ERROR 与流上逐字相同）。**变异实证**：把 `isProviderSideFailure` 改成 `return false`（分类表退回一律 snag），两个文件当场红 10 条。(7) **写集零溢出，两件留给下一段**：① 通知组件 `apps/web/components/otto/OttoStreamErrorNotice.tsx:51` 的「Edit and retry」按钮仍挂在 `kind === "error"` 上——要摘掉它得给 `OttoErrorData` 新增一个 kind，会连带 `otto-stream-bridge.ts`／`otto-status-helpers.ts`／`dto.ts` 三处白名单，超出本组写集，故本次只改文案（句子已把「later」说清）；② `packages/otto/src/otto-acceptance-map.test.ts` 的 ENGINE-A4 指针未加本次新测试文件（跨模块，本 PR 一行未动 `packages/`）。 | |
| 2026-09-05 | **Founder 裁决③落地登记（ENGINE-A3，确认卡加张数／形状／精修三格）**。裁决原话「加进确认卡」：⑦段退役直出 composer 之后，这三格在商家那一侧无处可选，而确认卡是今天唯一的花钱入口。**落地形状（按现码做，五处与清单写法不同）**：(1) **改档 = 服务端重铸整张卡，不是卡面算一个新价**——新增纯函数 `packages/otto/src/skills/propose-card-options.ts`（`applyCardOptions` / `cardOptionMenu`）与 $0 Server Action `apps/web/lib/otto-actions.ts` 的 `ottoUpdateGenCardOptions`。理由是钱路的单一权威本来就是**持久化的那张卡**：`startCoworkGen` 从卡读 `estimatedCredits` 当 `expectedCredits`，再用 `pricedGenCredits` 现算一次比对，对不上就在 create/reserve **之前**拒（`apps/web/lib/gen-actions.ts:979`）。所以「报价与预扣同源」不是靠约定，而是靠界面**一分钱都不算**：菜单来自卡上新增的 `options`，价与规格条目由服务端重算后随新卡一起回来。变异证据钉在 `apps/web/lib/__tests__/otto-card-options-ledger.test.ts`（把卡上的价钉死在旧值 ⇒ 付费路拒、ledger 零新增行）。(2) **精修接的是既有能力位，不是新价目**——`fineDetail` 在铸卡侧与改档侧都只做一件事：按可售判据（`isSellableImageSku`，与付费闸同一个函数）挑 pro 槽位，价仍由 `pricedGenCredits` 按那一档算；`buildGenRequestFromCard` 一个字节没动（付费请求带走的仍是卡上的 `model`），Creation 规格里「未定价的 pro SKU ⇒ 拒绝、$0」那条服务端围栏一格未碰。商家侧读法是**卡上不出现这一格**（`options.fineDetailAvailable` 为 false），不是给一个点了必然被拒的开关。(3) **`propose` 新增 `fineDetail`（图片专用）**，与画质档那条逐字同形：没点名 ⇒ 一格不动；点了名而这一格今天没有价 ⇒ 新拒绝 `ImageFineDetailUnavailableError`（`ProposeRefusal` 家族，入口按基类接住）；换到精修档之后形状必须重新落在**那一档**的菜单上（pro 的像素上限更低，16:9 / 9:16 收不下），落不上由既有的画幅两证人如实拒绝，绝不静默换一格。(4) **三格控件是一个共用组件**（`apps/web/components/otto/CardOptionControls.tsx`），同时接在两处确认位上——对话抽屉里的 `OttoPlanCard` 与画布上始终可见的 `OttoTurnCard`；两处各抄一份就是把「改了什么、按什么价」复制成两份（全局法 7.3），与参考回执同一条理由。已排队／已完成／已取消的卡不再出现这三格，已有任务行（`cowork:<cardId>`）的卡服务端直接拒改。(5) **写集溢出三个文件，均为最小的那一处**：`packages/otto/src/index.ts`（只新增导出，零行为）、`apps/web/lib/otto-client-actions.ts`（跨客户端边界的薄包装，零逻辑）、`apps/web/components/otto/plan-card-contract.ts`（DTO 边界上新增两格的运行时解析——不加它，新字段会被静默丢掉，那正是该文件存在的理由）。**测试**：纯判词 `packages/otto/src/skills/propose-card-options.test.ts` 与 `propose.test.ts` 新增一节；账本 `apps/web/lib/__tests__/otto-card-options-ledger.test.ts`（真库、真 reserve/settle，含跨租户与已在跑两条拒改）；界面 `apps/web/lib/__tests__/otto-card-options-ui.test.tsx`。**未做（留登记）**：视频卡的三格（一条片子不成组、画质走 `desiredResolution` 自己那条路），以及两处确认位并存这件事本身——`OttoTurnCard` 与 `OttoPlanCard` 是否该合成一处，属画布状态卡那份小规格的范围。 | |
| 2026-09-05 | **Founder 裁决③判官落修轮登记（ENGINE-A3，一条 P1 已修、P2 三条当场修一条只记录）**。(1) **[P1-1 已修] 重铸后的卡不再停在组件里**——同一个 cardId 今天有两处确认位（抽屉里的 `apps/web/components/otto/OttoPlanCard.tsx`、画布上的 `apps/web/components/otto/OttoTurnCard.tsx`），而画布形态下抽屉只是 CSS 隐藏、不是卸载（`apps/web/components/otto/OttoChatStream.tsx` 的 `canvasHistoryOpen ? "flex" : "hidden"`）。两处各留一份 `reminted` 局部 state，就会一处写着新价、另一处仍按旧价出 Generate；而批准请求不带价（服务端从库里那张卡重建），陈旧那一侧按下去照旧按新价预扣。落修：两个 `reminted` 整块删掉，改由父组件 `OttoChatStream` 的 `applyRemintedCard(cardId, payload)` 改写该条消息的 `metadata.payload`，两处从此读同一份、下一帧一起换；轮询回来的库里那一份照旧压过它。变异证据：把抽屉卡的 payload 冻住（模拟旧写法），`apps/web/lib/__tests__/otto-card-options-ui.test.tsx` 新增的「两处确认位同挂」那条与原有的「重铸换价」那条同时转红。(2) **[P2-1 已修] 精修可售派生补上变异证据**——旧断言两边都是今天的真值（同义反复，把派生写死成 `true` 照样全绿）。新增 `packages/otto/src/skills/propose-card-options.sellability.test.ts`：把 `isSellableImageSku` 对 pro 打桩成 false，钉「菜单那一格变 false」与「打开精修被拒、卡一个字节不动」；打桩只在该文件生效，原 15 条判词一个字未动。(3) **[P2-2 已修] record-only 引擎成本按槽位取钉点**——`packages/otto/src/skills/propose-card-options.ts` 与 `packages/otto/src/skills/propose.helpers.ts` 的 `estimatedPriceUsd` 从写死的 lite 基数改成 `genImageCostUsd(model)`（`packages/core/src/gen.ts` 的既有映射，pro 走自己的钉点）。报价路径一格未动，仍只有 `pricedGenCredits` 一处。(4) **[P2-3 已修] 验收映射表补指针**——`packages/otto/src/otto-acceptance-map.test.ts` 的 ENGINE-A3 那一行补上本轮四份证据文件。(5) **[P2-4 只记录，不改码] 清单字面的 `imageFineDetailCapability` 路线没走**，改由卡上的 `model` 承载：付费请求里只有 `model: seedream-pro`（`packages/core/src/gen-from-card.ts`），`fineDetail` 全仓只被能力路由与付费 schema 的交叉校验消费，没有适配器读它；未定价的 pro SKU 仍由 `packages/core/src/model-config.ts` 的 `assertSpendableModel` 按白名单拒、$0，`packages/core` 本轮零改动。行为等价，字面不同，留待 S5 认或不认。 |
| 2026-09-05 | **Founder 裁决④落地登记（折叠摘要换 Haiku，按 Haiku 实价计入商家账单）**。**裁决原话**：「折叠摘要换 Haiku，按 Haiku 实价计入商家账单」（Founder 2026-09-05）。**前置条件的解除有出处**：上一行「七裁决回写」④记的前置条件是「Founder 本人须先把 Haiku 官方单价贴进价目表」；Founder 2026-09-05 在会话中把「本人贴价」改为授权 agent 按官方定价页取数并处理（编排者代记，原话与两名取证员结果见 <https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/pull/1228#issuecomment-5551252203>），因此本条按该授权开工，价目仍不来自任何人的记忆。本条同时**关闭** `model.ts` 里 `OTTO_SUMMARY_MODEL` 注释登记的那两个待决问题（加价目行 / 折叠腿怎么计价）。**(a) 官方单价（每百万 token 美元，2026-09-05 由两名取证员独立抓取且一致）**：型号 id `claude-haiku-4-5-20251001`；Base input **1**；Output **5**；Cache hits and refreshes（缓存读）**0.10**；5m cache writes（缓存写）**1.25**。来源：<https://platform.claude.com/docs/en/about-claude/pricing>（型号 id 另见 <https://platform.claude.com/docs/en/models/overview>）。落在 `packages/core/src/llm-prices.ts` 的 `TABLE` 新增一行（perToken = 每百万数字 ×1e-6），来源 URL 与日期写在该行注释里；`PRICED_MODEL_IDS` 自动含它。**(b) 折叠腿计价口径改法**：从前折叠烧的 token 被 `addTokenUsage` 直接并进本轮 usage，整包按 `billableModelId`（Sonnet）结算——**本改动之前折叠自己也跑 Sonnet，那样算逐分正确，从未多收**；但**折叠一旦换成更便宜的型号**，同一段代码就成了跑便宜的按贵的收，对商家是多收。现在折叠腿**单独记成一条计费明细**（`meter.ts` 的 `LlmBillingLeg`，不变量 #13）：本轮总额 = 主腿按主型号价 + 折叠腿按 Haiku 价。**没有改的**：幂等键、refId、reserve/settle 的 exactly-once 结构（仍是一次 reserve、一次 settle、一个 refId）、毛利加成的两个常量（`OTTO_LLM_MARGIN` / `OTTO_CONVERSATION_TURN_MARGIN`）——但施加方式变了：**按腿各自计入后相加，取整因此各取一次，单轮最多多 1 内部 credit（$0.01），方向是多收；是否改成合计后一次取整待 Founder 定**、折叠的两端硬顶（输入 24,000 字符、输出 400 token）。**(c) 三处与原写法不同、均按现码做**：①manifest 多一个字段 `summaryBillableModelId`（缺省 = `billableModelId`，所以夹具与 CLI 那几份 manifest 一个字未改）；②折叠腿的 529 接管型号**刻意不跨价档**——新常量 `OTTO_SUMMARY_FALLBACK_MODEL = OTTO_SUMMARY_MODEL`，因为用 Sonnet 接管一次 Haiku 折叠就是跑贵的按便宜的收，而折叠从不 load-bearing（抛错即吞、这一轮照常返回），代价只是偶尔少折一次摘要；③腿的价钱在 meter 侧与登记表**逐字段取大者**而不是像主腿那样抛错——这一步跑在 `fn` 之后、settle 之前，抛出去会把预扣挂在那里（既不结算也不退款）。**(d) 截断轮口径未变**：`usageOnError` 只交回错误状态里的用量，折叠腿的 token 在截断轮上仍不计费（本改动之前即如此）。**验收**：沿用 ENGINE-A6，八条行为测试见 `packages/core/src/llm-prices.test.ts` 与 `packages/otto/src/runtime-history-budget.test.ts`、`packages/otto/src/model.test.ts` | 编排者代记 Founder 会话原话：<https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/pull/1228#issuecomment-5551252203>；建议 Founder S5 时在该评论下补一行本人确认 |
| 2026-09-05 | **尾巴组十一落地登记（`otto-actions.ts` 两门的钱话判据 ＋ 确认卡三格的并发，ENGINE-A4 / ENGINE-A3）**。（1）**钱话统一判据**：`ottoApprove` 截断那一支从前只凭 `onRefundedFailure` 点亮的那面旗就对商家说「没收钱」，与同一个 catch 第 5 支「PROVEN, not assumed」的规定正好相反——恢复轮是先跑完被批准的那件工具再撞上步数上限的，那笔生成可能已经付过钱。现在两门共用一个新的只读判据 `chargedNothingProven`（`apps/web/lib/otto-actions.ts`）：要账本上真有一行 `refund:<refId>`（退款函数会返回 `already-settled` / `no-reservation`，而钩子是 `void` 的，那个返回值到不了入口）＋ `otherHoldsSince === "none"`，读不出来就 fail closed 不说这句话；两门的旗因此从布尔换成 `refundedRefId`（钥匙才能回账本对证），钱路一行未动（零 reserve/settle/refund 改动、零幂等键、零迁移）。（2）**两门此前零测试的两处补上**：meter 包装里 `budgetArgs.onRefundedFailure?.()` 的转调（今天引擎不产出这个钩子，故由测试给一份带钩子的 budgetArgs 走一趟；删掉转调 2 红）、以及传给错误文案的 `{ chargedNothing }`（删掉传参 2 红）。判据本身的变异（改回只看那面旗）8 红。（3）**确认卡三格：查与写进同一个事务、写句补齐条件**（措辞已按判官 P1-1 纠正，见（7）①：窄 TOCTOU **并未合上**）：`ottoUpdateGenCardOptions` 从前是先查账本任务行、回到 Node、再改卡；现在查与写进同一个 `$transaction`，写的那一句是条件更新（`updateMany` 的 `where` 带 id/threadId/ownerId/kind/deletedAt），零行受影响时如实说读不到、不假报成功；改回先查后写 3 红（红的是形状断言，不是并发见证）。（4）**跨租户补一条拆闸即红的见证（钉的是读闸）**：原来那条跨租户测试拆掉应用层两道闸仍绿（底下 Prisma 租户守卫会注 `ownerId`），新增一条把身份帧换成无租户系统帧、让守卫放行读，拆掉应用层读闸当场红；红的机制是别人的卡被**读**了出来并进了改档路径，越权**写**仍被租户守卫拒（`ChatMessage.updateMany requires runAsTenant before system writes`），故不能读成「只靠应用层闸拦住越权写」（判官 P2-4，措辞已改）。（5）**追认上一轮的写集溢出**：Founder 裁决③落修轮给六个既有测试文件各加了一行 `onOptionsChanged`（`front-a12-canvas-turn-failure` / `creation-otto-canvas-seam` / `otto-plan-card-detail` / `creation-reference-receipt-card` / `otto-turn-cost` / `otto-narrow-cards`），当时只写在 PR 描述里，此行补记。（6）**已知未做**：`apps/web/components/otto/PackCard.tsx` 是第三处确认位，今天没有三格控件（本组写集不含画布/卡片组件，未动），留待下一刀判是否要一致。（7）**判官落修轮登记（本 PR 第二轮）**：**①[P1-1 已修，改的是措辞不是行为]** 上一轮把这道闸说成「窄 TOCTOU 合上了／缝合上了」是不实主张：Prisma 交互式事务不传 `isolationLevel` 即 PostgreSQL 默认 READ COMMITTED，事务内那条 SELECT 对**尚不存在**的 GenJob 行不取谓词锁；而 interactive transaction 每条语句都要回一次 Node，窗口的物理长度也没变短。并发的 `startCoworkGen` 在两条语句之间插入 `cowork:<cardId>` 仍然看不见，`updateMany` 仍会落地（幻读）。三处 durable 假话已改成实话（`apps/web/lib/otto-actions.ts` 那道闸的注释、`apps/web/lib/__tests__/otto-actions.test.ts` 该 describe 的抬头与那句「证据」注释、本行（3））。现有代码**不回退**（新加的 `where` 条件与 `count === 0` 如实报 `Card not found.` 是净改进）。**已知未做 ＋ 触发条件**：把「未在跑」搬进 UPDATE 谓词（`NOT EXISTS (SELECT 1 FROM "GenJob" …)`）才是真正原子的写法；触发＝下一次动这条路时改，它绕过 Prisma 租户守卫，须另配一条真库并发测试。**②[P2-3 已修]** `ottoTurn` 那条转调测试的 refId 断言从家族前缀正则改成**逐字全串**（探针 refId 改成不同形的 `probe:NOT-MINE`，断言 `refund:<这一门自己的 refId>`）——旧断言对探针值同样成立，「记的是自己那把钥匙」在这一侧此前未被证明（`ottoApprove` 那条本来就是逐字全串，未动）。**③[P2-1 只登记，不改码]** 「统一判据」只统到 `ottoTurn` 与 `ottoApprove` 两门；**流式门**（`apps/web/app/api/otto/stream/route.ts` 的 `chargedNothing` 裸旗 → `ottoDegradeText` / `ottoFailureMessage`）本 PR 一行未动，今天仍用旧的裸钩子判据，同一句商家可读的钱话因此在三门有两套举证标准。留下一刀：把 `chargedNothingProven` 提成三门共用。**④[P2-2 只登记，不改码]** `ottoApprove` catch 第 5 支的 `chargeVerdict` 仍是旧标准（`refundedRefId !== null` ＋ `otherHoldsSince`），**不查** `refund:<refId>` 那一行，因此现在比同一个 catch 的第 4 支松。改它会连带 `markApprovalFailed` 与卡面文案、需要自己的测试，不宜顺手做，留下一刀。**⑤[P2-5 只登记，不改码，两处副作用]** (a) `otherHoldsSince` 是 **org 级**（同 org 内任何 `RESERVE` 且 `createdAt >= 本轮 reserve`，不限本次动作或本条对话），`ottoTurn` 从前没有这道：同一商家在本轮预扣之后开始的任何一笔生成，都会把本来属实的「没收钱」吞掉——方向保守、不撒谎，但商家会少读到一次真话；日后要更准，应按本次动作的 refId 家族收窄。(b) `applyCardOptions` 现在跑在账本判据**之前**，「卡已在跑 ＋ 编辑本身非法（超张数上限、这一档收不下的画幅）」这个角落返回的错误从 already under way 变成编辑错误——零写入、非缺陷。 |
