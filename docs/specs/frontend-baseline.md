...
| 2026-09-03 | **Create 起步页的「+ Add context」参考菜单本段不渲染，Founder 具名裁决未能照字面落地**。裁决原文要求它「接线到现有能力（上传／从素材库选；贴网址无契约则不显示并登记）」。实测阻塞：起步页没有把参考带进画布的后端契约 —— `apps/web/lib/canvas-entry-actions.ts:41` 的 `createCanvasConversation` 只解析 `{prompt, requestId}`；同文件 `:87` handoff 行只落 `{prompt, threadId}`；`:102`/`:120` 的 `getCanvasConversationHandoff` 返回类型就是 `{prompt}`；上传侧 `apps/web/lib/upload-actions.ts:157` 的 `finalizeCandidateUploads(projectId, …)` 第一个参数就要 projectId，而起步页提交前根本没有 projectId。夹具那三条（`apps/web/design-system/patterns/canvas/CreationComposer.tsx:113`/`:114`/`:115`，以及 `:101` 的上传回调）全都只改一个显示字串、什么都不落库，照搬即是 Founder 规则①要禁的假按钮。第⑨段据规则①整组不渲染（`components/start-something/StartSomething.tsx` 控件行改 `justify-end`，发送键位置与夹具逐像素同位），并登记于此。**待 Founder 二选一**：① 立项「起步页参考契约」（handoff payload 增引用字段＋画布侧消费，跨 `components/canvas/**` 与 `apps/web/lib/`，须另开写集），补上 Upload image / Choose from Library 两条；② 认可本段先不渲染，本条转下一循环。 |  |
