...
| 2026-09-05 | **登记轮 · 本场判官与走查剩下的四条 P2（前端基线侧，本轮不做）**。① **判官 #1180 P2-1**（评论 5534795997）：`apps/web/app/globals.css:367` 的注释写「what every surface under `.gb` declares」，措辞偏差——`--foreground` 只由 `.gb` 的两个主题根声明（`:502` 与 `:629`），不是「每个面各自声明」；未改。② **判官 #1180 P2-3**：那一票双方的真浏览器核验都是 CSS 级联镜像页，不是整 app 走查；下次矩阵在真 app 的画布出片弹窗上对文字对比度复量一次收口。③ **判官 #1185 P2**（评论 5537276609）：两面素材的可读名口径不完全一致——素材库网格用 `conciseAssetTitle`（无类型后缀，靠 Choose ／ Open ／ Actions 前缀区分，`apps/web/components/otto/stuff/StuffLibrary.tsx:112`），画布的 Choose from Library 半屏用 `libraryItemAccessibleName`（带类型后缀，`apps/web/components/canvas/CanvasLibraryPicker.tsx:145`）；规则源头已经是同一份（`apps/web/lib/library-item-a11y.ts`），差的是要不要统一后缀策略。④ **矩阵走查（2026-09-04 09:45，本机 3310 生产构建）画布一条**：生成刚落卡的那一瞬间图片卡短暂点不中（react-flow 节点残留 `visibility:hidden`），稍后自行恢复。**同轮订正一条结论**：矩阵门 4 记的 ❌ 是**假阴性**——多选证伪 worker（2026-09-04 10:15，主干 8b3318ef）实测 Claude Browser 的修饰键点击不发真 keydown，而 React Flow 12.11 只认 window 上真实的 Shift keydown；macOS 上 Ctrl ＋ 点击是右键；空白拖拽＝手型平移、Select 工具框选正常（裁决六）。同一次实测留下的窄条件缺陷：Meta ＋ 点击在上一次点击落在 Otto 输入框之后失效，Shift 不受影响 | 登记；待排期 |
