...
| 2026-09-04 | **Brand 的「预览效果」偏离裁决四，待 Founder 追认**。裁决四（本节 2026-09-03 行与 §7.3④「草稿流动作链」）要的 Preview effect 是**调模型出样例文案**，并在点之前把那笔小额成本按钱引擎的计费口径披露。本 PR 的 `previewBrandContextEffect`（`apps/web/lib/memory-actions.ts:384`）换成了**免费、不调模型**的另一件事：把这条草稿保存前后 Otto 真读到的两段品牌上下文原文并排摆出来。理由——调这一次模型要花的钱，今天在集中配置里没有单一权威价目；没有价目就做不到「披露先于扣费」，那就不该有这颗会扣钱的按钮。要回到设计原样，须先给这次模型调用定价（另立规格），本行是这处偏离的登记。 | |
