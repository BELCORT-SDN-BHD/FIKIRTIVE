...
| 2026-09-03 | 上传素材 Regenerate：因 regen 路不携带源图（`handleRegen` 无 `sourceGenerationId`），兜底句会让商家花钱得到无关结果，暂维持拒绝；待 i2i 请求形状接上后再开放。触发＝PR #1145 修复者指出。证据：`apps/web/components/asset/DetailPanel.tsx` 的 `handleRegen` 请求体无 `sourceGenerationId`；兜底表 `ASSET_ACTION_FALLBACK_PROMPTS`（`packages/core/src/gen.ts`）只留 `animate`；行为测试 `apps/web/lib/__tests__/animate-uploaded-source.test.ts`「上传的图按 Regenerate ⇒ 原地拒收、$0、连 GenJob 都不建」 | 裁决待 Founder |
