...
- 清单上标 **Not yet confirmed** 的行是另一回事:哨兵首见它时**读不动账本**,所以还没有人确认过
  它到底是不是缺口 —— 记下来只是为了不让它滑出 48 小时窗口后消失,下一轮账本读得动时自会判定。
  除非你另有确证,否则先让它跑下一轮,不必急着关。
- 这笔付款是用别的方式了结的(退了款、是测试 session)⇒ 人工关闭:**admin 面 → Reconciliation**
  ...
  - **Credits granted by hand** —— 填账本 refId 或补发时用的幂等键,**服务端当场查账本**,查不到不许关。
    补发时有两条硬要求(否则关闭动作会拒绝):
    1. **补发的 reason 里必须粘上完整的 session id**(`cs_…`)。同一个商家同一天补两次同额,
       金额与形态都对得上 —— 不指名就分不出是哪一笔,拿错的那一笔关掉,商家仍然少一份钱。
       (webhook 后来补投成功、幂等键是 `stripe:<session id>` 的那种自动入账形态同样算指名。)
    2. **一行补发只能关一个缺口**。系统按账本行 id 记一条占用标记,同一行再用于关第二笔缺口会被
       当场拒绝,并告诉你它已经关了哪一笔。真有两笔缺口,就补两笔。
