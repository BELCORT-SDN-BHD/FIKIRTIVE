# SOP v1.1：ComfyUI 工作流 → Artlio 模板 / Modal 端点
...
> 依据：设计文档 `~/.gstack/projects/artlio/winnin-master-design-20260610-024210.md`（"技术栈定案"表第 7/9/10 行）。

## 阶段适用性（先看这个）

| 部分 | 何时可执行 | 说明 |
|------|-----------|------|
| Part A（组包 + 注册） | **今天即可** | Artlio 应用未上线前，注册 = 写入文件版注册表（见 A3.5） |
| Part B（Modal 部署） | **阶段二解锁** | 解锁条件：环境前置块全部就绪 + Artlio 回调端点（或 mock 收口）可用 |
...
| R2 凭证（account_id/bucket/key） | Modal Secret `artlio-r2` | boto3 `head_bucket` 成功 |
| HMAC 回调密钥 | Modal Secret `artlio-hmac` + Artlio 侧 env `MODAL_CALLBACK_SECRET` | 双侧一致性自检 |
| Artlio 回调地址 | env `ARTLIO_CALLBACK_BASE`（Artlio 未上线时填 mock 收口 URL，如 webhook.site，冒烟仅验证投递） | POST 测试包 200 |
| GPU 默认表 | 本文件下表 | — |
...
3. 用途一句话 + **槽位精确声明**：创始人必须给出精确标题字符串（如 `槽位：ARTLIO:prompt, ARTLIO:ref_character`）。中文意译（"角色参考图"）不是声明——agent 此时回示精确字符串清单请创始人确认，**不许模糊匹配**。
4. **每个 ref 槽一张默认参考图文件**（组包与冒烟测试必需；纯 t2i 无 ref 槽则免）。

## Part A：制成 Artlio 模板（今天即可执行）

### A1. 槽位标题检查

解析 `workflow_api.json`（顶层为 `节点ID → {class_type, inputs, _meta.title}` 扁平 map）：

1. **扫描全部节点的 `_meta.title`，匹配 `ARTLIO:` 前缀**——不按 class_type 筛（自定义编码器/加载器如 WanVideoTextEncode、LoadImageFromUrl 同样合法）。class_type 仅记录入 manifest 作诊断信息。
2. 标题匹配规则：**精确字符串匹配**（区分大小写，无空白容差）。`artlio: prompt` ≠ `ARTLIO:prompt` → 报告为未匹配。
3. 验证：声明的每个槽位**有且只有一个**精确匹配节点。
   - 缺标题 → 列出候选节点（class_type + 现标题）让创始人在 ComfyUI 改名后重导。**不许替创始人猜。**
   - 重复标题 → 响亮报错列冲突节点。静默选一个 = 本 SOP 最严重违规。
4. **多余的 `ARTLIO:` 节点**（有标题但未声明）：照常收入 manifest 的 slots，并在报告中标注"未声明槽位，已收录"——声明清单是必须存在校验，不是排他白名单。
...
```json
{
  ...
  "exported_at": "<ISO 时间戳>",
  "slots": [
    { "title": "ARTLIO:prompt", "declared": true, "node_id": "6", "class_type": "CLIPTextEncode", "field": "inputs.text" },
    { "title": "ARTLIO:ref_character", "declared": true, "node_id": "12", "class_type": "LoadImage", "field": "inputs.image" }
  ],
...
{ "slug": "i2v-character-walk", "version": 1, "hash": "<template_version_hash>",
  ...
  "slots": ["ARTLIO:prompt", "ARTLIO:ref_character"], "registered_at": "<ISO>",
  "modal_endpoint": null, "gpu": null, "cost_usd_per_run": null }
```

判重：`hash` 已存在 → 同版本跳过并报告；新 hash → 新版本，旧行保留（不可变历史）。**M0 上线后**：Artlio 启动时导入该 registry.json，此文件继续作为真相源直到阶段二站内注册表替代它（届时 agent 改写本节）。

### A4. 填槽（Artlio 运行时行为，实现参照）
...
2. 创建/复用 Modal Volume `artlio-weights`；agent 写并运行一次性下载 job 灌装；`modal volume ls` 验证文件与大小。
...
3. 渲染函数尾部（三保险之二、三）：产物写本地盘 → SHA-256 → **boto3 multipart 直传 R2**（key = `u/<owner_id>/<hash>`，凭证自 Secret `artlio-r2`）。**禁止** CloudBucketMount 直写（不支持 ffmpeg muxing 随机写）；**禁止**函数返回值传视频（~100MB gRPC 上限）→ 只回 `{generation_id, content_hash, size, mime, duration_s, attempt}`。
4. 完成回调：HMAC（Secret `artlio-hmac`）签名 POST 到 `$ARTLIO_CALLBACK_BASE/api/modal/callback`。
...
3. 冒烟：模板默认输入真跑一次 → 验证 ①R2 出现哈希命名对象（用 r2 凭证 head_object）②回调投递到 `$ARTLIO_CALLBACK_BASE`（mock 收口时验证收到即可）③status 可查。
...
1. 在 ComfyUI 设计工作流；2. 给填槽节点起 `ARTLIO:` 标题；3. Save (API Format) 导出，连同用途一句话 + **精确槽位字符串** + 每 ref 槽一张默认图丢给 agent；4. 收报告点头或改图。
