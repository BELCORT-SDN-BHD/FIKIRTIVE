...
- 判据 3（租户泄露）**本轮证据不足以触发，但「不成立」要按下面三条限定读**（第 2 轮收紧）：**已证**的是读路零命中、深链无越权读（`backend §5.1`／`§5.2`）；**未证**的是写路（`PRODID-A9` 后半从未真试，只有代码形状证据）；**另有一处要 Founder 知道**：`FSE-209` 说的「登录审计 26 行全挂在 `founder` 名下」意味着**任何按 `ownerId` 读 `ActionEvent` 的 founder 侧界面都会读到别的租户的登录邮箱** —— 本轮没有找到也没有取证过这样的界面，所以现状是「**未发现暴露面**」，不是「已证明没有暴露面」。
- 判据 4（进错账号／进不去／密码面复活）**不成立** —— `credential` 行数 0、`password` 全 NULL、七个旧端点全 404（`backend §2.2`、`ledger §R2-04`）。
- 判据 5（版本不对应）**不成立，但只覆盖「部署这一面」**（第 2 轮收紧）：已证的是走查全程 `2a96750e`、web／worker 同版、中途未换版（`ledger §0 P0-1`，`backend §0` 复核）。**没覆盖的是「范围这一面」** —— 必测范围里的前置票 **#1320／PR #1349（登录门⑤）走查期间全程 OPEN、根本没上线**（preflight P0-2 判 PARTIAL）。受它影响的 `SIGNIN-A12` 已照实记 NOT RUN；其余登录门条目跑的是**已上线的那 14 张票**的行为，判定成立。
...
**范围**：`coverage-matrix.md` 共 **65 行判定**（第 2 轮重数更正，上一版写 46 行是**少数了 19 行**）：**PASS 17／PARTIAL 21／FAIL 5／NOT RUN 22**。即 **22 行整条没跑**、21 行只跑了一半。留第三轮的项目未执行、未判定。
...
| **A6** Google 门复验＋登录路径闭合 | **未执行完（范围缺口，非产品失败）** | 通过的部分很硬，但**每一句都带边界**：密码面彻底退役（机器闭合，七端点 404 ＋ `credential` 行 0）；暂停新注册**在码门这半边**全流程正确（陌生人真的不寄码、零建号）—— **Google 那半边一次没跑，所以 `SIGNIN-A6` 整个编号不算通过**；大小写归一**只证到「没造出第二行」**（全表零非小写），规格要的「两种大小写各登一次 → 同一个账号」本轮只登了一次 ⇒ `SIGNIN-A16` **第 2 轮由 PASS 改回 PARTIAL**。未执行：Google 门 A2／A13／A14／A12 全 NOT RUN（缺陌生 Google 夹具 ＋ #1320／PR #1349 走查期间 OPEN）、A7／A15 NOT RUN、A8 有一条文案缺陷（**FSE-201**） |
...
> **第 2 轮重数更正**：上一版写「46 行」是**少数了 19 行**（少的全在 PARTIAL 与 NOT RUN 两列，PASS 与 FAIL 两个数一直是对的）。判官逐行点数、W3 复核并用脚本重数确认：**全表 65 行判定**。逐表点数见 `coverage-matrix.md`「汇总计数」，任何人可以自己数一遍。

| 判定 | W1 走查后 | W2 取证后 | **W3 第 2 轮修订后（现行）** |
|---|---:|---:|---:|
| PASS | 14 | 19 | **17** |
| PARTIAL | 24 | 19 | **21** |
| FAIL | 4 | 5 | **5** |
...
**W3 的两处改判都是往下改**：`R2-20 §5 :162④`（PASS → PARTIAL，全称句有反例）、`SIGNIN-A16`（PASS → PARTIAL，借号）。
**BLOCKED 不再单列**：`R2-24 §5 :178` 整行是 PARTIAL，②③⑤ 三格 BLOCKED 是行**内**的格，不是独立一行；上一版把行与格混着数了。工具侧 BLOCKED 的事实本身没变（见 §1.2）。
**这次重数只会让 NO-GO 更硬**：22 行整条没跑、21 行只跑一半。

---

## 3. 每个 PASS 的证据落点（plan §3.4 要求截图；本轮有一处必须写明的偏差）

**偏差（对冻结计划的一处正式偏离，不要读成「证据齐全」）**：`plan.md` §3.4 要求「涉及余额、账本、错误提示的判定，截图必须能同屏看到判定依据」，§3.5 对账表也逐条点名了要哪张图。**本轮不满足这条**：本目录只有 **4 张截图**，且全是**登录页**（无头 Chrome 只能拍未登录可达的 URL 首屏）；`coverage-matrix.md` 大多数行的截图格是「无」。其余 PASS 的现场证据改用 **verbatim 页面文本**（`get_page_text`／`read_page` 原文抄进 `run-ledger.md`）＋ 后端查表。
...
- ⇒ **:162 第三句按「前半 PASS、后半未触发」记**，不再写成整句已核。它不改 R2-11 的整体判定（正路两张参考直接出片那部分证据独立且硬），但下一轮必须补一条「带官方演员的失败样本」才能验后半句。失败文案不说原因那一面仍由 **FSE-204** 承接。

**另有两条不构成借号但口径要写明**：R2-18（只覆盖服务器交付的字节，**不覆盖**点 `Download` 后本地落盘）、SIGNIN-A6（码门半边 PASS、Google 半边 NOT RUN ⇒ **整个编号不算通过**）。
...
| SIGNIN-A15 | NOT RUN | 需人为制造建号中途失败（断库连接）。**理由更正**：Founder #1330 评论第 4 条已逐字授权失败注入 ⇒ **未执行是时间与取舍，不是没批准**（上一版写「未获授权」不属实，`run-ledger.md` §NOT RUN 清单同字样一并更正） |
...
| 4 | `SIGNIN-A15／A17` 的 NOT RUN 理由写「**未获授权**」，与 Founder 本人在 #1330 的授权记录**直接抵触** —— 把执行取舍说成 Founder 没批 | **P1** | **已关闭** —— 三处理由全改：Founder #1330 评论第 4 条**已逐字授权**失败注入与批量开号，**未执行是时间与取舍**。同一条授权本轮已被用来改 `SIGNUPS_PAUSED`，走查方知道它存在。`run-ledger.md` §NOT RUN 清单同字样，按惯例**在报告里更正、不改现场记录** | 报告 §1.2、§4 两行；`coverage-matrix.md` A15／A17 两行 |
| 5 | **覆盖计数错**：全表实际 **65 行**判定（NOT RUN 22、PARTIAL 19），报告与矩阵写成 46 行 —— 对一份以覆盖率为产出的报告是实质失真 | P2（W3 复核后按 P1 处理） | **已关闭** —— 用脚本对四张表逐行重数复核，判官的点数**成立**。全表改成 **65 行**，并给出**逐表点数表**让任何人自己数；`BLOCKED` 不再单列（它是 `R2-24 :178` 行内的三格，不是独立行）。现行计数 **PASS 17／PARTIAL 21／FAIL 5／NOT RUN 22**。**方向不变，NO-GO 更硬** | 报告 §0 范围行、§2 计数块；`coverage-matrix.md` 汇总计数整节重写 |
...
| 14 | 判据 3（租户隔离）的排除**证据不足**：`FSE-209` 记录了其他租户的登录事件挂在 `founder` 名下（含邮箱），「payload 只含邮箱」不足以排除跨租户边界问题 | P1 | **已关闭（往下改口径，不是往上凑）** —— 判据 3 改成「**本轮证据不足以触发**」并三段限定：已证读路与深链、**未证写路**、**未发现暴露面 ≠ 已证明没有暴露面**（任何按 `ownerId` 读 `ActionEvent` 的 founder 侧界面都会读到别的租户邮箱，本轮没找到也没取证过这样的界面）。`FSE-209` 的复测口径里补了这一面 | 报告 §0 判据 3；`findings-catalog.md` FSE-209 建议格 |
...
**这一轮修订对结论的影响**：**NO-GO 不变，且更硬** —— PASS 少一条（17）、PARTIAL 多两条（21）、NOT RUN 从 9 改成 22。判据 1（A1–A6 有未通过或未执行）与判据 2（P1：FSE-204）仍然是触发项；判据 4（进错账号／密码面复活）仍然明确不成立；判据 3 与判据 5 从「不成立」改成「**带限定的不成立**」，理由见上表第 14、15 条。

---

*产出文件清单见 `plan.md` §8。*

**本报告改了哪些文件（两个口径分开写 —— 第 2 轮更正，上一版把「本次 commit」说成了「PR 全量 diff」）**

| 口径 | 内容 |
|---|---|
| **PR [#1351](https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/pull/1351) 的全量 diff** | **9 个文件**：4 张登录页截图 ＋ `run-ledger.md`（W1）、`backend-evidence.md`（W2）、`coverage-matrix.md`（W1 写 / W2 回填 / W3 修订）、`findings-catalog.md`（W1 写 / W3 追加）、`report-round2.md`（W3）。**`plan.md` 一字未改**，与 `origin/main` 逐字节相同（blob `80aab12b44c098198c0be23a78facf51e5681017`）。 |
| **W3 第 2 轮修订这一次改的** | 3 个文件：`report-round2.md`、`coverage-matrix.md`（A16 改判 ＋ 计数重数 ＋ A15/A17 理由更正）、`findings-catalog.md`（删码值 ＋ 收紧 FSE-209/210 措辞 ＋ 补 FSE-211）。**未改** `plan.md`、`run-ledger.md`、`backend-evidence.md` —— 那两份是 W1／W2 的现场与取证记录，对它们的事实更正一律写在本报告里（§7.1 三处、§4 两处），**不动原始记录**。 |
