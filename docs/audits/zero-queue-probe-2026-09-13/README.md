...
| +7,082ms | 200 | `running` |
| +14,182 ~ +64,095ms（7 次） | 200 | `running`（未观测到 `queued`） |
...
零成本旁证（非 QUEUE-A7 正式验收点，顺手记录）：单条提交耗时 71.2s，四条并发的完成时刻却铺开到 66.7s–107.6s——API 层面从未报告 `queued`，但服务端渲染队列内部显然仍有排队/争抢（这与「账户接纳层不排队、渲染层仍受真实算力约束」并不矛盾）。
