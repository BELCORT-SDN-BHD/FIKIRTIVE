...
| REAL-03 | EXT-10 EXT-08 | 品牌商品从首页到人物视频 | NOT RUN | staging 第三轮付费旅程第一组（2026-09-16）：fixture 前置条件未达成——`/brand` 五分区全部枚举后确认零产品创建控件，Library Elements Products 空态只读，`@` 菜单无建产品选项，DB 核实 org founder 零存活 PRODUCT Entity（唯一一条 2026-09-11 已软删）；worker 未直接调用后端 action 或驱动 Otto 代为建产品（会伪造商家没有的路径且花费未预算的对话额度），如实报 NOT RUN 而非跳过。根因即 R3-F20（指路死路，修复 PR #1463 在飞）；路径已查明，付费组待该 PR 合并或另行授权直建后重跑。证据：[步骤](real-scenarios.md#real-03-商家场景品牌商品从首页到人物视频)、`local-logs/staging-r3-paid/preflight-fixtures.json` 步骤 FIX-1。 |
