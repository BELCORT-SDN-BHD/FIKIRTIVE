...
| #1458 | `28f3cdfa` | 翻闸前置：四颗形状雷根治、warn 基线重取、A3/A4 真扣费路径复验——本报告撰写期间合并，已重新 rebase 核对 |

**在合并链中，撰写时仍为 OPEN**（`gh pr view <n> --json state` 核实于 2026-09-17；#1458 已于撰写期间合并，见上表）：#1462（自动封面撤菜单项，见上文 A①）、#1452（画布 Shift 多选修复，见上文 A③；本 PR 分支最终 HEAD `141a6ba6`）、#1461（后台可丢弃单条死信）、#1459（CRM 未知路由送回冻结去处）。另有两条本次核实时同样 OPEN、超出原始清单但与本报告直接相关：#1464（上传理解回执，见上文 A②）、#1463（R3-F20 指路修复，见上文与 findings-catalog）。
