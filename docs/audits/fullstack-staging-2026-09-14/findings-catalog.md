...
## R3-F34 · Otto 面板长回答后滚轮失灵、回复框在可视区外、不自动到底
...
修后本机真量到的（1280×720、停靠 360px、真 Chromium、production build，`e2e/journeys/31-otto-panel-scroll.spec.ts`）：**修前**（把这一处改回去重新 build 做变异实证）体 `scrollHeight 6063 / clientHeight 586`、视口 `5848 === 5848`（零行程）、回复框 `bottom 6081.25`（在浏览器可视区下面 5,361px）、滚轮上下各一次 `scrollTop 0 → 0`、视口 `scrollTop 0`（不在底部）、展开视图体 `3333/586` 同样全红——11 条软断言无一幸免，与 staging 现场同形。**修后**体 `586 === 586`（不再溢出）、视口 `clientHeight 372 / scrollHeight 5848`（行程 5,476px）、打开即 `scrollTop 5476`（＝最大值，自动到底活着）、回复框 `top 537.5 / bottom 605`（面板 0..720 之内）、滚轮上 `5476 → 4876`、下 `4876 → 5476`，展开视图 `2746 → 2146`。
...
证据：`local-logs/staging-r3-otto-panel/repro.json`（`dom_under_test`、`root_cause`、`control_experiment`、`measurements` 四节；`auto_scroll_to_bottom` 记 345 次采样，`additional_finding_streaming_layout_runaway` 记暴涨那一条，`side_observation_enter_to_send` 记 Enter 那一条）。走查全程只读，除一次 Otto 对话（2 credits）外未改任何生产状态，页面按原样留回。
