...
**状态**：**已执行——那条被污染的品牌记忆已于 2026-09-18 12:38:41Z 从 staging 删除**（Founder 2026-09-18 对谈裁决「删除。」，由编排者在 staging 数据上执行，不走 PR；原状态：新发现，未修，登记（隐私／质量））。删法走的是产品自己那颗键——那条事实卡头部的「Remove this context」，不是直接改库：点下去没有确认对话框，当场消失并弹一句「Removed. You can restore it from the list.」，左栏多出一个「Removed」清单、里面挂着它和一颗「Restore」——即**软删**，`deletedAt` 被写上，商家自己随时捞得回来。库里只读复核对得上：`Memory 01M2N6Y9NEK11F406B9WDJFF2S`（`ownerId=founder`）的 `deletedAt` 由 NULL 变成 `2026-09-18 12:38:41.178`，`content ilike '%solid orange%' and "deletedAt" is null` 的计数归零；**其余事实一格未动**——Audiences 那条 `Memory 01M1ZQWCNQYDP75JGJ9VG9MHSW` 仍 `deletedAt=NULL`、内容逐字节相同，org founder 的记忆行数仍是 2（其中 1 条活着）。证据：`local-logs/staging-r3-paid/f22-delete-evidence.json`（前后对照 + 只读 DB 复核；连库串从头到尾只活在 shell 变量里，没落盘、没上过命令行）。**产品侧的两处根因仍然开着，不随本次删除关闭**：`origin` 标成 `manual` 而非素材来源、Otto 自写的句子直接落 `Ready` 跳过商家确认闸——两条未裁、未修，正文留在下方等后续。裁决登记见 `report-round3.md` 2026-09-18（四）一节的裁决表第⑧条，执行登记见同文件 2026-09-18（五）一节。原始证据同上，`preflight-fixtures.json` 的 `findings[3]`；DB 层复核见 `workflow-group1-result.json` 的 `verdicts[0].findings[2]`。
...
**状态**：**已修复并合入主干**——PR #1466，合并 commit `66ef15fa`（2026-09-17T07:22:52Z，`gh pr view 1466` 核实）。装配层改读 `@fikirtive/core` 的 `videoAttachmentRole`，模型自填的 `mode` 只在有服务端证据时算数；提示词里的首帧两句只在真有首帧时出现。**仍开着的口子（不读作已关）**：复审 P3 两条随本 PR 内的 commit `0766f536` 落进规格——`docs/specs/creation-engine.md` §5 2026-09-17 行的「未关掉的缺口」由三条改四条，新增 (d)「候选名单本身读失败时整道闸停摆」（`loadAvailableRefsForAgent`，`apps/web/lib/otto-actions.ts:599-600` 是 best-effort，prisma 一出错就 `return []`，那一轮整道闸判「没有演员」），并另记一条 TOCTOU 理论例外（同一轮里 `Entity.type` 在两次读之间由 CHARACTER 改成 PRODUCT，闸读旧值、卡读新值），不采取行动、只为免得子集关系被读成绝对成立。**批准**：该规格行仍写着「批准: 待 Founder 追认」（编排者依 2026-09-15 裁决先行施工）。
...
**状态**：**已关闭（2026-09-19 收官据复测判定；复测本身执行于 2026-09-18 18:53–19:03Z，build `bd5877c3`，含门槛 sha `e81366f8`）**。修复 PR #1476（合并 `e81366f8`）之后三条腿在真浏览器上原样再走一遍，**走查原报的「空白 40–60 秒」一次都没复现**：① 切回标签页——可见性翻成 visible 之后 **+174 毫秒**就补读了一次线程（上一格定时读 18:54:10.373、下一格本该 18:54:12.873，这一次 18:54:12.719，明显不在节拍上），随后节拍从返回那一刻重新起算；② 返回腿 A（侧栏链接走客户端导航再按 Back）——面板根本没卸载，会话正文全程在 DOM 里，零 `pageshow`／零 `visibilitychange`／零服务端动作，也就没有空窗；③ 返回腿 B（地址栏整页跳走再按 Back）与纯刷新腿——那句 `Opening your conversation…` **0.44–0.50 秒**上屏，正文 **0.84／1.15 秒**补齐。顺带把此前只是推论的一句坐实了：Home 文档响应真的带 `no-store`，标记变量在 Back 之后不见了，**bfcache 确实被挡掉**，所以 `pageshow(persisted)` 这条路在本 build 上不可达。**残余（照实登记，不阻断关闭）**：2026-09-18 复审回修的四格里，(d)（只在本来就是快轮时才重上膛）、(e)（切走那一声不读／终态不读）、(g)（每批活只在第一次切回时重臂）这三条本轮**没在真环境走到**——那一单图片 37 秒就跑完，快轮额度 2.5s×48≈2 分钟根本没烧完，两句关于钱的话（「This is taking longer than usual…」「This looks stuck…」）压根没资格出现，要测得换一单视频级的长任务；(f)（一次返回只读一次）在本 build 上**结构性不可达**——bfcache 被 `no-store` 挡掉，`pageshow` 与 `visibilitychange` 不会连敲，去重旗没有机会被压。四格今天都只有单元测试（`otto-return-refresh-r3f27.test.tsx` 8 条，含变异实证）。钱：一张已批的图 1 credit ＋ 一轮 Otto 聊天 2.1 credits ＝ 3.1 credits，零退款、零悬挂预留。证据：`local-logs/staging-r3-f27/{r3-f27-return-legs.json,baseline.txt,final-panel-textcontent.txt}`。**以下是原状态行，保留作历史**：修复 PR #1476（真浏览器复测待做，未关闭）——Founder 2026-09-18 裁（对谈）：修；登记行见 `docs/specs/frontend-baseline.md` §5 的 2026-09-18 行。**当场受益的只有「切走标签页再切回来」那一腿**；按返回那一腿走整份文档重载（Home 是 `force-dynamic`，`apps/web/app/(home)/page.tsx:22`，动态页文档响应带 `no-store`，而 `no-store` 是 Chrome 明写的 bfcache 阻断项——响应头未在 staging 实测，这半句是推论）或走 App Router 客户端回退（popstate）时，`pageshow(persisted)` 与 `visibilitychange` 都不会敲，本票给它的只有那句看得见的等待文案。按返回与纯刷新两腿须一次真浏览器复测才能判，复测之前此条不关。修掉的是两条读码坐实的根因——① 回到这一页（切回前台／bfcache 摊开）没有任何人补读一次，观察窗只剩计时器一个触发源，慢档一格 60 秒且在 bfcache 里是冻住的；② 面板取种子那几秒正文只有灰条加一条 `sr-only`，屏幕上一个字都没有。**40–60 秒这个数字本身未复现、未测量**：冷加载路径上仍有一次 `loadOttoPanelSeed` 服务端动作要等（时延未量），且本轮另一组证据独立记录过 harness 的 `get_page_text`／`read_page` 会对刚导航完、其实已渲染的标签页连续几十秒返回空文档（`workflow-groups2-4-result.json`），两者可能各占一部分——要坐实需一次 staging 时延追踪，已登记待裁。
...
**状态**：**已关闭（2026-09-18）**——修复合入主干、真浏览器复跑两步全 PASS、代码形状三条由围栏钉住、规格行当天获 Founder 追认，四样齐了。PR #1469，合并 commit `66f9c766`（2026-09-18T12:09:08Z，`gh pr view 1469` 核实）。两刀：①`FlowCanvas` 的 `onNewNode` 按 id 去重（重放同一张卡不再往板子那份清单里塞第二条同 id 记录，屏幕上本来就只有一张图，翻倍的是**数**这份清单的那些读数——工具条消失、底部「Download 2」、凭空的「Batch of 2」），`nodeDataRef` 改成合并写入；②画布交给对话的四个接缝回调改成稳定身份、空写入不换身份（`NorthstarCanvasWorkspace`）。**复审两轮后落地的三处更正，照实记**：(a) 第一刀**这两步走查现场都解释不了**——S13（Create variations）走 `runImageEvolve`、每次带全新 actionId／genJobId，S10（第二标签重放）走 Otto 审批那条路、根本不经过 `onNewNode`；原稿把第一刀的射程写宽了；(b) 原稿说的第三处翻倍「在飞付费卡计数把同一个任务算两次」是**假的**，已删——`hasInFlightPaidNode`（`FlowCanvas.tsx:1405`）与 `canvasJobActive`（`:1416`）都是 `.some()` 的布尔值、不是计数；(c) `fitView` 留在 `setNodes` updater 之外（该文件自己的纯度规矩），不为省一次遍历破例。**当时未证成的那一半，已于 2026-09-18 在真浏览器上证成**：`Minified React error #185` 在 jsdom 里复现不出来（环境限制，不是产品被证无罪），所以那一刀当时只能靠 staging 真跑判定——复跑组 `verify-r3-f28` 在 build `66f9c766` 上把走查现场那两步原样再跑一次，**两步都 PASS**：① S13 从节点工具条确认「Create variations」（12:43:33Z 点，12:43:43Z 读 DOM＋控制台）——零控制台错误、无 `Minified React error #185`、无 maximum update depth、无错误边界那句「This canvas didn't open」，「2 selected」／「Download 2」／「Batch of」在确认当刻、结算之后、点新卡之后三次扫描全部零命中，画布 3 张卡变 4 张、恰一条新 GenJob `01M2T8PAWHG1A768CMQB8J823P`、恰一对 RESERVE／SETTLE；② S10 第二标签重放已批卡（标签 2 在批准之前 12:46:55Z 就载好、到 12:48:26Z 还停在未批准的「Generate · 1 credit」，是真陈旧态）——标签 1 批准一次得 GenJob `01M2T8WT62K7VXTBXZV1KVHQRQ`，标签 2 于 12:48:35Z 重放那一下**零新 GenJob、零新账本行**、不崩、不出多选条，两个标签最后收敛成同一块 5 张卡的画布，重放出来的那张卡 Animate 照旧按得动（验收①②③）。控制台那份「空错误列表」不是哑读数：同一标签里先故意抛过一个探针错误并读到了，才据以判空。**核证员独立复核三条判定全数维持**，并把 worker 自报的唯一一条「未证成」升级为已证成：`cowork:<cardId>` 这把幂等钥匙不在账本上、在 `GenJob` 上——`GenJob 01M2T8WT62K7VXTBXZV1KVHQRQ.idempotencyKey = cowork:01M2T8TM7BCKYDAW5BTW8ZA681`，而 `packages/db/prisma/schema.prisma` 的 `GenJob` 注释写明有一条局部唯一索引 `GenJob_cowork_idempotency_once` 让 `cowork:%` 跨所有状态唯一——于是「重放零写入」不只是这一次观测到的现象，是数据库约束结构性保证的：一张卡这辈子只生成得了一次。**钱面 MONEY-F28 PASS**：全天 org founder 的账本恰 6 行，两对生成 RESERVE／SETTLE（各 1 credit、各 US$0.035）＋ 一对 Otto 聊天（净 2.4 credits，worker 自己披露、没藏），零 REFUND、零悬挂预留、余额与屏幕逐位对上；本组花费 US$0.07，累计 US$0.94，远低于 US$16 暂停线与 US$20 封顶。**代码形状那三条（验收⑤⑥⑦）不靠真跑，靠围栏**：`apps/web/lib/__tests__/canvas-replayed-placement-r3f28.test.ts`、`canvas-seam-callback-stability-r3f28.test.ts`、`canvas-seam-effect-deps-r3f28.test.ts` 三份都在主干上（本 PR 分支 `ls` 核实）。**核证员另外推翻了 worker 的一条旁记，勿再当缺陷登记**：「2026-09-17 那次 S13 崩溃弄丢了一张商家已付费的卡」不成立——`GenJob 01M2PX2N965JH5HCYCGBWA9BS2` 的 `threadId` 为 NULL、幂等钥匙属 `asset:regen:` 族，不走画布卡那条路（`threadId` 只在 Cowork 生成卡发起时才写），它自己那张 `Generation` 好端端地生出来了；而且它建于 05:22:52，比那次崩溃任务（05:20:23）晚约 2 分半，因果上也轮不到崩溃背锅。规格登记：`docs/specs/frontend-baseline.md` §5 2026-09-17 行（含当时未证成部分的如实登记），其「批准:」格已于 2026-09-18 由 Founder 对谈追认（原话「这四条追认」）。证据：`local-logs/staging-r3-paid/verify-r3-f28.json`（执行）与 `verify-r3-f28-verifier.json`（独立核证）。
...
**状态**：修复 PR #1475（Founder 2026-09-18 裁「本版修，派生图继承源图记录」；验收与副作用见 `docs/specs/brand-product-identity.md` §5 2026-09-18 PRODID-R12 行）。
...
## R3-F33 · 一次重部署把正开着画布的商家送进一屏全白，没有任何「请刷新」的话（新发现，待 Founder 裁 fix-now 或排队）

**状态**：新发现，未修，登记（部署／构建切换行为，不是代码回归；交 Founder 裁 fix-now 或排队）。2026-09-18 复跑 R3-F28 的过程中撞上：12:50-12:51Z 之间 staging 从 `66f9c766` 换成 `80d1e280`，而这一跳的 `git diff` 只有 7 份文档（`docs/DEFERRED.md`、`docs/audits/**`、4 份规格 `.md`），`git diff --name-only 66f9c766 80d1e280 -- apps/web packages` 返回**零**个文件（本 PR 分支复核，与核证员结论一致）——**产品代码一个字没改，白屏照样发生**，所以这条不记在任何一条修复账上，是部署切换本身的形状。

现场：两个开着的画布标签在 12:50:21Z 重新载入后**双双全白**——`document` GET 200、所有 JS chunk 200、`readyState` 是 `complete`，DOM 里躺着一个没解开的 Suspense 占位（`DIV#S:0`），`document.body.innerText.length = 0`，零控制台错误、零错误边界。12:51:58Z 读 `/api/health` 才看出原委：build 已经换了，这两个标签手里的 chunk／RSC 引用指着已经退役的那次部署。新构建完全上线之后再刷一次（12:53:27Z）两标签都恢复正常、画面一致。**对商家意味着什么**：我们每合一次 PR（哪怕只是改文档）都会重部署，正在画布上干活的商家这一刻看到的不是「新版本来了，刷新一下」，也不是一句错误，而是**一屏什么都没有的白**——错误边界不会接手（因为没有抛错），加载态也不出现（因为文档本身 200 且已 complete），全靠商家自己猜要再按一次刷新。与 R3-F21（重部署后已开页面的上传失败，被误导文案说成商家网络问题）是同一个根下的两片叶子：**已经开着的那一页不知道脚下的部署换了**。这一条只登记现象与代价，不预设修法。
...
## R3-F34 · Otto 面板长回答后滚轮失灵、回复框在可视区外、不自动到底（走查判定 PARTIAL）

**严重度**：走查按自己的口径判 **PARTIAL**，不是 REPRODUCED——它的定义是「商家用滚轮或键盘都够不着回复框」，而拖面板右边那条 10px 滚动条、以及从页顶按 23 下 Tab，这两条路当天实测都还能到。但自然手势（滚轮）在整块面板里完全无效、回答完成也不自动到底，而那两条能走的路无人能发现，所以商家实际读到的就是「面板卡死了」。原文判词与两条替代路径的实测见 `local-logs/staging-r3-otto-panel/repro.json` 的 `verdict_detail`。

**状态**：**已关闭（2026-09-19）**——修复 PR #1487 合并 `92193a4d`（2026-09-19T12:14:36Z，`gh pr view 1487 --json mergeCommit,mergedAt` 现查），staging 部署到这颗 commit 之后当天 12:19–12:25Z 复测，**D-098 PASS、D-099 有答**，两条台账（`docs/DEFERRED.md` D-098／D-099）同日销账。① **D-098 PASS**：流式那 36.5 秒里 **75 个无缝样本**（间隔 496–504 毫秒，窗口无缺口），`body.scrollHeight` 每一次都等于 `clientHeight`（600／600，比值恰好 1.00，阈值 ≤2 倍），走查修前采到的 2,131,081→4,832,520→8,261,642px 那个暴涨环一次都没回来（**这组修前数字出自修前走查记录，不在本次复测证据集内，核证员未能核对**——见 `retest/verifier.json`，核证员原话是不要再转引这组数，并指出修复提交自己的本机变异实证记的是 6,063 vs 586px、比它小三个数量级，两者必有一处记错；修后的结果不靠这组数成立）；回答写完那一刻会话视口**落底 0 px**（阈值 ≤4，且 `10997 − 386 − 10611 = 0` 自洽）。**这不是本条自己警告过的那种「零行程空转」假通过**——视口真有 10,611px 行程，最新那条消息高 2,817px、顶端在 −2295，只有真的自动到底才可能把它的下缘（522）放进面板（面板下缘 768）。② **D-099：Enter 真的发送**，而且「Return 不发送」那条走查观察是**哈尼斯的键名问题、不是产品缺陷**——键名「Return」到达输入框的那下 keydown 带着 `key=""`／`keyCode=0`（既没发出去也没插换行，`defaultPrevented` false），任何应用都无法对一个没有键名的事件动作；换成键名「Enter」当场发出（`key="Enter"`、`isComposing=false`、冒泡后 `defaultPrevented=true`、输入框清空）。应用侧的闸逐字核在 `origin/main`：`apps/web/components/otto/OttoChatStream.tsx:1301`，同文件 `:2523` 的提示写着「Enter sends. Shift+Enter starts a new line.」。**服务器侧独立坐实**：数据库里有一条 12:24:35.544 的 7 字 USER 消息（`谢谢，先到这里`）、12:24:38.408 的 AGENT 回复、12:24:35.748 的 `otto-stream` RESERVE——这一下是在数据库里证明的，不只是在页面上。**残留一格照实记**：文字是注入的、`isComposing` 全程 false，中文输入法**组字那条分支没被走到**。③ 三条回归腿 PASS（停靠态滚轮上下各 300px、回复框 `top 552／bottom 619` 稳在面板 0..768 与窗口内、展开与收起后高度链和滚轮都照旧）。④ 钱：**零 `GenJob`**（founder 仍 37 条，最新一条建于前一天 18:53），四条新账本行全是 Otto 文字聊天自己的计费（2.1 ＋ 0.4 ＝ 2.5 credits，与屏幕上两句「This reply used N credits.」逐字对上）——没有任何付费生成卡被提出或批准。⑤ **证据纪律缺口一处**：截图没落盘（哈尼斯只把图返回在转录里），所以三条回归腿只有数值一条来源；独立核证员据此把它们记作「按数值判 PASS、图证缺失」，并把执行 worker 写的「106 个样本」收紧为「75 个无缝样本」、把「消息条数 8→11」订正为 **8→10**（11 只是流式中途的瞬态，数据库落地的只有 2 条）。**⑥ 覆盖面旁注（核证员提出，照实记）**：本条三个效果里，**「打开有长回答的对话直接停在最新一句」那一腿，staging 复测没有走真正的新开页面／新开对话**——最近的替代只有展开重排（开局 dfb 0）与发送前那一次采样（正文已有 4,842 字、dfb 0）。这一腿由 e2e 旅程 31（`e2e/journeys/31-otto-panel-scroll.spec.ts`，「打开就停在最新一句」）守着，所以整体有围栏，只是**没有在 staging 上重新证过**。**两条观察级旁注，都不是新缺陷、不另开编号**：**OBS-1**——从展开视图收回停靠时不重新贴底（收回后距底 1332px，40 秒后发下一条消息即自行归位）；这判为良性甚至正确：商家本来就是自己往上滚的，两种布局内容高度不同（展开 6085px vs 停靠 10503px），同一阅读位置映射出的像素差本就更大，滚轮照旧能用、自动到底在下一条消息时照旧生效。**OBS-2**——面板里没有任何带标签的「正在生成／停止」按钮，「是否在流式」在无障碍树上读不出来（75 个样本里零命中），本轮只好用文本长度增长来判流式；将来要写可靠的流式断言、或做一次 a11y 观察时值得记着这一条。证据：`local-logs/staging-r3-otto-panel/retest/{r3-f34-retest.json,samples.json,baseline.json,verifier.json}`。**以下是原状态行，保留作历史**：修复 PR #1487（staging 复测待做，两条待测项进台账 **D-098**〔流式回答写完那一刻是否自动到底 ＋ 流式期间 `scrollHeight` 是否仍会暴涨〕与 **D-099**〔「Enter 不发送」用真键盘验一次〕，触发条件都是下一次 staging 部署带上本 PR）。修的是一处 CSS 高度链：`apps/web/design-system/patterns/otto-panel/OttoPanel.tsx` 的 `[data-otto-panel-body]` 补上 `flex flex-col`（顺带给 `OttoThreadList` 补 `shrink-0`，它在一列 flex 里本来会被压扁）。规格登记见 `docs/specs/frontend-baseline.md` §5 的 2026-09-19 行（批准: Founder 2026-09-19 对谈授权编排者代裁）。围栏两道：结构合同 `apps/web/lib/__tests__/otto-panel-scroll-chain-r3f34.test.tsx`（4 条，jsdom）、真浏览器量尺 `e2e/journeys/31-otto-panel-scroll.spec.ts`（真 Chromium、真排版、真滚轮）。

现场（staging build `bd5877c3`，2026-09-19，只读浏览器 agent 复现，1024×768 CSS px；逐步证据 `local-logs/staging-r3-otto-panel/repro.json`，只读走查花费 2 credits）：商家问了一句「怎么规划一个月的 Instagram 内容」，Otto 答了 4,870 字。① **回复框不见了**——`textarea[aria-label="Reply to Otto"]` 的矩形落在可视区下面 **6,982px**（干净重载后仍在下面 527px；面板打开时 `panelBody.scrollTop` 是 0，从不停在底部）。② **滚轮全死**——鼠标停在会话正文上滚 10 下、2×10 下、上滚、下滚、贴右边缘滚，`panelBody.scrollTop` 全程 983 不动；滚轮事件确实到了（`deltaY 1000`、window-capture / panel-bubble / document-bubble 三个阶段读到的 `defaultPrevented` 都是 false），`el.scrollTop=200` 赋得进去并留得住，同一份合成输入在页面上临时插的一个对照 `div`（300×200、`overflow-y:auto`、2000px 子元素）上 `scrollTop 0 → 500` 滚得动——所以是面板专门不收，不是自动化假象。③ **回答完成不自动到底**——从发送前到之后 143 秒、每 400ms 一次共 345 次采样，`panelBody.scrollTop` 每一次都是 983，而 `scrollHeight` 从 1583 长到 8714；那条 4,870 字的回答整段在折线下面，从没被带进视野。④ **展开视图同病**——点「Expand Otto」面板变 614px，结构与陷阱逐字相同，商家被钉在底部、滚不回去读回答的开头。能到回复框的只剩两条商家想不到的路：拖面板右边那条 10px 经典滚动条（`offsetWidth-clientWidth = 10`，实测 `983 → 8114` 有效），或者从页顶按 23 下 Tab 让浏览器的 focus-scroll-into-view 把面板体拖过去。（走查因此判 PARTIAL：按它自己的口径「商家用滚轮或键盘都够不着」不完全成立；但自然手势完全无效，两条可用的路无人能发现。）

病根（读码＋实测坐实，两处叠在一起）：① `[data-otto-panel-body]` 从前只有 `min-h-0 flex-1 overflow-y-auto`，**少了 `flex flex-col`**——实测 `computed display` 就是 `block`。于是子层 `[data-otto-panel-conversation-wrap]`（`min-h-0 flex-1 flex-col`，`apps/web/design-system/patterns/otto-panel/OttoPanelHost.tsx`）是块级子元素而不是 flex 项，它写着的 `flex:1 1 0%` 是一句空话，高度改由内容决定：实测 `height 1582.75px`，而父亲 `clientHeight` 只有 600。整条会话列连同回复框一起长到体外面。② 高度链一断，`apps/web/design-system/primitives/message-scroller.tsx` 的视口（`size-full min-h-0 min-w-0 … overflow-y-auto overscroll-contain contain-content`）那句 `height:100%` 就对着一个由内容撑出来的父亲解析，`clientHeight === scrollHeight` 恒成立（三次采样 1368/1368、8499/8499、4886/4886，`maxScroll` 恒为 0）：它仍然是一个滚动容器、仍然带着 `overscroll-behavior-y: contain`，却一格行程都没有——Chrome 把滚轮咬在它身上，找到 0px 可滚，然后停下，永远不往上传给真的能滚的 `[data-otto-panel-body]`。回答长起来之后这个视口的矩形从页面 y −831 铺到 7669，覆盖整块可见面板，连走查一开始找到的那条 63px「顶部逃生带」都没了。已排除：监听器 `preventDefault`（三个阶段读到的都是 false）、`pointer-events`（每一级祖先都是 `auto`）、JS 钉住 `scrollTop`（程序赋值留得住）、自动化假象（对照实验）。

修后本机真量到的（1280×720、停靠 360px、真 Chromium、production build，`e2e/journeys/31-otto-panel-scroll.spec.ts`）：**修前**（把这一处改回去重新 build 做变异实证）体 `scrollHeight 6063 / clientHeight 586`、视口 `5848 === 5848`（零行程）、回复框 `bottom 6081.25`（在浏览器可视区下面 5,361px）、最新那条消息的下缘落在 `5983.75`（面板下缘 720）、滚轮上下各一次 `scrollTop 0 → 0`、展开视图体 `3333/586`、视口 `3118 === 3118`、回复框 `bottom 3351.25`、滚轮同样不动。旅程的 13 条判定全部是 `expect.soft`，所以这一次跑**一口气报全**：**13 条里红了 11 条**（原文逐条见 `local-logs/staging-r3-otto-panel/local-before-after.json` 的 `mutationRunSoftFailures.failures`）。没红的两条照实说明：① 「回复框被推到面板上边界外面」是单侧护栏，而这个缺陷只往下推、从不往上推；② 「会话视口没有停在自己的最底」在零行程时**空转**（`scrollHeight - clientHeight - scrollTop` 恒等于 0，读起来永远在底部）——真正抓住这一格的是同段那条「最新那条消息的结尾不在面板里」，它正是为此补上的。**修后**体 `586 === 586`（不再溢出）、视口 `clientHeight 372 / scrollHeight 5848`（行程 5,476px）、打开即 `scrollTop 5476`（＝最大值，自动到底活着）、回复框 `top 537.5 / bottom 605`（面板 0..720 之内）、滚轮上 `5476 → 4876`、下 `4876 → 5476`，展开视图 `2746 → 2146`。

尚未证实的那两格（照实记）：**流式期间 `scrollHeight` 暴涨**——走查另记到一条原始观察：回答正在流的时候 `panelBody.scrollHeight` 采到 2,131,081 → 4,832,520 → 8,261,642px（每 400ms 采样涨约 62,000px，文字都写完了还在以每 800ms 约 123,000px 继续涨），约 30 秒后自行收回到 8714；推测机制是百分比高度（`size-full`）对着一个由内容撑出来的父亲解析，叠上 `contain-content` 与消息项上的 `[content-visibility:auto]` / `[contain-intrinsic-size:auto 10rem]`，在内容不断追加时形成跨帧的排版反馈环。修后这条链每一层都有界，没有任何一层的高度再由内容决定，按构造不该再有这个环——但本机造不出真的流式，**未实测，列为 staging 复测项**。**「Enter 不发送」**——同场记到：输入框聚焦、已打 180 字，一次合成 Enter 既没把消息发出去、也没插入换行（点 Send 才发），而输入框下面的提示写着「Enter to send」。走查自己标了 low confidence：合成键盘事件不一定走得到应用真正的 keydown 处理路径。**本票不修、不下结论**，登记在此，等真人按一次键复测；真成立的话它是一条独立发现，不属于本条。

证据：本机前后两跑的完整测量与那 11 条失败原文落在 `local-logs/staging-r3-otto-panel/local-before-after.json`（`measurements.broken` / `measurements.fixed` 各五个测点，`mutationRunSoftFailures` 是同一次跑打印出来的原文）——上面每一个数字都从这一份读得回来，不是三处各抄一遍。staging 现场：`local-logs/staging-r3-otto-panel/repro.json`（`dom_under_test`、`root_cause`、`control_experiment`、`measurements` 四节；`auto_scroll_to_bottom` 记 345 次采样，`additional_finding_streaming_layout_runaway` 记暴涨那一条，`side_observation_enter_to_send` 记 Enter 那一条）。走查全程只读，除一次 Otto 对话（2 credits）外未改任何生产状态，页面按原样留回。
...
**缺口**：浏览器那一边从 #1317 起就有一道 `beforeSend` 脱敏（`apps/web/lib/sentry-browser.ts` 的 `scrubShareTokens`），服务端 `apps/web/instrumentation.ts:41-45` 的 `Sentry.init` 一直**没有**——只有 `dsn`／`tracesSampleRate`／`environment` 三个字段。于是一条服务端错误只要请求地址是 `/s/<token>`、`/api/media/pub/<token>`、`?t=<token>`，或者请求头／cookie 里带着那颗装 token 的 `__Secure-sp_t`（`apps/web/lib/share-preview-cookie.ts`；客户每打开一次预览页都会把它发上来，这是服务端**必然**拿得到完整 token 的地方），分享 token 就原样送去第三方。**对商家意味着什么**：拿到那串 token 就等于拿到商家发给客户的那条链接本身——能看那一页、能拉那些图，直到链接过期或被撤销。而浏览器那一道当时也只咬地址字段（`request.url` 与导航面包屑的 `from`/`to`），错误正文与 fetch 面包屑没洗；两个 init 又都没接事务事件那只钩子 `beforeSendTransaction`（`beforeSend` 在 SDK 里只作用于错误事件）——今天 `tracesSampleRate: 0` 没有事务事件被采样，所以那一处不是正在漏，但那是个随时会被改掉的设置值，不是一道门。本条由 R3-F31／F32 的跨厂复审（2026-09-17）在「残余」里点名，登记在 `docs/specs/share-preview.md` §5 2026-09-17 那一行，属于**早于那张票**的缺口，不是它修出来的。

**修法（修根不修表、关类不补例）**：要关的是「分享／媒体 token 进遥测」这**一类**，不是「服务端那一例」。洗法与形状搬进一个运行时中立的新模块 `apps/web/lib/sentry-scrub.ts`（不碰 `window`、不 import node 内建），**浏览器与服务器两个 `Sentry.init` 的 `beforeSend` 从此是同一只函数** `scrubSentryEventTokens`——服务端不抄第二份正则，浏览器也不再只洗地址字段（`apps/web/lib/sentry-browser.ts` 只剩转出去那几行）。这只函数比原来浏览器那一侧多洗的几块：`request.query_string`（node 把 query 另拆一份，`request.url` 洗干净了它还留着原文）、`request.cookies` 与 `Cookie` 请求头（按 cookie 逐对洗，其余 cookie 原样留着）、凭据回声请求头（一份写死的短名单：`authorization`、`proxy-authorization`、`x-api-key`、`x-auth-token`、`x-amz-security-token`，整条值不要）、`exception.values[].value` 与 `event.message`（一条 `fetch failed: …/api/media/pub/<token>` 的报错不经过任何 URL 字段）、面包屑的 `data.url` 与 `message`。两个 init 的 `beforeSend` **与** `beforeSendTransaction` 都接这一只，所以事务事件的 `transaction`／`contexts.trace`／`spans[]` 同样洗（trace/span 只洗一层字符串外加各自的 `data` 一层，不递归——更深的结构今天不存在）。不接 `beforeBreadcrumb`：面包屑在 `beforeSend` 之前就已挂进事件，这一道已经逐条洗过。由 `apps/web/lib/__tests__/sentry-server-scrub.test.ts` 钉住两半——四种形态逐字段脱敏，以及 `instrumentation.ts` 真的把这只函数交给了 `Sentry.init`（洗法再对，没接上等于没有）。浏览器那一侧原本漏着同一类的另外两处——错误正文 `exception.values[].value`（一条 `TypeError: Failed to fetch https://…/api/media/pub/<token>` 不经过 `request.url` 那道门）与 fetch 面包屑的 `data.url`（不是导航面包屑的 `from`/`to`）——本 PR 一并关上，由 `apps/web/lib/__tests__/sentry-browser.test.ts` 新增的三条钉住（第三条钉 `beforeSendTransaction` 的接线）；该文件既有断言一字未改、仍全绿（Founder 2026-09-19 追加裁决：「浏览器端行为不变」放宽为「浏览器端只会洗得更多」）。
...
证据：`docs/specs/share-preview.md` §5 2026-09-17 R3-F31／F32 行的「残余」段（缺口出处）；代码现场 `apps/web/instrumentation.ts:41-45`（修前的三字段 `Sentry.init`）、`apps/web/lib/sentry-browser.ts` 的 `scrubShareTokens`（浏览器侧既有的那道门，修后搬进 `apps/web/lib/sentry-scrub.ts`）。
