...
## R3-F35 · 服务器端 Sentry 不洗分享 token（SHARE-A6 遥测那一句只关了一半）
...
**缺口**：浏览器那一边从 #1317 起就有一道 `beforeSend` 脱敏（`apps/web/lib/sentry-browser.ts` 的 `scrubShareTokens`），服务端 `apps/web/instrumentation.ts:41-45` 的 `Sentry.init` 一直**没有**——只有 `dsn`／`tracesSampleRate`／`environment` 三个字段。于是一条服务端错误只要请求地址是 `/s/<token>`、`/api/media/pub/<token>`、`?t=<token>`，或者请求头／cookie 里带着那颗装 token 的 `__Secure-sp_t`（`apps/web/lib/share-preview-cookie.ts`；客户每打开一次预览页都会把它发上来，这是服务端**必然**拿得到完整 token 的地方），分享 token 就原样送去第三方。**对商家意味着什么**：拿到那串 token 就等于拿到商家发给客户的那条链接本身——能看那一页、能拉那些图，直到链接过期或被撤销。验收 SHARE-A6 的后半句「遥测里的任何 URL 都不含 token」因此只在浏览器上、且只在**地址字段**上成立（浏览器那一道当时也只咬 `request.url` 与导航面包屑的 `from`/`to`）。本条由 R3-F31／F32 的跨厂复审（2026-09-17）在「残余」里点名，登记在 `docs/specs/share-preview.md` §5 2026-09-17 那一行，属于**早于那张票**的缺口，不是它修出来的。

**修法（修根不修表、关类不补例）**：要关的是「分享／媒体 token 进遥测」这**一类**，不是「服务端那一例」。洗法与形状搬进一个运行时中立的新模块 `apps/web/lib/sentry-scrub.ts`（不碰 `window`、不 import node 内建），**浏览器与服务器两个 `Sentry.init` 的 `beforeSend` 从此是同一只函数** `scrubSentryEventTokens`——服务端不抄第二份正则，浏览器也不再只洗地址字段（`apps/web/lib/sentry-browser.ts` 只剩转出去那几行）。这只函数比原来浏览器那一侧多洗的几块：`request.query_string`（node 把 query 另拆一份，`request.url` 洗干净了它还留着原文）、`request.cookies` 与 `Cookie` 请求头（按 cookie 逐对洗，其余 cookie 原样留着）、`Authorization` 一类的凭据回声（整条不要）、`exception.values[].value` 与 `event.message`（一条 `fetch failed: …/api/media/pub/<token>` 的报错不经过任何 URL 字段）、面包屑的 `data.url` 与 `message`。不接 `beforeBreadcrumb`：面包屑在 `beforeSend` 之前就已挂进事件，这一道已经逐条洗过。由 `apps/web/lib/__tests__/sentry-server-scrub.test.ts` 钉住两半——四种形态逐字段脱敏，以及 `instrumentation.ts` 真的把这只函数交给了 `Sentry.init`（洗法再对，没接上等于没有）。浏览器那一侧原本漏着同一类的另外两处——错误正文 `exception.values[].value`（一条 `TypeError: Failed to fetch https://…/api/media/pub/<token>` 不经过 `request.url` 那道门）与 fetch 面包屑的 `data.url`（不是导航面包屑的 `from`/`to`）——本 PR 一并关上，由 `apps/web/lib/__tests__/sentry-browser.test.ts` 新增的两条钉住；该文件既有断言一字未改、仍全绿（Founder 2026-09-19 追加裁决：「浏览器端行为不变」放宽为「浏览器端只会洗得更多」）。
