...
**Goal:** Give Artlio's video editor **burned-in captions** (auto-transcribed, then human-editable) and **static text overlays** — entirely on the existing $0 self-hosted render path. Captions are produced by a NEW, SEPARATE pg-boss `caption` job: extract clip audio with ffmpeg → self-hosted **whisper.cpp** (binary + model baked into the worker image alongside ffmpeg) → word/segment timestamps → cache the transcript keyed by content hash. The transcript seeds an editable caption track in the contract (`timeline.captions[]`); the RENDER job consumes the (possibly-edited) captions → builds an ASS subtitle file → ffmpeg `subtitles=` burn-in on the final composited video stream. Static text overlays are additive `timeline.textOverlays[]` → worker `drawtext`. Everything is additive to `ArtlioEdit`, backward-compatible, bounded, and **$0** (whisper + ffmpeg, never fal).
...
- `artlioEdit = z.object({ timeline, output })` (L273-277): **two top-level keys, plain zod object → unknown top-level keys are STRIPPED**. Doc L6-11: parsing is CANONICALIZING; callers persist the PARSED value; the worker re-parses (`artlioEdit.parse` in render.ts:193). Adding an optional key to `timeline` is additive; old edits keep parsing.
...
- Imports from `@artlio/core` (L22-34): `artlioEdit`, `editDuration`, `renderDuration`, `newId`, `srcToStorageKey`, `RENDER_RETRY_LIMIT`, types. EP3 adds `renderedTimelineSeconds` here (or co-locate in render.ts as a private helper).
...
3. **Worker registry** — `apps/worker/src/queues.ts:4-15` (`QUEUES` object; import from `@artlio/core` L1).
...
- `snapshot()` (L424-435): `currentMergedEdit()` → `artlioEdit.safeParse` → first-issue error. Unchanged in shape; it now carries captions/overlays because the merge does.
...
- [ ] Create `packages/core/src/captions.test.ts` (new file; do NOT bloat `timeline.test.ts`). Pattern after `timeline.test.ts`: `import { describe, expect, it } from "vitest"`; `import { artlioEdit } from "./timeline.js"`; `const HASH = "a".repeat(64)`; `const SRC = ` + "`/files/u/founder/${HASH}.mp4`" + `;`. Build a base valid edit with one visual track `[{ asset:{type:"video",src:SRC}, start:0, length:6 }]` + `output:{format:"mp4"}`.
- [ ] Write these failing assertions (the schema does not exist yet):
  ...
  - **captions happy path:** `timeline.captions = [{ startMs:0, lengthMs:1500, text:"hello" }, { startMs:1500, lengthMs:1500, text:"world" }]` parses; `artlioEdit.parse(x).timeline.captions` round-trips.
...
- [ ] Run `pnpm --filter @artlio/core test -- captions` → expect FAIL (schema missing). Verify the failures are "unrecognized/has no effect" / parse-succeeds-when-it-should-fail, NOT import errors.
...
- [ ] Run `pnpm --filter @artlio/core test -- captions` → expect PASS. Run `pnpm --filter @artlio/core test` (full suite) → expect ALL green (no regression in `timeline.test.ts`/`timeline-ops.test.ts`). Run `pnpm --filter @artlio/core typecheck` → expect 0 errors.
...
- [ ] Run `pnpm --filter @artlio/core build && pnpm --filter @artlio/core typecheck` → expect green.
...
- [ ] Apply LOCAL ONLY (never prod in this plan): `DATABASE_URL=postgresql://artlio:artlio@localhost:5432/artlio pnpm --filter @artlio/db exec prisma migrate deploy` then `pnpm --filter @artlio/db exec prisma generate`.
- [ ] Run `pnpm --filter @artlio/db build` → expect green. Verify the client has `prisma.captionJob` + `prisma.transcript`.
...
- [ ] Local verify (Docker available): `docker build -f apps/worker/Dockerfile -t artlio-worker-ep3 .` (or build only the runtime target). Then `docker run --rm artlio-worker-ep3 sh -c "whisper-cli --help | head -5 && ls -la /opt/whisper/models/"` → expect the CLI help + `ggml-base.en.bin` present. If Docker is not available locally, note that explicitly and defer this verify to the codex/CI gate (the build block is grounded; do not skip the assertion silently).

#### Task 6 — Caption handler (copy the render.ts $0 skeleton; bounded whisper)
- [ ] Create `apps/worker/src/jobs/caption.ts`. Imports: `mkdir, readFile, rm` from `node:fs/promises`, `tmpdir` from `node:os`, `path`, `execa`, `prisma` from `@artlio/db`, `storage` from `../storage.js`, `probeFile` from `./ingest.js`, and from `@artlio/core`: `captionCue`, `CAPTION_RETRY_LIMIT`, `srcToStorageKey`, `newId`, `type CaptionJobData`, `type CaptionCue`.
...
- [ ] `export async function handleCaption(data: CaptionJobData, retryCount = 0): Promise<void>` — copy the render.ts control flow EXACTLY (L171-188 claim, L362-379 catch/finally):
  ...
  - work dir `path.join(tmpdir(), `artlio-caption-${job.id}`)`; `try { await mkdir(work,{recursive:true});`
...
- [ ] **NO spend tokens:** the file MUST NOT import `startGen`/`createGenJob`/fal/`@artlio/generation` provider, and MUST NOT have `spent`/`committed`/`spentUsd`. Confirm by grep at the end of the task.
...
- [ ] Run `pnpm --filter @artlio/worker build && pnpm --filter @artlio/worker typecheck` → expect green.
...
- [ ] Read `node_modules/next/dist/docs/` ONLY if touching routing — these are server actions in an existing file, so no routing change. Run `pnpm --filter @artlio/web typecheck` (or the repo's web typecheck) → expect green.
...
- [ ] Extend `currentMergedEdit()` (L404-422): after merging `transitions` onto track 0, also fold the React captions/overlays into the merged TIMELINE (one level up, NOT on a track):
  ...
  ```
  This keeps `snapshot()` (L424-435) and the debounced live-validate (L286-292) carrying captions/overlays through `artlioEdit.safeParse` unchanged. (Absence stays absent — no empty arrays written, matching the "None = absence" rule.)
...
> **COMMIT (user runs):** `git add apps/web && git commit -m "feat(opt4): EP3 UI — Captions panel (generate + edit) + static text overlays, merged into ArtlioEdit"` — review the Editor.tsx diff (confirm captions/overlays live in React state, never on a Shotstack clip) before committing.
...
- [ ] `pnpm --filter @artlio/core test` (full) → all green; `pnpm -r typecheck` (or each package's typecheck) → 0 errors; `pnpm -r build` → green.
- [ ] **No-spend grep (rule #1):** `grep -RnE "startGen|createGenJob|coworkGenerate|@artlio/generation|spentUsd|GENERATION_PROVIDER" apps/worker/src/jobs/caption.ts apps/web/lib/actions.ts` (the new symbols only) → expect NO hits in `caption.ts`/`startCaption`/`getTranscript`. The caption job + caption action introduce NO spend path.
