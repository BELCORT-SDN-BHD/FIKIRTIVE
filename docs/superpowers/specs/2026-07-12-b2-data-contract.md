...
> **R-010 schema authority 硬停**：本冻结契约要求 identity issuer/version、`ConsentEvent` 四轴与 Campaign 结构化 `utmJson`；#314 已合 schema 实际为较窄 identity、Contact consent 字段与 `utmBase`。D-038 没有选择哪边胜出，现状也不能自行成为方向。独立 Founder-approved schema alignment 前，三处相关施工不得启动；本稿与本对齐 PR 都不改 schema。
