# B2 数据契约 spec（事件 / 身份 / 同意）（v1.2——冻结候选〔拆分稿：观测流门机制段=设计草案，见 §五冻结范围〕）
...
> **——设计草案段到此为止（R-009 裁定 B）；以下恢复冻结候选文本。**
