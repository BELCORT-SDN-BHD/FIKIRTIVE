...
| **ContactDndEvent** | append-only DND set/clear 事实；字段 `id, ownerId, contactId, action(set\|clear), actorKind(merchant\|otto\|legacy_migration), actorId?, sourceKind, evidenceRef?, idempotencyKey, receivedAt, createdAt`（`:335`）；Contact-wide compatibility projection `Contact.doNotDisturb`（`:359`） | **已裁决（#356 D-Q1，2026-07-19，与 ConsentEvent 同题）**：平台不设强制保留期限，不做自动删除/清理/压缩；处置由商家自主决定 | **已裁决（#356 D-Q2/D-Q3，2026-07-19，与 ConsentEvent 同题）**：删除机制 deferred（tag-only Phase 1，触发条件=商家处置工具立项时呈批，台账 #359-2）；导出随 CRM 排期，非门阻塞（台账 #359-3）。DND 是 Contact 属性之一，受同一 Contact 处置原则约束（`:365`） | 已冻结：append-only、无 ordinary UPDATE/DELETE（`:335`）；writer 组合仅限 closed matrix `crm_ui × merchant × set|clear`、`otto_approved_action × otto × set|clear`、`legacy_contact_snapshot × legacy_migration × set`（仅 `set`，无 `clear`），未列组合拒写（`:335`）；Otto 只能经「既有可见审批/动作层」调用同一 shared action（`:335`）。**已裁决（#356 D-Q4，2026-07-19，与 ConsentEvent 同题）**：工作区成员均可查看/清除；平台侧沿用冒充模式（禁写+审计）；细分 RBAC deferred 至「团队协作+市政厅 v2」街区（台账 #359-6） | 结构性排除：字段清单不含 raw 内容字段，仅 `evidenceRef?` opaque 引用（`:335`）；未见 R-010 对 `ContactDndEvent.evidenceRef` 逐字重申「不得默认保存 raw」，按 `:363` 的通用 evidenceRef 原则类推适用 | 已冻结：tenant-qualified Contact relation（`:335`）；共用 tenant 不变量 §6 | **已裁决（#356 D-Q5，2026-07-19，同 ConsentEvent）**：受托处理方姿态；正式法律依据分类 deferred 至 B13 法务面（台账 #359-17） | **已裁决（#356 D-Q7，2026-07-19，同 ConsentEvent）**：第一期平台托管加密（存储加密+TLS）；企业级字段级/自管密钥 deferred（台账 #359-18） |
...
- 标准 2（§2 矩阵原【待 Founder 裁决】格子均已回填为具体、可执行的规则）：满足——见本次更新后的 §2 矩阵，
  ...
  五行全部维度（保留期/删除与导出/访问控制与审计/raw payload 排除/跨租户隔离/法律依据/加密-key scope）已
  冻结或已裁决或已登记延后触发（每处延后触发条件均已写入 §2 对应格子并链接台账 #359）。
