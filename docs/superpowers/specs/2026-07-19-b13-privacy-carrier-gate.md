...
> 对外隐私政策/ToS 文本，也不代替任何下游施工或授权步骤。2026-07-22 依 #405（C6-M0 §9.2 隐私闸、Founder
> 批件）增补 C6 两 carrier 行，详见 §2 表尾 scope 说明；原 #372 范围与 scoped-PASS 判定不变。2026-07-22 另依
> #416（C7-M0 §10.2 隐私闸、Founder 批件 issuecomment-5044757427）增补 C7 七 carrier 行，详见 §2 表尾 scope
> 说明；原 #372/#405 范围与 scoped-PASS 判定不变。
>
...
| **MessageDeliveryEvent**（C6-M0 契约 PR #402 `9dcf8078` §7.2；[#405](https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/issues/405)） | append-only 入站 provider 送达事实（`accepted`/`delivered`/`read`/`failed`/`replied`）；结构字段 `ownerId`、`channelScopeId`+`channel`、`providerConnectionId`、`factKind`、`actorKind` + opaque ref/hash（`logicalSendRef`/`providerCode`/`receiptRef`/`externalMessageRef`/`sourceEventKey`/`sourcePayloadHash`）+ 时间戳（C6 §7.2）；可关联某联系人的某次发送，PII-adjacent（C6 §9.1） | **已裁决（Founder Resolution 2026-07-22，[#359](https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/issues/359#issuecomment-5037075282)）**：回执数据商家自有；平台默认保留 24 个月，到期清理前提醒，商家可调长/调短/关闭；随时手动删/导出 | 走后续统一 privacy capability；Contact/Identity/BroadcastRun archive 不级联抹回执（`onDelete: Restrict`，C6 §9.2）；删除须与 C6 §10「回执是外部真相，不得删后重投」协调 | 仅 owner-scoped 只读对账/报告面，无独立 mutation API（C6 §7.1）；tenant 内 RBAC/capability matrix 仍 Unknown，default deny（C6 §14 :581,595） | 绝不存 raw phone/消息内容/token/签名（C6 §7.2）；日志/错误/telemetry 一律 redaction，不落 raw 值、仅 opaque ref（C6 §9.2 :428-429） | `ownerId` 只从 authenticated session、verified server route 或验签后的 channel account claim 得到；每个 owner-scoped relation tenant-qualified；全查询强制（C6 §3.2 :122-124） | 商家指令处理（平台为保管人）+ PDPA 数据最小化为工作口径；#356 D-Q1/D-Q3 原则援引；正式法律分类与对外文本留 B13 法务面逐字呈批（本文 :306-309） | DB at-rest + tenant boundary，不做 ad-hoc field crypto（C6 §9.2） |
| **MessageDeliveryState**（C6-M0 契约 PR #402 `9dcf8078` §7.3；[#405](https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/issues/405)） | rebuildable per-`logicalSendRef` 对账真相投影（`lifecycle`/`reconciliation`，C6 §7.3），PII-adjacent（C6 §9.1） | **已裁决（Founder Resolution 2026-07-22，#359，同 `MessageDeliveryEvent` 行）**：与 `MessageDeliveryEvent` 同策 | 同上；投影可由事件重建 | 仅 owner-scoped 读面，无独立 mutation API；tenant 内 RBAC/capability matrix 仍 Unknown，default deny（C6 §14 :581,595） | 只存规范化状态与 ref；日志/错误/telemetry 一律 redaction，不落 raw 值、仅 opaque ref（C6 §9.2 :428-429） | `ownerId` 只从 authenticated session、verified server route 或验签后的 channel account claim 得到；每个 owner-scoped relation tenant-qualified；全查询强制（C6 §3.2 :122-124） | 同上 | 同上 |
| **WorkflowDefinition** | **配置类、minimal PII**：商家拥有的 workflow stable identity/origin/current-revision pointer；`name` 是 bounded business text，`createdByMembershipId` 是成员引用，因此不是「零个人数据」，但不存顾客档案或执行正文（C7 §7.2、§10.1） | **提案：account-lifetime，merchant-deletable。** draft/published/archived 随账户保留，便于商家继续编辑与解释历史；archive 不是删除。商家要求处置时可删，但必须先确认 `currentRevision`、Revision、Routine 等引用已安全处理；本 carrier 没有理由采用 24 个月自动清理 | 统一 privacy capability 提供 normalized export 与受控删除；Definition→Revision/current pointer、Routine 与 Membership 历史关系均 `onDelete: Restrict`，绝不 cascade。删除按引用顺序执行，不能把 archive 当 erasure，也不能连带抹 Run/Journey/Step | 同租户 owner-scoped read；create/update/publish/archive 只经 authenticated C7 shared action + CAS。平台 support 与 impersonation 未批前 default deny；候选仅只读且审计。Definition mutation 不是 Routine 授权，不能借此启用执行 | 只存 stable IDs、normalized slug、bounded name、recipe opaque key/version 与 revision pointer；禁止 raw phone/name/message/provider payload/token/signature。`name` 不是 execution prompt | `ownerId` 只来自 authenticated session/server action；owner→Organization、creator→Membership、current revision→本 owner/本 Definition exact Revision 全 tenant-qualified；每次查询强制 owner filter | **工作口径（援引 D-Q1/D-Q3，不作正式法律裁决）**：数据归商家，平台按商家指令托管；处理目的为 workflow authoring/lifecycle，商家享标准导出/处置权。正式 legal basis、PDPA 分类、隐私政策/ToS 文本留 B13 法务面逐字呈批 | 平台托管 DB at-rest encryption + TLS + tenant boundary；不以 hash 代替加密，不做 ad-hoc field crypto。企业字段级/自管 key scope 另 gate |
| **WorkflowRevision** | **配置类、minimal PII**：不可变 readable YAML source、compiled artifact、dependency refs/hashes、validation facts；作者 Membership 与 template/policy/segment/channel refs 使其 PII-adjacent 风险非零，但合同禁止 source/manifest 放 raw PII/secret（C7 §7.3、§10.1） | **提案：account-lifetime，merchant-deletable when unreferenced。** current、曾获 Routine 授权或被 Run/Journey/Step pin 的 revision 跟账户保留，以维持重放与授权证明；完全 unreferenced 后才进入商家处置。独立 24 个月 TTL 会破坏历史 pin，故不建议 | 统一 capability 导出 readable source、compiled/hash/validation facts；普通 UPDATE 禁止，物理删前必须证明 Definition pointer、Routine/Run/Journey/Step 与 dependency use 均已解除。所有 relational refs `Restrict`，无 cascade；JSON dependency ref 也须先 server-side 重解，不能因「不是 FK」漏检 | 同租户 owner-scoped read；只允许 compiler/shared action insert 新 revision，旧 row 无普通 mutation API。作者 relation tenant-qualified；support default deny/只读审计候选 | `rulesSource` 不得含 raw phone/name/message/provider secret；`dependencyManifestJson` 只存 exact `{kind,resourceId,resourceRevision,contentHash}`；validation 只存 stable code + safe source location；不存 raw content 或 `latest/current` alias | Definition/author relation tenant-qualified；JSON dependency refs 在 compile、authorize、dependent-step execution 都以 `(id,ownerId)` 重解并比 hash；跨 owner、missing、hash drift 一律 unavailable | 同 D-Q1/D-Q3 工作口径：商家拥有规则与版本证明，平台按指令保存确定性 runtime artifact；正式 legal basis/对外文本留 B13 法务面 | DB at-rest + TLS + tenant boundary；opaque ref/hash 只做引用与完整性，不是密钥；企业 key scope 另 gate |
| **Routine** | **配置类、minimal PII 的 standing-authorization envelope**：商家拥有 exact revision/scope/budget/summary/authorizer/kill facts；Membership 与 contact/segment/channel scope refs 可能间接关联个人，但它不是 consent 或 send eligibility（C7 §5、§7.4、§10.1） | **提案：account-lifetime，merchant-deletable when history permits。** draft/active/paused/revoked/expired/superseded envelope 都随账户保留，确保商家能解释谁授权了什么；只有其 Run/Journey/Step 与 supersession 链不再需要时才可受控处置。24 个月不适合作独立默认，因为授权链可能比单次执行历史更长 | 统一 capability 导出 canonical scope/hash/budget/summary/authorization/kill facts。kill/revoke/expire 只改状态，不删除历史；Definition/Revision/self-supersedes/Membership/Run/Journey 全 `Restrict`，无 cascade。删除不得使旧授权看似从未存在 | owner-scoped read；draft 可经 shared action 编辑，但 authorize/reauthorize/resume confirmation 与 authorization fields **human-only mutation**，Otto 不得代签。Otto kill 未决时 default deny；support/capability matrix 未批前 default deny，所有 mutation 审计 | `scopeJson` 只存 closed IDs/boundaries/caps；`summaryPolicyJson` 是结构化 policy，kill reason 只 stable code；禁止 raw contact list、phone/name/message/provider payload/token/signature。authorization hash 不是正文 | exact Definition/Revision、自我 supersedes 与 creator/authorizer/killer Membership 均 composite tenant bind；scope 中 channel/contact/segment refs server-derived 并按 owner 重解；rule/Otto/client 不得提交 owner 或 authority facts | D-Q1/D-Q3 工作口径：商家拥有授权书，平台按商家指令编排；Routine 不构成 consent/send authority。正式 legal basis 与 authorization 文本留 B13 法务面 | DB at-rest + TLS + tenant boundary；authorization/scope hash 是完整性证明，不替代加密；企业 key scope 另 gate |
| **RoutineRun** | **PII-adjacent execution metadata**：一次 trigger occurrence、frozen authorization snapshot、cursor 与 summary；可由 customer event 触发，但不是 provider job、receipt 或 delivery truth（C7 §7.5、§10.1） | **提案：terminal `finishedAt` 后默认 24 个月，商家可调长/调短/关闭，清理前提醒；nonterminal 不按年龄清理。** 理由：商家需在合理窗口内看自动化历史/争议与重试证据；与 C6 receipt history 同窗便于对账。没有更好独立默认；若 Run 比 Step 更早删除会破坏 exactly-once/recovery | 统一 capability 导出 trigger/authorization/status/summary normalized facts。Run→Routine/Journey 与 Step refs `Restrict`，无 cascade；Contact 处置不能直接连带删。删除/terminal compaction 必须保留足以阻止同 `runIdempotencyKey` 重跑的最小 non-PII proof；tombstone/去标识物理形状另批 | writer 只限 trusted scheduler/worker 经 shared service；商家只读 owner-scoped run history/summary。terminal Run 不得 UPDATE 重跑；support default deny/只读审计候选。新 Run 仍须 live status/kill/expiry/hash gate | `triggerEventRef` 只 opaque verified source ref，trigger payload 只 hash；authorization snapshot 是 bounded canonical facts；summary 禁 raw phone/name/message/provider payload，block/error 只 stable code；不存 receipt | full Routine authorization composite key绑定 exact owner/routine/revision/hash；journey run再 bind exact owner/routine/revision/Journey；worker context 只能使用 trusted server-derived owner，queue payload 不得覆盖 | D-Q1/D-Q3 工作口径：执行历史归商家，平台作为处理方保存 orchestration/exactly-once 证明；外部 delivery truth 仍归 C6。正式 legal basis/DSAR 文字留 B13 法务面 | DB at-rest + TLS + tenant boundary；snapshot/ref/hash 不作为 crypto；企业 key scope 另 gate |
| **ContactJourneyState** | **PII-adjacent execution metadata**：直接引用 Contact，可选 pin exact ContactIdentity，保存 per-contact pinned step/wait/terminal state；不是 Contact shadow、consent state 或 audience list（C7 §6.1、§7.6、§10.1） | **提案：active/waiting/paused 不自动清理；terminal `terminalAt` 后默认 24 个月、商家可调长/调短/关闭、清理前提醒。** 理由：活跃 journey 是当前产品状态，不能用 TTL 删除；终态后 24 个月与 Run/C6 窗口一致。没有明显更好默认，但 Contact delete/merge 可触发提前受控审查 | 统一 capability 做 owner/contact-scoped normalized export。Contact/Identity/Revision/Routine/lastRun 全 `Restrict`，无 cascade；Contact delete/archive/merge 不能静默抹 journey 或猜 identity repin。匿名化、重绑、硬删/最小 tombstone 的选择仍须 Founder 冻结 | writer 只限 trusted engine/shared service + CAS；商家仅 owner-scoped journey read surface。无普通跨面 mutation；人工 pause/resume 语义未批前 default deny；support default deny/只读审计候选 | `stateJson` 仅 closed engine state、opaque refs、stable codes；不得复制 Contact name/phone、consent、message、provider payload/token/signature。只保存 `contactId/contactIdentityId` refs | Contact composite、Identity triple、Revision/Routine 与 lastRun reverse ownership 都 tenant-qualified；enrollment key 由 server 从 owner+definition+contact+verified occurrence 推导；identity missing/cross-owner 即 unavailable | D-Q1/D-Q3 工作口径：journey state 是商家顾客经营数据，平台按商家指令处理；不成为 consent authority。正式 legal basis、Contact merge/delete DSAR 文本留 B13 法务面 | DB at-rest + TLS + tenant boundary；direct refs 受同一 tenant key scope 管理，不加临时字段级 crypto；企业 key scope 另 gate |
| **WorkflowStepExecution** | **PII-adjacent execution metadata**：引用 Run/Journey/Contact/Identity/action，承载 row-before-dispatch exactly-once/recovery proof；不是 outbox、send job、ActionReceipt 或 delivery truth（C7 §6.4、§7.7、§10.1） | **提案：至少与所属 Run 同期；以 `settledAt` 明确 finalize 的 Step 从该时间起默认 24 个月，商家可调长/调短/关闭，清理前提醒；不得早于 parent Run 清理。** 未有 `settledAt` 的 reserved/delegated/blocked/unavailable/failed row 不按年龄清理，直至 closed transition contract 冻结其 final anchor。理由：Step 与 Run 必须一起解释 delegation/retry/idempotency，单独更短会打开重复 action 风险；customer-facing step 也应覆盖 C6 receipt window | 统一 capability 导出 normalized action/hash/verdict/downstream-ref facts。Run/Journey/Contact/Identity/Connection refs 全 `Restrict`，无 cascade；DSAR/compaction 可提案去除 direct contact refs，但必须保留防重放的最小 step/action idempotency proof，具体 tombstone/去标识形状另批 | writer 只限 trusted worker/shared service 的 closed transitions；商家 owner-scoped audit read。reserved row 先于 downstream call，所有 retry 复用同 step/action key 与 downstream ref，不凭状态盲重投；support default deny/只读审计候选。purpose/caller/eligibility 由 server 派生，不能由 rule/client/Otto 写 | message/action 只存 `actionPayloadHash`；`downstreamRef` 只是 C4/C5 opaque action ref，不是 provider ref/receipt；verdict 是分轴 snapshot+hash；reason/error 只 stable code，禁止 raw phone/name/message/exception/provider payload/token/signature | base Run pin + Run/Journey/Revision双 bind；Contact、Identity+channel、Connection+kind 全 composite tenant bind；step/action keys server-derived，所有 query 带 owner；relation mismatch 零 downstream call | D-Q1/D-Q3 工作口径：执行账归商家，平台作为处理方维持 exactly-once 与审计；真实发送/回执法律 truth 仍归 C4/C5/D8/C6。正式 legal basis/DSAR 文本留 B13 法务面 | DB at-rest + TLS + tenant boundary；hash/ref 不是加密；企业 key scope 另 gate |
| **BusinessHoursPolicy** | **配置类、minimal PII**：商家拥有的 IANA timezone + canonical weekly windows revision；timezone/windows 通常不是顾客 PII，但 `name` 与 author Membership 是商家数据，且可能被 workflow dependency 引用（C7 §6.2、§7.8、§10.1） | **提案：account-lifetime，merchant-deletable when unreferenced。** draft/published/archived revision 随账户保留；被 WorkflowRevision manifest/active Routine pin 时不得清理。archive 不删 history、不停 Routine；完全 unreferenced 后才受控处置。没有理由采用 24 个月执行历史默认 | 统一 capability 导出 timezone/windows/revision/hash。self-supersedes/author relation `Restrict`，无 cascade；archive 前先走 active-Routine 逐条确认门。JSON manifest 虽不是 FK，删除时仍必须重解 exact policy id/revision/hash，避免 orphan dependency | 同租户 owner-scoped read；draft/create 与 status/archive 只经 shared action，published content immutable，编辑插新 revision。support/capability 未批前 default deny/只读审计候选；policy archive 不能代替 Routine kill | 只存 canonical IANA zone、closed weekly windows、bounded business name 与 hash；不复制 reply template、Contact、phone/name/message/provider payload/token/signature。template 只在 WorkflowRevision 中作 exact opaque ref | owner、自我 supersedes 与 author tenant-qualified；manifest policy ref 在 compile/authorize/step 各按 `(id,ownerId)` 重解并比 revision/hash；missing/cross-owner/hash drift 一律 unavailable | D-Q1/D-Q3 工作口径：营业时间配置归商家，平台按指令计算 inside/outside/unavailable；不是 scheduler、template truth 或 send authority。正式 legal basis/对外文本留 B13 法务面 | DB at-rest + TLS + tenant boundary；content hash 只保完整性，不替代加密；企业 key scope 另 gate |

**备份/副本到期（R-010 `:364` 的 backup/replica expiry 维度，适用于上表原五行；C6 两行另见下方 scope 说明）**：已裁决（#356 D-Q3，
...
**C6 两行 scope 边界（`MessageDeliveryEvent`/`MessageDeliveryState` 两行）**：C6 两行系依 C6-M0 §9.2 隐私闸与
2026-07-22 Founder 批件（[#405](https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/issues/405#issuecomment-5037395417)）
增补；不属于本文原五-carrier scoped-PASS 判定范围，原判定不因增补而变。

**C7 七行 scope 边界（`WorkflowDefinition`/`WorkflowRevision`/`Routine`/`RoutineRun`/`ContactJourneyState`/
`WorkflowStepExecution`/`BusinessHoursPolicy` 七行）**：C7 七行系依 C7-M0 §10.2 隐私闸与 2026-07-22 Founder
批件（[#416](https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/issues/416#issuecomment-5044757427)）增补；不属原
scoped-PASS 判定范围，原判定（原五-carrier 与 C6 两行）不因本次增补而变。

**C7 七行共同的 backup、support 与 telemetry 边界（逐字转述自
[#416 批件](https://github.com/BELCORT-SDN-BHD/FIKIRTIVE/issues/416#issuecomment-5044757427)所附
draft「七行共同的 backup、support 与 telemetry 边界」节）**：

- **Backup/replica/PITR 提案**：灾备副本仅用于恢复并按平台策略自然轮换，不构成第二用途；production 实际
  backup/PITR cadence、replica expiry 数值仍是 **Unknown**，须在 production gate 核验，不能从本增补推定。
- **Support access**：tenant capability/RBAC 与 support access 尚未冻结，全部 default deny；若 Founder 采纳
  候选，平台 impersonation 仅 owner-visible/只读、禁写、全程审计。
- **Log/error/telemetry redaction**：七表一律不记录 raw phone/name/message/provider payload/token/signature
  或 raw exception；仅允许 opaque ID/ref、hash、stable code 与安全 source location。
- **Formal legal boundary**：本增补只援引 #356 D-Q1（商家数据 ownership/平台受托处理姿态）与 D-Q3（标准导出
  权）作工作口径；正式 legal basis、PDPA 分类、隐私政策/ToS、删除回调与 DSAR 逐字文本全部留 B13 法务面。
...
姿态文件」当前状态为 `absent`（`docs/archive/route-b/matrix/13-B13.md:16`）。
...
| 矩阵行 | 能力 | 现状 |
|---|---|---|
| B0-89 | 法务面三页（隐私政策/ToS/数据删除回调）——Meta App Review 硬前置 | `ui-shell`（现有 `/privacy` 等路由=旧文本，`docs/archive/route-b/matrix/13-B13.md:12`） |
| B0-93 | PDPA 姿态文件 | `absent`（`docs/archive/route-b/matrix/13-B13.md:16`） |
