...
## 五、成色抽审(2026-07-10,GPT-5.6 Terra 怀疑论审计,总审查员终审采纳)

**抽 15 个最要抄的技能:A 级 2 个(13.3%)/ B 级 12 / C 级 1。整库判词:不是专业级方法论语料,是打磨精良的工作流脚手架** —— 进场清单、阶段编排、输出契约一流,但**决定性的判断层大多留白**(加权分没有锚点、结论建立在未验证的代理指标上、"分析/排名"没有可证伪规则),等于把判断外包给执行模型。仅有的两个真判断系统:`churn-risk-detector` 与 `customer-win-back-sequencer`(信号×权重×门槛齐备)。

**对本图 §一 的约束(内容工程施工与质检的硬标准)**:抄它的骨架,**判断层必须我们自己铸** ——
1. 每个评分因子给**点数锚点**(什么样算 8 分、什么样算 3 分),不许"总分 100 排个序";
2. **最低证据规则**(几个独立来源才许下结论)+ 来源质量/新鲜度规则;
3. 结论若建立在代理指标上(广告投放时长≈有效等),必须**标注为假设**并写明验证法;
