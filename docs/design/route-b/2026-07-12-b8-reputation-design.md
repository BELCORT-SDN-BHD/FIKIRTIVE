...
> **不改五本账**：账行提案见 PR 描述，不在本文件写入 `docs/ops/route-b/`。
...
| **B2** 量测 L0 + 分析 | B0-09（口碑/转介绍/忠诚效果衡量：新客数/NPS/复购率归 B2 决策日志）；E5-06（六原语含 VoucherToken/AttributionEvent/SourceTag） | 口碑动作**写** AttributionEvent（喂 B2）；效果**数值**在 B2 读/算，不在 B8 自算；转介绍奖励载体**复用 VoucherToken**（L0 原语） |
...
| **口碑/转介绍/忠诚的效果数值衡量**（新客数/NPS/复购率） | 矩阵 B0-09（归 B2 决策日志）；缺失大陆第 2 名工作 7 | **归 B2**。B8 只**写** AttributionEvent（口碑动作事件），不自算 NPS/复购率。 |
...
1. **抓图规则**：同视口（桌面 1280×800 + 移动 390）、同旅程**终态**（非空态）、对手真账号真数据 vs 我方 staging 真数据。每条关键旅程一对图，命名 `anchor-<锚>-<旅程>-{them,us}.png` 入证据台账。
...
| H-08 | 四对标锚版本为近似值，spec 当日实机复核 + 抓真截图 | 本文 §二 | spec 冻结日实机抓版本号 + 并排截图入证据台账；不假装抓图 | 待复核 |
