...
- **权限/花费闸列是程序级法律**：18 个 mock-变真风险点全部有归属行；**矩阵实数 7 条💰行**（口径注：A′ 舱单按文件标 6 个💰，矩阵按能力行计 7——含舱单外的 refgen 与两个 costing-先行 SKU），💰行闸列一律非 TBD（机器强制）；任何💰行变真必过 `money-safety-review`。
...
**能力行 216（B0 签署 204 + 修宪包#1 新增 12——D-019）** = 存量 111（沿用 MATRIX-V0 ID）+ 新增 105（`B0-01`~`B0-105`，94-105 为修宪包#1）。**留痕 27（+CC-O12）** = 不在本程/改判/横切 22 + 商业证据行 5。
...
**18 个 mock-变真风险点**：18/18 有归属行；**7 条💰行**（矩阵行口径）闸列全部非 TBD——此断言已写进校验脚本（💰行闸非 TBD = 机器强制），另有冻结 ID 锁（删行必红）、TBD 格式与块号一致性、撕裂单元格检测（校验可复跑：`node scripts/route-b-matrix-check.mjs`）。迁移保真：111 条存量行与 V0 逐格比对 PASS（`coverage-audit/migration-fidelity.json`；仅两类声明变换）。
