...
| B9 | 引擎横切（接口冻结先行） | B0 | **在途**：接口冻结候选 v0.9（#253 @ `37590da9`），六契约 R9 全 CLOSED；合并待 R-009 founder 裁 | — |
| B2 | 量测 L0 + 分析（数据契约先行） | B0 | **在途**：数据契约冻结候选 v1.1（#253 同上），契约〇/1/2/3/UTM R9 全 CLOSED；口碑观测流门机制余 2 反例=R-009 待裁 | — |
