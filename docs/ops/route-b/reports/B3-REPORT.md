...
- **W-B3-E-P · 锚 X1 钱路证明层证据（本批交付，账本级全真测——如实标注：MockProvider 语境 $0 / 零 provider 调用 / 非浏览器七态截图，canvas 壳接线=批3 W-B3-E-W）**：证明对象=既有唯一 gen spend 权威链 `pricedGenCredits`（报价）→ `startGen`（预留）→ worker 六态结算（settle/refund），**权威文件零改动**（gen-actions.ts / core gen.ts / credits.ts / worker gen.ts 四文件 hash 锁定，diff=测试+报告 only）。四层证据：
  - **直证层（新 `apps/web/lib/__tests__/gen-ledger.test.ts`，真 *_test Postgres，9 用例）**：EP-A2 报价=预留=结账三数一致 image（count=4，逐行断言 RESERVE.reservedDelta==quote==4cr×10、SETTLE.reservedDelta==-quote、净扣==quote）与 video（seedance-2-fast 720p/10s=14cr，且断言 job 落库 videoOptions 与报价输入同源）各一例；EP-A5 同幂等键顺序重放与**真并发**重放（partial-unique 索引 backstop：恰一 job/恰一 RESERVE/输家事务整体回滚）；EP-A5 Retry 新键=新 job 恰结算一次+死 job 迟到 settle/refund 双双 no-op（账行零增）；EP-A3 count 1/2/4 image + video 四 job 混合 settle×2/refund×2——**reserved(150)==settled(90)+refunded(60) 分账恒等式**、只退失败格、失败的 count=4 job 全额释放 4 格 hold；六态②余额不足 fail-closed（零 job/零账行/余额分毫不动）；EP-A4 route④ 取消：QUEUED 取消恰退一次+二次取消诚实 no-op、in-flight 取消诚实「太晚」（hold 存续→结算保留收费，「停按钮」判决 2026-07-14 同向）。worker 结算=与 worker 同一 `settleCredits`/`refundReservation`（金额读 RESERVE 行，worker 永不重算——factory-batch-ledger 同型先例）。
  - **worker 路由层（`apps/worker/src/jobs/gen.test.ts` 扩 9 用例，13 全绿）**：EP-A4 route①provider 拒（charged 错→终态 FAILED+spent:true+恰一次 refund+零 settle+TURN_ERROR 恰一条；预扣费拒+余重试→requeue 保 hold 零退款；最后一试拒→FAILED+refund+spent:false 免费失败可辨）；route②超时（终试超时→FAILED+恰一 refund；claim 丢失+STALE 占有者→fail-closed+refund+「未扣款」诚实消息+provider 永不重调；claim 丢失+RECENT 占有者→零退款零消息不碰活赢家）；route③worker 崩溃恢复（committed 重投递→provider 零重调+settle 恰一次+零 refund+DONE+GEN_RESULT；free-delivery guard：REFUND 已赢→FAILED 不交付不再碰钱；FAILED 无产出→终态零钱路动作）。
...
- **W-B3-E-P 可重跑（本批交付）**：`DATABASE_URL=<*_test> pnpm --filter @fikirtive/web exec vitest run lib/__tests__/gen-ledger.test.ts lib/__tests__/gen-actions.test.ts`（9+12 用例，必须真 *_test Postgres——F35 守卫拒非 _test 库）；`pnpm --filter @fikirtive/worker exec vitest run src/jobs/gen.test.ts`（13 用例）；`DATABASE_URL=<*_test> pnpm --filter @fikirtive/db exec vitest run src/credits.test.ts`（14 用例）。本地四关（ci.yml 全量口径 check/test/web-build/lint）命令+退出码见工单 EVIDENCE（mailbox WO-B3-E-P/r001）。CI job 链接=PR checks（见 §①）。
...
13. **W-B3-E-P 覆盖盘点表（EP-A1：五组不变量 × 现有用例矩阵，逐格=已有证据引用 or 本单新增用例）**——工单第一步硬性纪律「只补真实缺口，禁止重复造既有用例」的落判依据；同表进 runtime mailbox REPORT：
    ...
    | ②同幂等键重放不重复扣款（含并发） | mock 层：`gen-actions.test.ts:129/:147/:168/:245/:283`（active fast path/generic P2002/factory P2002 验料/FAILED exact attempt/durable reuse）；批量层真 PG：`factory-batch-ledger.test.ts:146/:174/:424/:455`（batch 重放/锁时并发/DONE 重放/晚到 duplicate）；原语层：`credits.test.ts` case 9（并发 reserve） | **plain 键真 PG（原缺口——并发重放只在 factory 键有真库证据）**：`gen-ledger.test.ts`「sequential double-submit」「CONCURRENT double-submit（partial-unique backstop：恰一 job/恰一 RESERVE）」 |
    ...
    | ④count 1-4 部分失败只退失败格（reserved==settled+refunded） | 批量层真 PG：`factory-batch-ledger.test.ts:281`（N=4 二败只退二）/`:323`（text $0）/`:349`（余额不足逐格 fail-closed） | **直接 startGen 层分账恒等式**：`gen-ledger.test.ts`「EP-A3 count 1/2/4+video 混合——reserved(150)==settled(90)+refunded(60)，失败 count=4 job 全额释放 4 格 hold」。**架构判读（如实）**：既有权威中 image count 1-4=单 GenJob 单次 provider 调用单一 hold——「格」的失败粒度=per-job（全退或全结，无格内半退）；跨 job 集合只退失败 job=本单直证+F-P 批量层既证 |
    ...
    六态显式映射：①报价确认=组① · ②余额不足=`gen-ledger` 六态②例+`gen-actions` InsufficientCredits 例+`factory-batch-ledger:349` · ③provider 拒=route① · ④超时=route②+reaper 既有 · ⑤partial=组④ · ⑥恢复=route③+`gen-ledger` 迟到 finalizer no-op+`credits` case 11-13。
    **结论（EP-A7 同判）**：全部新用例对既有权威**首跑即绿**——未暴露任何既有钱路缺陷，无需 escalation；权威文件零改动（diff=四测试文件+本报告）。**如实边界**：真 PG 直证层的「worker 结算」经与 worker 同一 `settleCredits`/`refundReservation` 函数（金额读 RESERVE 行，worker 永不重算；factory-batch-ledger/W-B3-F-P 同型先例），非驱动完整 `handleGen` 进真库——handleGen 的钱路路由已由 worker 单元层（mock prisma）separately 钉死；两层合并=链路完备，但「真库跑真 handleGen」的端到端形态留待批3 引擎集成期如需再补。断言零连环确认反向要求（GRILL 2026-07-14 审批粒度判决：一个 request=一次批准；取消断言与「停按钮」判决同向）。
