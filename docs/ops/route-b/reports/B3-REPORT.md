...
- **W-B3-H-P 可重跑（v3 终态：证明层查漏 + NODE-282/R2 两轮授权修复）**：对照 spec §5.2 W-B3-H-P 行三项点名 + §6.1 两项通项逐条落判——`pnpm --filter @fikirtive/web exec vitest run lib/__tests__/storyboard-gate1-actions.test.ts`（**81 用例全绿**：v1 基线 68 + v1 补 2 + v2 净增 5 + v3 补 1 + v4 补 5）：
  ...
  - **微修轮 v4（NODE-282-R3①/③）**：五个上锁事务块的 `fresh?.payload ?? cur` 型 fallback **整类清除**（`grep '?? cur'`=0）——锁内重读 fresh 为 null（卡在锁前被删/kind 变更/payload 缺失）时不再回落锁前旧快照派生写集，一律 fail-closed 零写返回 `Card not found.`（prepare×2=`storyboard-gate1-actions.ts:312/:738`+surface `:371/:831`；regen×2=`:420/:888`+surface `:481/:963`；sync=null 哨兵 `:577`+surface `:656`）。五条 fresh-null 用例逐处钉住（test `:360/:539/:844/:1781/:2035`：断言 `{error: Card not found.}`+零暂存零提交+锁已取），**红对照已证**：v3 实现上五条全红（cur 回落路径复活=按过期指针铸卡/写回）。
  - **纪律自检（v4 终态）**：diff 对 main 共三文件=`apps/web/lib/storyboard-gate1-actions.ts`（**有改动**：并发锁 helper+五处取锁+sync 读半段入锁+五处 fresh-null fail-closed（无 cur 回落），均为控制面明示授权的缺陷修复，零新 spend 入口/权威/报价函数）+ `apps/web/lib/__tests__/storyboard-gate1-actions.test.ts` + 本报告；money-safety-review 全清单已在 PR #282 修复轮评论逐条跑（Step-1 严格判 NO+catch-all 零新付费点；(a)-(d) PASS；(e)=NODE-282 对抗二审链本体）；`mockGenJobCreate` 全 81 用例断言 0 次调用（$0 铁证含错误/并发/fresh-null 路径）。本地四关：typecheck 净、`lint:parity` 棘轮 36 不增、web 包 1428 全绿（v2 时全仓 `pnpm -r test` 2753 全绿+web-build ✓，v3/v4 增量=同文件 6 用例）。CI job 链接=PR checks（见 §①）。
