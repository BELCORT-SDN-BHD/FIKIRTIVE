...
- **W-B3-H-P 可重跑（v3 终态：证明层查漏 + NODE-282/R2 两轮授权修复）**：对照 spec §5.2 W-B3-H-P 行三项点名 + §6.1 两项通项逐条落判——`pnpm --filter @fikirtive/web exec vitest run lib/__tests__/storyboard-gate1-actions.test.ts`（**82 用例全绿**：v1 基线 68 + v1 补 2 + v2 净增 5 + v3 补 1 + v4 补 5 + v5 补 1）：
  ...
  - **纪律自检（v5 终态）**：diff 对 main 共三文件=`apps/web/lib/storyboard-gate1-actions.ts`（**有改动**：并发锁 helper+五处取锁+sync 读半段入锁+五处 fresh-null fail-closed（无 cur 回落）+v5 数据流封类（锁前值零流入写路径），均为控制面明示授权的缺陷修复，零新 spend 入口/权威/报价函数）+ `apps/web/lib/__tests__/storyboard-gate1-actions.test.ts` + 本报告；money-safety-review 全清单已在 PR #282 修复轮评论逐条跑（Step-1 严格判 NO+catch-all 零新付费点；(a)-(d) PASS；(e)=NODE-282 对抗二审链本体）；`mockGenJobCreate` 全 82 用例断言 0 次调用（$0 铁证含错误/并发/fresh-null/数据流路径）。本地四关：typecheck 净、`lint:parity` 棘轮 36 不增、web 包 1429 全绿（v2 时全仓 `pnpm -r test` 2753 全绿+web-build ✓，v3/v4/v5 增量=同文件 7 用例）。CI job 链接=PR checks（见 §①）。
