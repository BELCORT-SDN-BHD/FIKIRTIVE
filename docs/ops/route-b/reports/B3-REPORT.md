...
- **W-B3-F-P 可重跑（本批交付，三关本地全绿）**：`pnpm --filter @fikirtive/web exec vitest run lib/__tests__/factory-batch.test.ts lib/__tests__/factory-batch-ledger.test.ts`（8 行为 + 7 账本全真测=15 用例；账本测需 `DATABASE_URL` 指向 *_test 库，配方 `docs/runbooks/local-ci.md`）；`pnpm --filter @fikirtive/otto exec vitest run src/skills/run-factory-batch.test.ts`（7 用例：spend 闸机器推导/身份零模型注入/双模式路由/无 port 优雅拒绝）；全量回归 `pnpm -r test`（7 包全绿：web 142 文件 1430 用例 + otto 63 文件 602 用例含 registry 43-skill 清单）；`pnpm lint:parity`（债闸绿：187 entries，todoSkill 棘轮 36 不增——两新 action 以真 skill `runFactoryBatch` 入册非 todoSkill）；`pnpm --filter @fikirtive/otto run catalog:check`（CATALOG fresh）；`node scripts/route-b-matrix-check.mjs`（矩阵闸绿）；`node scripts/check-margin-floor.mjs`（毛利闸绿，5 SKU 全过 45% 地板）；`pnpm --filter @fikirtive/web build`（web-build 关 exit=0）。CI job 链接=PR checks（见 §①）。
