...
- **W-B3-F-P 可重跑（R4 attempt 原子语义）**：`pnpm --filter web exec vitest run lib/__tests__/batch-idempotency.test.ts lib/__tests__/gen-actions.test.ts lib/__tests__/factory-batch.test.ts lib/__tests__/factory-approval.test.ts`（4 helper + 7 startGen disposition/material/durable replay + 17 factory behavior + 13 审批/strict action 连续链 = 41 用例）；加真账本 `factory-batch-ledger.test.ts` 后 web targeted = 5 files / 57 tests（必须 `DATABASE_URL` 指向 *_test）；`pnpm --filter @fikirtive/otto exec vitest run src/skills/run-factory-batch.test.ts`（7 用例，含模型 schema strict 拒 token）；`pnpm --filter web exec tsc --noEmit --pretty false` 与 `pnpm --filter @fikirtive/otto typecheck`。全量 `pnpm -r test`、catalog fresh、parity、route-b matrix、margin、web build 与 current-head CI 结果见本轮 repair report / PR checks。
