...
- **W-B3-H-P 可重跑（v3 终态：证明层查漏 + NODE-282/R2 两轮授权修复）**：对照 spec §5.2 W-B3-H-P 行三项点名 + §6.1 两项通项逐条落判——`pnpm --filter @fikirtive/web exec vitest run lib/__tests__/storyboard-gate1-actions.test.ts`（**76 用例全绿**：v1 基线 68 + v1 补 2 + v2 净增 5 + v3 补 1）：
  ...
  - **修复轮 v3（NODE-282-R2①/⑤）**：①sync 的读半段（子卡/Job/Generation 采样+写集生成）整体移入锁内、以锁内重读的 fresh payload 为准 derive——v2 的锁前旧快照采样可在 regen 换指针 A→B 后仍把 A 的 generation 写上/级联删 B 指针；交错回归用例 `:1074`（sync 等锁期间 regen 换指针→sync 锁后按 B 行事、A 的 generation 不落上、零写入），**红对照已证**：v2 实现上该用例红（update 被提交且 `videoGenerationId:"vid-A"` 落到指向 B 的镜头上）。②本证据行更新为 v2/v3 真相（删除 v1「authority 零改动」「70/70」过时陈述）。
  - **纪律自检（v3 终态）**：diff 对 main 共三文件=`apps/web/lib/storyboard-gate1-actions.ts`（**有改动**：并发锁 helper+五处取锁+sync 读半段入锁，均为控制面明示授权的缺陷修复，零新 spend 入口/权威/报价函数）+ `apps/web/lib/__tests__/storyboard-gate1-actions.test.ts` + 本报告；money-safety-review 全清单已在 PR #282 修复轮评论逐条跑（Step-1 严格判 NO+catch-all 零新付费点；(a)-(d) PASS；(e)=NODE-282 对抗二审链本体）；`mockGenJobCreate` 全 76 用例断言 0 次调用（$0 铁证含错误/并发路径）。本地四关：typecheck 净、`lint:parity` 棘轮 36 不增、web 包 1423 全绿（v2 时全仓 `pnpm -r test` 2753 全绿+web-build ✓，v3 增量=同文件 1 用例）。CI job 链接=PR checks（见 §①）。
