...
- **W-B3-F-P · 锚 F1/X1 批量编排证据（本批交付，账本级全真测——如实标注：MockProvider $0 / 零真实 provider 调用 / 非浏览器七态截图，工厂壳接线=批3 W-B3-F-W）**：批量编排层 = `apps/web/lib/factory-batch.ts`（headless 核心，逐格循环既有 `startGen` 唯一原语）+ `apps/web/lib/factory-actions.ts`（`runVariantBatch`/`runBulkGrid` 两 owner-scoped server action，requireOwner+防扮演+typed zod）+ gated Otto skill `runFactoryBatch`（`packages/otto/src/skills/run-factory-batch.ts`，cost:"spend"→needsApproval 机器推导 literal true，反翻转同 generate；经注入 port `ctx.runFactoryBatch` 走同一 server action 权威）。**X1 契约真值（真 Postgres 账本，`apps/web/lib/__tests__/factory-batch-ledger.test.ts` 12 用例，含修复/终修轮追加）**：报价=预留=结账三数一致（逐格 + 批求和 + 真实 video 模型 seedance-2-fast 三数一致）；同批 batchId 重放不双扣——in-flight 复用 + **全批 DONE 后重放零新 reserve（any-status 预检）** + 含 FAILED 格重放只重派失败格 + **DONE video 格重放复用（全字段比对与真持久化行一致，含 videoOptions 同源映射）**；N=4 批 2 格失败只退 2 失败格（SETTLE×2+REFUND×2，禁批级全退）；text 格 $0（零 reserve 零 GenJob）；余额不足 fail-closed 逐格拒绝；缺省 model 的 video 格=干净 per-cell error 不崩批；取消只退被取消格；F1 结构性 20 格全链 enqueue 不串行阻塞（逐格状态=queued，无一 done）。**编排层零 credits 直接 mutation** 由测试内 grep 断言机器钉死（strip 注释后 reserveCredits/settleCredits/refundReservation/grantCredits/creditLedger/creditAccount 全部零匹配）。批状态=逐格聚合 `batchCellStatuses`（QUEUED/GENERATING/DONE/FAILED 计数 rollup）。总价确认页 UI + 工厂壳接线 = 批3 W-B3-F-W（人工入口半件义务已由 spec v0.4 顺延，Otto skill 义务已随本 -P 交付）。
## ⑦ 测试全家桶可重跑链接
...
- **W-B3-F-P 可重跑（本批交付+修复/收尾/终修轮，三关本地全绿）**：`pnpm --filter @fikirtive/web exec vitest run lib/__tests__/factory-batch.test.ts lib/__tests__/factory-batch-ledger.test.ts`（16 行为 + 12 账本全真测=28 用例；账本测需 `DATABASE_URL` 指向 *_test 库，配方 `docs/runbooks/local-ci.md`）；`pnpm --filter @fikirtive/web exec vitest run lib/__tests__/factory-approval.test.ts`（10 审批链用例：park→铸 hash 绑定卡→批准→恢复真调被 park 的 skill（SDK resume 语义）→真 port→真 runVariantBatch→orchestrateBatch→mocked startGen **一条连续断言链** + 反翻转负例硬拒零消费 + 同 batchId 重 park dedup）；`pnpm --filter @fikirtive/otto exec vitest run src/skills/run-factory-batch.test.ts`（7 用例：spend 闸机器推导/身份零模型注入/双模式路由/无 port 优雅拒绝）；全量回归 `pnpm -r test`（7 包全绿：web 145 文件 1477 用例 + otto 65 文件 618 用例含 registry 45-skill 清单）；`pnpm lint:parity`（债闸绿：187 entries，todoSkill 棘轮 34 不增——两新 action 以真 skill `runFactoryBatch` 入册非 todoSkill）；`pnpm --filter @fikirtive/otto run catalog:check`（CATALOG fresh）；`node scripts/route-b-matrix-check.mjs`（矩阵闸绿）；`node scripts/check-margin-floor.mjs`（毛利闸绿，5 SKU 全过 45% 地板）；`pnpm --filter @fikirtive/web build`（web-build 关 exit=0）。CI job 链接=PR checks（见 §①）。
...
12. **W-B3-F-P 并发重放双扣窗（终修轮①b 如实登记，待裁）**：`batch:<batchId>:cell:<n>` 幂等键只有 ACTIVE-only 部分唯一索引（`GenJob_active_idempotency_key`）拦截并发插入——**顺序**重放已由编排层 any-status 全字段预检收口（DONE 复用/FAILED 重派/换内容 fail-closed），但**并发**重放同一批存在极窄双扣窗：重放 A 重派 FAILED 格且该新 job 在重放 B 的预检→插入间隙内到达 DONE → active 索引不拦 B 的第二次插入 → 该格重复计费。根治 = 为 `batch:%` 键建 **all-status partial unique index**（`cowork:%` 键 20260617 迁移先例）——涉 migration=founder 级，超本 PR 边界，待裁。本轮已交付：诚实化（skill description + factory-batch.ts 头注明示顺序安全/并发不安全 + 禁并发重试指引）+ 防御性收窄（派发前紧邻二次 findFirst 复核 prior 状态，注释如实标注「narrows, cannot close」非根治）。
