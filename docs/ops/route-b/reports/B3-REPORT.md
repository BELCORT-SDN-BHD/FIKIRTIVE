...
- **W-B3-F-P · 锚 F1/X1 批量编排证据（本批交付，账本级全真测——如实标注：MockProvider $0 / 零真实 provider 调用 / 非浏览器七态截图，工厂壳接线=批3 W-B3-F-W）**：批量编排层 = `apps/web/lib/factory-batch.ts`（headless 核心，逐格循环既有 `startGen` 唯一原语）+ `apps/web/lib/factory-actions.ts`（`runVariantBatch`/`runBulkGrid` 两 owner-scoped server action，requireOwner+防扮演+strict typed zod；每次明确确认携 caller-stable `attemptId`）+ gated Otto skill `runFactoryBatch`（`cost:"spend"`→needsApproval 机器推导；模型 schema 不含且 strict 拒绝 `attemptId`；hash 核验并 CAS 消费成功后，服务端以 `APPROVAL_CARD.id` 注入 attempt）。逐格 key = 79 字符 `batch:<32hex logical>:attempt:<32hex attempt>`。`startGen` 先以 `ownerId+projectId+logicalPrefix` any-status history 做只读 durable verdict：完整 material mismatch（FAILED 亦绑定；`entityIds` 顺序敏感且保留重复；image `variantSel` 缺省/`null`/`{}` canonical 为 `null`，非空 key/value exact）即 conflict，exact 或任一 non-FAILED 可直接 reused/0，故 guardian/admin model 等 fresh-only gate 后续漂移不会破同 attempt response；这个 fast path 不 create、不 reserve。只有 miss 才跑所有动态 gate，并在既有 project advisory transaction lock 内重读同一 verdict 后决定 create + reserve；只有同事务真正提交两者才回 fresh，P2002 loser 仍 reused。factory 的只读预检仅早拒，BatchResult 完全服从 `startGen` disposition，故 `totalCredits == 本次真实新 reservation`。**X1 契约真值（真 *_test Postgres，`factory-batch-ledger.test.ts` 16 用例）**：报价=预留=结账三数一致；同 attempt 的 in-flight/DONE/FAILED delayed duplicate 均零新 job/reserve；FAILED A + 明确 attempt B 恰一新 reserve，B 重放零；barrier 强制两路都越过 factory precheck 后，并发首次与并发 FAILED retry 均得到同 jobId、合计 1 queued/full + 1 reused/0、1 RESERVE；并发异内容一方 conflict/0；跨 owner 相同 structural key 各自 fresh；N=4 中 2 失败只退 2 格；text $0；余额不足逐格 fail-closed；video quote/reserve/settle 与 full-material replay；取消单格退款；20 格全 enqueue 不等生成。**编排层零 credits 直接 mutation** 由源码断言钉死；批状态仍由 `batchCellStatuses` 聚合。总价确认页 UI + 工厂壳接线 = 批3 W-B3-F-W。
- **W-B3-E-P · 锚 X1 钱路证明层证据（本批交付，账本级全真测——如实标注：MockProvider 语境 $0 / 零 provider 调用 / 非浏览器七态截图，canvas 壳接线=批3 W-B3-E-W）**：证明对象=既有唯一 gen spend 权威链 `pricedGenCredits`（报价）→ `startGen`（预留）→ worker 六态结算（settle/refund），**权威文件零改动**（gen-actions.ts / core gen.ts / credits.ts / worker gen.ts 四文件 hash 锁定，diff=测试+报告 only）。**修复轮 v2（NODE-307-R1）**：b/d 两条不变量按「先探针坐实事实」重做——探针坐实两处 **spec 不变量 ↔ 既有权威行为冲突**（R-EP-01 终态同键重放双重预留 / R-EP-02 单 job 子格 partial 无子格结算），依工单红线**停手上报，本单以 escalation 结案**（详 §⑫.13 + mailbox `WO-B3-E-P/r001/ESCALATION.md`；权威文件仍零改动）。四层证据：
  - **直证层（新 `apps/web/lib/__tests__/gen-ledger.test.ts`，真 *_test Postgres，11 用例）**：EP-A2 报价=预留=结账三数一致 image（count=4，逐行断言 RESERVE.reservedDelta==quote==4cr×10、SETTLE.reservedDelta==-quote、净扣==quote）与 video（seedance-2-fast 720p/10s=14cr，且断言 job 落库 videoOptions 与报价输入同源）各一例；EP-A5 **在途面**同幂等键顺序重放与**真并发**重放（partial-unique 索引 backstop：恰一 job/恰一 RESERVE/输家事务整体回滚——只覆盖 QUEUED/GENERATING 在途重放）；**PROBE R-EP-01a/01b（终态面探针，锁现状非背书）**：DONE 后同 plain 键重放=新 job+**第二笔预留**（双重预留事实真库坐实）、FAILED 后同键重放=新 job（净额安全但不去重）——与 spec:301 未限定措辞冲突→escalation；EP-A5 Retry 新键=新 job 恰结算一次+死 job 迟到 settle/refund 双双 no-op（账行零增）；**跨独立 job** count 1/2/4 image + video 四 job 混合 settle×2/refund×2——**reserved(150)==settled(90)+refunded(60) 分账恒等式**、只退失败 **job**、失败的 count=4 job 全额释放 4 格 hold（**per-job 恒等式，非单 job 子格 partial 证明**——子格事实见 worker 层 PROBE）；六态②余额不足 fail-closed（零 job/零账行/余额分毫不动）；EP-A4 route④ 取消：QUEUED 取消恰退一次+二次取消诚实 no-op、in-flight 取消诚实「太晚」（hold 存续→结算保留收费，「停按钮」判决 2026-07-14 同向）。worker 结算=与 worker 同一 `settleCredits`/`refundReservation`（金额读 RESERVE 行，worker 永不重算——factory-batch-ledger 同型先例）。
  - **worker 路由层（`apps/worker/src/jobs/gen.test.ts` 扩 11 用例，15 全绿）**：EP-A4 route①provider 拒（charged 错→终态 FAILED+spent:true+恰一次 refund+零 settle+TURN_ERROR 恰一条；预扣费拒+余重试→requeue 保 hold 零退款；最后一试拒→FAILED+refund+spent:false 免费失败可辨）；route②超时（终试超时→FAILED+恰一 refund；claim 丢失+STALE 占有者→fail-closed+refund+「未扣款」诚实消息+provider 永不重调；claim 丢失+RECENT 占有者→零退款零消息不碰活赢家）；route③worker 崩溃恢复（committed 重投递→provider 零重调+settle 恰一次+零 refund+DONE+GEN_RESULT；free-delivery guard：REFUND 已赢→FAILED 不交付不再碰钱；FAILED 无产出→终态零钱路动作）；**PROBE R-EP-02a/02b（单 job 子格 partial 探针，锁现状非背书）**：真实 byteplus partial 形态（2/4 已计费→chargedError，`byteplus.ts:50-60`）=整单 FAILED+整笔 hold **全额退款**+零子格 settle（用户一格不付，founder 吸收已计费子图）；provider 静默短返（3/4）=整笔 count=4 hold **全额结算**+零 partial 退款（worker 无 `outputs.length==count` 守卫，exactly-count 保证只住在 adapter 契约里）——权威无子格账可退，与 spec:301 子格粒度读法（NODE-307-R1）冲突→escalation。
  - **报价权威层（`apps/web/lib/__tests__/gen-actions.test.ts` 扩 4 用例，12 全绿）**：image count=4 报价=预留且钉死价目字面（count×1cr）；video count>1 请求只收单 clip 且落库 count=1（fan-out 不双收首 clip）；720p/10s=14cr 档位钉死（margin-parity）；InsufficientCredits→友好报错+零 enqueue+零审计写。
  - **账本原语层（`packages/db/src/credits.test.ts` 扩 case 11-13，14 全绿）**：幂等双 refund（恰一 REFUND 行，余额只恢复一次）；settle 赢后迟到 refund no-op（成片已交付永不倒赔）；**真并发 settle∥refund**（两边都不抛、恰一 finalizer 落账、账面与赢家一致）。
  - 五组不变量 × 用例覆盖盘点表见 §⑫.13（EP-A1）。
...
- **W-B3-H-P 可重跑（v3 终态：证明层查漏 + NODE-282/R2 两轮授权修复）**：对照 spec §5.2 W-B3-H-P 行三项点名 + §6.1 两项通项逐条落判——`pnpm --filter @fikirtive/web exec vitest run lib/__tests__/storyboard-gate1-actions.test.ts`（**83 用例全绿**：v1 基线 68 + v1 补 2 + v2 净增 5 + v3 补 1 + v4 补 5 + v5 补 1 + v6 补 1）：
  | 项 | 落判 | 引证（行号=v3 head 时点） |
  |---|---|---|
  | 防双扣 | 已有引证 + v2 修复加固 | `storyboard-gate1-actions.test.ts:1403`（matching+幂等 job→复用 spent:true）、`:1510`（P1 kill-shot，二次 prepare 0 计费）；v2 并发面：`:1676`/`:1872`（交错 prepare/regen → 恰好一次铸卡） |
  | reuse-if-matches | 已有引证 | `:1335`（四字段一致→复用）、`:1358`/`:1380`/`:1479`（duration/source/model 不一致→铸新）、`:1430`/`:1453`（吸附后一致→复用不 churn，P2）；v2 补 `:1538`/`:1848`（spent+mismatch→铸新=合法新报价）、`:1566`（悬空指针→铸新不炸） |
  | spent-aware | 已有引证 | `:1403`/`:1510`（spent 排除出 totalCredits）、`:1823`（regen：spent 子卡不复用铸新） |
  | 部分失败只退失败格（§6.1 通项→本层=逐镜头独立结算） | v1 新增用例 | `:1585`「批内三态混合→逐镜头独立结算,totalCredits 只计未花钱铸新」 |
  | fail-closed（§6.1 通项→本层=零吞错+真回滚） | v1 新增 → v2 升级真回滚 | `:1636`「首镜头已铸(暂存)、次镜头读取抛错→整批零提交、指针零变更、抛错不吞」（缓冲事务 mock：暂存写成功才提交、抛错全弃） |
  - **停手上报→授权修复链**：v1 按红线零改权威、上报「零发现缺陷」→ codex NODE-282: BLOCK(1,2) 坐实**既有钱路缺陷**（五个 RMW 事务 READ COMMITTED 下无锁→并发双铸可双扣）→ 控制面授权修复轮 v2；NODE-282-R2: BLOCK(1,5) 坐实 sync 读半段仍在锁前 → 修复轮 v3。
  - **修复轮 v2（authority 有改动）**：卡级 `pg_advisory_xact_lock`（key=`card:<cardId>`，`lockCardTx`=`storyboard-gate1-actions.ts:101`，cowork-actions.ts:180/gen-actions.ts:118 同款家法），五个 RMW 事务（两 prepare/两 regen/sync）第一步取锁；零 schema、每事务单锁无死锁面。测试升级：缓冲事务 mock（+`$executeRaw` 真互斥锁语义）、fail-closed 真回滚、spent+mismatch/悬空指针意图用例、两条并发回归。**回归力已证**：并发用例在无锁实现上直接观察到双铸（committed create 2≠1）。
  - **修复轮 v3（NODE-282-R2①/⑤）**：①sync 的读半段（子卡/Job/Generation 采样+写集生成）整体移入锁内、以锁内重读的 fresh payload 为准 derive（fresh 为 null 即 fail-closed 零写，无 cur 回落——v4 收口）——v2 的锁前旧快照采样可在 regen 换指针 A→B 后仍把 A 的 generation 写上/级联删 B 指针；交错回归用例 `:1074`（sync 等锁期间 regen 换指针→sync 锁后按 B 行事、A 的 generation 不落上、零写入），**红对照已证**：v2 实现上该用例红（update 被提交且 `videoGenerationId:"vid-A"` 落到指向 B 的镜头上）。②本证据行更新为 v2/v3 真相（删除 v1「authority 零改动」「70/70」过时陈述）。
  - **微修轮 v4（NODE-282-R3①/③）**：五个上锁事务块的 `fresh?.payload ?? cur` 型 fallback 清除（`grep '?? cur'`=0；**注**：此轮只清了字面形态，锁前派生值经其它变量流入写路径的语义形态由 v5 数据流完备清扫封类）——锁内重读 fresh 为 null（卡在锁前被删/kind 变更/payload 缺失）时不再回落锁前旧快照派生写集，一律 fail-closed 零写返回 `Card not found.`（prepare×2=`storyboard-gate1-actions.ts:312/:738`+surface `:371/:831`；regen×2=`:420/:888`+surface `:481/:963`；sync=null 哨兵 `:577`+surface `:656`）。五条 fresh-null 用例逐处钉住（test `:360/:539/:844/:1781/:2035`：断言 `{error: Card not found.}`+零暂存零提交+锁已取），**红对照已证**：v3 实现上五条全红（cur 回落路径复活=按过期指针铸卡/写回）。
  - **微修轮 v5（NODE-282-R4①/③，数据流完备清扫封类）**：五个上锁事务块逐一枚举**全部**锁前计算、锁内引用的变量并按数据流判定（完备对照表随 PR #282 v5 说明发布=R5 封类审计基线）。凡流入写路径的锁前值一律搬进锁内以 fresh 重派生：`ownedIds`/`allEntityIds`（R4 点名实例，prepare-frames，现 `storyboard-gate1-actions.ts:322-323` 锁内以 fresh `payload.shots` 派生+entity 校验读在 tx 内）、`disabledModels`（四铸卡块，现 `:321/:437/:758/:913` 锁内读）、`ctx`/`threadId`（现锁内以 `fresh.threadId` 重建，`:324-325/:438-439/:759/:914`；四铸卡块 fresh select 加读 `threadId`，`card.threadId` 全文件零引用）。锁前只剩三类：身份（session ownerId/不可变主键 card.id=锁主体，行活性由锁内 guard 复核）、zod 入参、只读预检（regen×2 的 `cur`/`target0`，拒绝路径零写，铸径在锁内对 fresh target 重查重验）；sync 无锁前派生。回归用例（test `:389`「等锁期间 owned-entity 集变化→锁后按新集派生 ownedIds 进铸卡」），**红对照已证**：v4 实现上该用例红（`expected ['e0'] to deeply equal ['e0','e1']`——锁前快照直接观察到流入铸卡）。实现头注新增「数据流规则」段与对照表互为印证。
  - **微修轮 v6（NODE-282-R5①/③）**：①**thread 活性并入五处锁内 fresh 守卫**——锁前 loadCard 校验过 thread.deletedAt/ownerId，但等锁期间 thread 可失活；五处锁内 fresh 查询补 live-thread 关系过滤（`thread: { deletedAt: null, ownerId }`，`storyboard-gate1-actions.ts:311/:431/:599/:757/:915`），失活=与卡消失同形 fail-closed（"Card not found."，零写）。回归用例（test `:1846`「等锁期间 thread 软删→锁后 fail-closed 零写」），**红对照已证**：v5 实现上该用例红（死 thread 上照常铸出子卡被直接观察到）。②**措辞变真**：`spentOf`/`ownedEntityIdsFor`/`doneJobFor`/`firstGenerationIdOf` 改传调用方 tx（4 helper 全改、10 处调用点跟改），锁内读真正跑在被锁事务里，「在 tx 内」陈述自本轮为真；`resolveDisabledModels` 为跨文件全局配置读（非卡状态、不受卡锁覆盖），按锁后时点调用、如实陈述不改口径；实现头注「三类穷尽」补 thread 活性一项后恢复准确。
  - **纪律自检（v6 终态）**：diff 对 main 共三文件=`apps/web/lib/storyboard-gate1-actions.ts`（**有改动**：并发锁 helper+五处取锁+sync 读半段入锁+五处 fresh-null fail-closed（无 cur 回落）+v5 数据流封类（锁前值零流入写路径）+v6 thread 活性守卫与 helper 传 tx，均为控制面明示授权的缺陷修复，零新 spend 入口/权威/报价函数）+ `apps/web/lib/__tests__/storyboard-gate1-actions.test.ts` + 本报告；money-safety-review 全清单已在 PR #282 修复轮评论逐条跑（Step-1 严格判 NO+catch-all 零新付费点；(a)-(d) PASS；(e)=NODE-282 对抗二审链本体）；`mockGenJobCreate` 全 83 用例断言 0 次调用（$0 铁证含错误/并发/fresh-null/数据流/thread 失活路径）。本地四关：typecheck 净、`lint:parity` 棘轮 36 不增、web 包 1430 全绿（v2 时全仓 `pnpm -r test` 2753 全绿+web-build ✓，v3~v6 增量=同文件 8 用例）。CI job 链接=PR checks（见 §①）。

- **W-B3-F-P 可重跑（R5 attempt/material 原子语义）**：`pnpm --filter web exec vitest run lib/__tests__/batch-idempotency.test.ts lib/__tests__/gen-actions.test.ts lib/__tests__/factory-batch.test.ts lib/__tests__/factory-approval.test.ts`（5 helper + 8 startGen disposition/material/durable replay + 19 factory behavior + 13 审批/strict action 连续链 = 45 用例）；加真账本 `factory-batch-ledger.test.ts` 后 web targeted = 5 files / 61 tests（必须 `DATABASE_URL` 指向 *_test）；`pnpm --filter @fikirtive/otto exec vitest run src/skills/run-factory-batch.test.ts`（7 用例，含模型 schema strict 拒 token）；`pnpm -r typecheck`。R5 red-first = 3 failed / 29 passed，GREEN = 3 files / 32 tests；完整命令与 current-head CI 结果见 repair report / PR checks。
- **W-B3-E-P 可重跑（修复轮 v2 终态）**：`DATABASE_URL=<*_test> pnpm --filter @fikirtive/web exec vitest run lib/__tests__/gen-ledger.test.ts lib/__tests__/gen-actions.test.ts`（11+12 用例，含 PROBE R-EP-01a/01b；必须真 *_test Postgres——F35 守卫拒非 _test 库）；`pnpm --filter @fikirtive/worker exec vitest run src/jobs/gen.test.ts`（15 用例，含 PROBE R-EP-02a/02b）；`DATABASE_URL=<*_test> pnpm --filter @fikirtive/db exec vitest run src/credits.test.ts`（14 用例）。本地四关（ci.yml 全量口径 check/test/web-build/lint）命令+退出码见工单 EVIDENCE（mailbox WO-B3-E-P/r001）。CI job 链接=PR checks（见 §①）。
...
- **W-B3-F-P 成本证据（本批交付）**：开发/测试期真实花费 = **$0**（MockProvider + vitest 账本全真测，零真实 provider 调用；真钱验收只交方案不执行，执行点=founder 逐笔明示批准）。**批报价零新价格常量**：逐格 `pricedGenCredits`（与 startGen reserve 同一权威、同一输入）求和，测试断言逐格三数一致 + 批求和一致；毛利闸 `check-margin-floor.mjs` 本批绿（报价函数未动）。**批量上限** `MAX_BATCH_CELLS=24`（数量闸非价格闸，护住一次批准的扇出敞口）。延迟：F1 结构性验证 = 20 格 mock 级全链 enqueue 秒级完成、不串行阻塞（临时阈值 ≤30 分钟富余，真 provider 接入按 costing 实测受控修订）。回滚：本批零 schema 变更、零壳文件——revert PR 即整层拔除，不留迁移残骸。
- **W-B3-E-P 成本证据（本批交付）**：开发/测试期真实花费 = **$0**——纯证明层：零 provider 调用（真 PG 测试根本不触 provider；worker 单元测试 provider 为 mock 且断言「不该调时零调用」）、零新价格常量、零新 spend 入口、权威文件零改动。报价数字全部读自既有 `pricedGenCredits` 并与价目字面互钉（image 1cr/张、seedance-2-fast 720p/5s=8cr、720p/10s=14cr、ref-video=16cr 既有钉不动）；毛利闸 `check-margin-floor.mjs` 本批绿（报价函数未动）。真钱验收=只交方案不执行（L-C §八①②③④），执行点=founder 逐笔明示批准（宪法 2）。回滚：本批零 schema、零产品代码——revert PR 即整层拔除。
...
12. **W-B3-F-P attempt 原子语义（R4 已封类，不再待裁）**：晚到 duplicate 与后来明确 Retry 由 caller-stable attempt 区分；Otto 使用 hash-verified + CAS-consumed card id，直接 action strict 接收调用方 token。`startGen` 的 owner/project-scoped early verdict 只允许 durable reuse/conflict，miss 才经 fresh-only gates并在 project advisory transaction lock 内重查；factory precheck 不承担钱路权威。并发 ledger barrier 证据钉死首次与 FAILED retry 各最多一 job/RESERVE；同 attempt 的 FAILED replay永久 reused/0，新 attempt 仅在全部 prior FAILED 时 fresh；无 schema/migration。
13. **W-B3-E-P 覆盖盘点表（EP-A1：五组不变量 × 现有用例矩阵，逐格=已有证据引用 or 本单新增用例）**——工单第一步硬性纪律「只补真实缺口，禁止重复造既有用例」的落判依据；同表进 runtime mailbox REPORT：

    | 不变量 | 已有证据（引用，未重造） | 本单新增（缺口补齐） |
    |---|---|---|
    | ①报价=预留=结账三数一致 | 批量层真 PG：`factory-batch-ledger.test.ts:103`（逐格+批和）/`:493`（video 格）/`:543`（video 10s 全字段重放）；mock 层：`gen-actions.test.ts:91`（ref-video 16cr） | **直接 startGen 层真 PG 直证（原缺口——批量层证的是 factory 键路径）**：`gen-ledger.test.ts`「EP-A2 image count=4」「EP-A2 video 720p/10s」；报价权威钉字面：`gen-actions.test.ts`「count × 1 displayed credit」「video count>1 只收单 clip」「14cr 10s tier」 |
    | ②同幂等键重放不重复扣款（含并发） | mock 层：`gen-actions.test.ts:129/:147/:168/:245/:283`（active fast path/generic P2002/factory P2002 验料/FAILED exact attempt/durable reuse）；批量层真 PG：`factory-batch-ledger.test.ts:146/:174/:424/:455`（batch 重放/锁时并发/DONE 重放/晚到 duplicate——**durable 去重只对 factory attempt 键成立**）；原语层：`credits.test.ts` case 9（并发 reserve） | **plain 键真 PG，分两面（修复轮 v2 改准）**：**在途面已证**——`gen-ledger.test.ts`「sequential double-submit」「CONCURRENT double-submit（partial-unique backstop：恰一 job/恰一 RESERVE）」；**终态面=冲突坐实（非覆盖）**——PROBE R-EP-01a（DONE 后同 plain 键重放→新 job+第二笔预留，真库坐实）/R-EP-01b（FAILED 后同键→新 job，净额安全）：plain 键按权威注释与 active-only 索引即为在途守卫（`core/gen.ts:197-202`/`gen-actions.ts:144-150`/migration `20260612140000:5-12`），spec:301 未限定措辞与之冲突→**escalation R-EP-01，不得声称本组全覆盖** |
    | ③Retry 新键不重复结算 | 批量层真 PG：`factory-batch-ledger.test.ts:204`（并发显式 retry）/`:455`（attempt B 只补失败格）；原语层：`credits.test.ts` case 4（双 settle no-op） | **直接层全生命周期真 PG**：`gen-ledger.test.ts`「retry = NEW job settles once; late finalizers no-op」；原语查漏：`credits.test.ts` case 11（双 refund）/case 12（settle 赢迟到 refund no-op）/case 13（并发 settle∥refund 恰一 finalizer） |
    | ④count 1-4 部分失败只退失败格（reserved==settled+refunded） | 批量层真 PG：`factory-batch-ledger.test.ts:281`（N=4 独立格二败只退二）/`:323`（text $0）/`:349`（余额不足逐格 fail-closed）——**格=独立 GenJob 的形态** | **跨独立 job 恒等式已证**：`gen-ledger.test.ts`「across INDEPENDENT jobs——reserved(150)==settled(90)+refunded(60)，失败 count=4 job 全额释放 4 格 hold」（per-job 粒度，**不是**单 job 子格证明）；**单 job 子格面=冲突坐实（非覆盖）**——PROBE R-EP-02a（真实 byteplus partial 形态 2/4 已计费→chargedError→整单 FAILED+整笔全额退款+零子格 settle，`byteplus.ts:50-60`+`worker gen.ts:746-760`）/R-EP-02b（provider 静默短返 3/4→整笔 count=4 全额结算，worker 无 outputs.length==count 守卫）：权威无子格级 settle/refund，spec:301 子格粒度读法（NODE-307-R1）无法被现有权威满足→**escalation R-EP-02，不得以跨 job 组合冒充子格 partial** |
    | ⑤provider 拒/超时/worker 崩/取消 fail-closed（六态） | worker 层：`gen.test.ts:75` 系（ref-video 预扣费 fail-closed 4 例）、`gen-reaper.test.ts`（stale GENERATING/QUEUED 收割+committed 复活+free-delivery guard，12 例）、`gen-requeue.test.ts`（F04 status-guard）；web 层：`cancel-gen-job.test.ts`（6 例 unit）；原语层：`credits.test.ts` case 7（refund 赢 settle 让路） | **handleGen 本体路由（原缺口——reaper 证的是 reapStaleGenJobs 函数，非 handleGen 分支）**：`gen.test.ts` 扩 route①拒 3 例/route②超时 3 例（含 claim 丢失 stale/recent 双向）/route③崩溃恢复 3 例（resume 零重花/free-delivery guard/FAILED 终态）；**取消真账本**：`gen-ledger.test.ts` route④ 2 例（恰退一次+「太晚」诚实拒） |

    六态显式映射：①报价确认=组① · ②余额不足=`gen-ledger` 六态②例+`gen-actions` InsufficientCredits 例+`factory-batch-ledger:349` · ③provider 拒=route① · ④超时=route②+reaper 既有 · ⑤partial=组④（**跨 job 面已证；单 job 子格面=R-EP-02 冲突坐实**）· ⑥恢复=route③+`gen-ledger` 迟到 finalizer no-op+`credits` case 11-13。
    **结论（修复轮 v2 改判，EP-A7 条款执行）**：a/c/e 三组不变量已证；**b/d 两组探针坐实 spec 不变量与既有权威行为的冲突**（R-EP-01 终态同键重放=双重预留；R-EP-02 单 job 子格 partial=权威无子格账，真实形态是整单全额退款、静默短返则整单全额结算）——依工单红线（「测试暴露既有钱路缺陷/spec 与代码矛盾=停手上报，不自行修权威」）**本单以 escalation 结案而非 READY**，裁决归 founder/global control plane；事实记录=mailbox `WO-B3-E-P/r001/ESCALATION.md`，探针=`gen-ledger.test.ts` PROBE R-EP-01a/01b + `gen.test.ts` PROBE R-EP-02a/02b（锁现状、绿≠背书、裁决改权威则探针必须翻转）。权威文件零改动维持（diff=四测试文件+本报告）。**分层证明边界（如实，不称链路完备）**：真 PG 直证层的「worker 结算」经与 worker 同一 `settleCredits`/`refundReservation` 函数（金额读 RESERVE 行，worker 永不重算；factory-batch-ledger/W-B3-F-P 同型先例），且真库 helper 把账行与 job 状态更新拆两步、非单一原子事务；handleGen 的钱路路由由 worker 单元层（mock 钱原语）另行钉死——**两层是互补的分层证明，不是「真库跑真 handleGen」的端到端原子链证明**；后者留待批3 引擎集成期如需再补。断言零连环确认反向要求（GRILL 2026-07-14 审批粒度判决：一个 request=一次批准；取消断言与「停按钮」判决同向）。
