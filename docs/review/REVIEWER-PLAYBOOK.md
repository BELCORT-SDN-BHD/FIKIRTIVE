...
> **状态：可选择使用的历史技术参考，已于 2026-08-03 解除强制。**
> 它不再是 required checklist，也不授予 reviewer、merge 或产品权威。审查时优先使用当前代码、测试和运行环境提供的 review 能力；这里只在相关时查阅具体领域经验。
...
- [ ] Registration hygiene, all five spots: 1-line entry in registry.ts allSkills (order = tool order), registry.test.ts exact-name-list updated, gate assertion in migration.test.ts (cost/effect/needsApproval), CATALOG.md regenerated (catalog:check IS in CI — `pnpm quality` runs it via scripts/ci/quality.sh, so a stale CATALOG.md fails the build; wired 2026-08-08 by #800 — every earlier claim that ci.yml ran it was false, no workflow did), and an instructions.ts 'When to call X' section for any model-facing skill. (A sixth spot, the Parity Manifest, was real until 2026-08-04: the harness reset #626 / 6b6c537c deleted packages/otto/src/parity-manifest.ts and scripts/check-parity.sh. Neither the manifest nor `pnpm lint:parity` exists today.)
...
- ⚠️ requires preflight only checks null/empty-trimmed-string, and requires fields must be optional in the zod schema — marking a required-info field as z.string() (non-optional) makes the SDK hard-fail the call before the needMoreInfo path fires, degrading 刨根问底 into raw tool errors. (CATALOG.md freshness IS CI-enforced — `pnpm quality` runs catalog:check. True since 2026-08-08 (#800); the 2026-07-04 version of this line asserted it while no workflow actually ran the check.)
...
- **第九缝(Parity Manifest)——机器闸已于 2026-08-04 移除,本条现在只能靠人眼**:`packages/otto/src/parity-manifest.ts` 与 `scripts/check-parity.sh` 被 harness reset #626(6b6c537c)删除,`pnpm lint:parity` 这个命令今天不存在,漏登记没有任何东西会拦。审查时仍照双模两问核「新 server action / 新页面数据读取有没有配对 free/read skill 或明示豁免(ADMIN/VISUAL/MONEY_IN/ACCOUNT_SECURITY)」;viewContext(上下文桥)server 侧一律 ownerId 复核。要恢复机器闸 = 另开工单重建 manifest 与扫描器。
...
- [ ] 注册处所**五处**:registry.ts / registry.test / migration.test / CATALOG.md(`pnpm quality` 跑 catalog:check,2026-08-08 #800 起真在闸上)/ instructions.ts。(曾经的第六处 Parity Manifest 随 #626 一并删除,已无对应文件与闸。)
