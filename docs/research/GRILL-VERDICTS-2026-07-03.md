...
审计见 `docs/archive/research/2026-07-03-northstar-feature-capture-audit.md`(N-01~N-30)。12 个"可纳入"项直接折进相关区/和声;18 个"需拍板"判决:
...
> 来源 = founder 2026-07-07 拍板(会话内),由总审查员经 PR 入档;founder 合并本 PR = 确认。执行层挂靠见 `docs/archive/MASTERPLAN.md`。
...
> 来源 = founder 2026-07-07 拍板(同日第二批,会话内),由总审查员经 PR 入档;founder 合并本 PR = 确认。对应 `docs/archive/MASTERPLAN.md` 第七章待拍板清单 7-1~7-14 + 两笔引擎授权。
...
> 来源 = founder 2026-07-07 拍板(同日第三批,会话内),由总审查员经 PR 入档;founder 合并本 PR = 确认。执行层挂靠见 `docs/archive/MASTERPLAN.md` §〇点六「冲刺」。
...
- **点亮形状(founder 2026-07-10 批)**:"一条真实商业闭环先通"取代逐区点亮;**附加条款「每环最强」**(founder 原话:"每一个环节所用到的工具,都要是最完美无缺的 —— 如果用到 canva 那一环节,就要最强的 canva(magicpath or grok ish)")—— 闭环里每一环的工具体验必须对标该品类最强者,不许因为"先通圈"而降格任何一环。
- **伙伴分发**:批准备料(马来西亚候选机构清单:POS/收单/记账电子发票/商会,覆盖面+接口+切入话术)。
- **品牌三版 A/B**:批准,创始商家群阶段执行;现阶段照旧"AI 营销员工"。
- **手机 App 形态(founder 2026-07-10 拍板)**:"直接做成和 Otto 对话的对话而已 —— 老板只需要聊天就能达到全部效果;UIUX 要人性化且方便。" 总审查员注:= 对话流 + 结构化卡片(审批卡/回执卡/就绪卡,走缝 8 卡片契约与同一动作层);三条护栏 —— ①涉钱涉合规审批永远全屏精确卡(不在小 sheet 里点)②人工完整操作面仍由 web 城承担(App 是一扇门不是唯一门,宪法 7 不破)③推送只三类(要你决定/为防损停手/已验证结果)。语音口述可下指令,批钱永远视觉点卡。
- **全球为底、本地为皮(founder 2026-07-10 重申并升格为架构教义)**:"SEA 是入场,全球是最终要的。基础一定要以全球为底,只是做个本地化而已。" 工程含义:核心层永远全球通用(对话形态/证据阶梯/授权格子/卡片契约/credits),本地只以"可插拔层"存在 —— 渠道适配器(WhatsApp 是 SEA 皮,LINE/Zalo/iMessage 是别国皮)、合规规则包(halal 只是马来西亚实例)、语言包(rojak 是包不是硬编码)、币种配置、节庆日历、本地基准数据(如"KL 面包店均值"必须是可换的数据集)。任何把 SEA 特例写死进核心的实现 = 缺陷。
- **第一性缝律(founder 2026-07-10 升格总律)**:"遵守第一性原则,一定是有一个规则能够处理这些事情的(加国家只是一个例子)。" 定式:**凡是会变的东西,必须有且只有一条缝去装它;核心只写不变的东西。** 两问检验:新增实例动了核心=实现缺陷;找不到缝=缝清单有洞,先补缝再动工。九条缝即前九实例;本例暴露缺口 → 拟第十缝「本地化包缝」(国家=一个包:适配器/规则包/语言包/币种/节庆+基准数据),spec 期起草修宪案,founder 亲合。
- **三颗平台雷的活法（founder 2026-07-10 批，"批了"）**——依据 docs/archive/research/LEADER-PLAYBOOK-2026-07-10.md（五路龙头研究，证据分级见其第四章）：
  ① **Otto 措辞纪律**：对外一律定性"商家雇的营收员工/增长 OS 的辅助能力"，**永不**自称"通用 AI 助手/chatbot/万能问答"；WhatsApp 面的 Otto 永远锚定该商家业务上下文，不做开放闲聊 —— 这是 Meta「AI Provider」禁令（炸死 ChatGPT/Perplexity/Copilot）的安全侧，respond.io 等全体龙头的共同活法。措辞入设计法与营销文案纪律；每次扩功能回头核线（认定权在 Meta，属长期活风险）。
  ② **BSP 姿态**：先当 BSP 的**客户**，不自己申请 Solution Partner/Tech Provider —— 挡掉「名下商家违规连带赔 Meta」的合同暴露；规模与法务跟上后再评估升级。
...
## 追加判决（2026-07-14，grill 拷问批 —— founder 会话内逐题拍板，Fable control plane 经 PR 入档；founder 合并本 PR = 确认）

> 主题：FIKIRTIVE 到底卖什么、先卖哪条纵切、什么才算「不乱」。配图两张（founder 桌面）：现行全图（卖图）+ 并行建城图（建图）；源文件与拍板原始记录在外部档案库 `research/GRILL-2026-07-14-VERDICTS.md`。

| 项 | 判决 | 内容 |
|---|---|---|
| 第一期卖法 | **拍板** | 承诺=**雇一个会用全套工具的 AI 营销员工**；员工职责第一期钉死三环（做内容/发出去/唤回老客 = 2026-07-11 第一笔钱组合）；收银台=已 LIVE 的 credits 轨（seats 以后）；三环外全城一律诚实 Coming soon |
| 创作环主打 | **拍板** | Canvas 主打、打磨到品类最强；Storyboard 维持卡片形态照卖；Factory 第一期 Coming soon，不做假门面 |
| 发布环定义 | **拍板** | 提醒式发布 v1（排期真、到点提醒商家一键发、诚实标注）；Meta App Review 材料立刻排队；X 死 Connect 链接本期拆除；**一切未接通平台（TikTok 等及未来可排期平台）一律 Coming soon** |
| 唤回环定义 | **拍板（founder 改判）** | **直接 BSP 正式接入，不建辅助式过渡态**——「最终目标是要自动化，过渡=多此一举」；BSP 开户+模板审核与 Meta 递审同日排队。Meta 的提醒式仅因外部审核排队属无奈过渡，钥匙到即升自动 |
| 审批粒度 | **拍板** | **一个 request = 一次批准**（一张授权信封盖整单）；中途只有真正改变风险的事（追加花钱/对外发布/客户承诺）才再问一次；宪法 4 审批公式不动，改的是粒度；连环确认按缺陷处理 |
| 暂停/接管 | **拍板** | 设计上不必要——**不建「暂停-存档-接管-续跑」重机器**（永久砍，未来真实用户需求出现再议）；只建：①每个在跑任务一个「停」按钮（不开新动作、排队退款、在跑诚实「太晚」）②「人插手即停」由 M 区判决 7-8 推广为全城原则③想继续=再说一句话（作品即进度）。查档证实（四路机器搜档）：founder 亲定的收钱条件（D5 条件句/第一笔钱组合/收钱三闸）从不含 run-control；状态账「durable pause/takeover=launch blocker」系 agent 起草措辞，随本 PR 删除。watch/live reflection 不受影响：2026-07-07 定调与 B9 契约 6 照旧建 |
| 建/卖两图配对 | **拍板** | **「建完才收」与「纵切先收」不是二选一**：建城图管宽度——全城 13 板块对着 reference 并行建（「点亮」任务保留，多 session 各持不重叠 harness 工单）；卖图管深度——纵切三环到齐+生产三闸过即对顾客一号开收，**不等全城**。agent 只管产品，不拿「怎么卖」做工程推理（重申 2026-07-07 规矩） |
