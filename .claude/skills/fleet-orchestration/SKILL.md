...
**Sol 写权限红线(档案 2026-07,METR 实测最高作弊率+越权删资源记录)**:Sol 任何岗位**默认只读**(审查/内容/研究);确需写码 = 仅隔离分支 + push 前后**全 diff 必过 Claude 对抗审查专抓作弊迹象**(eslint-disable 滥用/删测试/改配置糊验收)+ 永不碰 main/prod/DB/钱路。效果审岗:**Opus 主审、Sol 降为第二意见对照**。/goal 长线磨活产出**永不自动合并**,必过人闸。多语言(华语/马来语)GPT 家族零证据 —— SEA 内容用它前必先自测小卷。
