...
> **FIKIRTIVE 适配注(2026-07-09,总审查员)** — 原文来自 github.com/emilkowalski/skills(MIT),一字未改。
> 使用规矩:凡涉及手感/动效/gesture 的工单必读本 skill;**与 `docs/design-system/design-rules.md` 冲突时设计法赢**,
> 三处已判决(见 design-rules §G8):①材质/深度只许浮动 chrome,卡片与数据面保持平面(§5/§D6 不动)
> ②声音/触觉原型层不采纳 ③系统字体默认不采纳(只收编字距/行高纪律)。
> 收编成法律的部分 = design-rules **§G 流体手感法**(含 Apple 实测参数值);本文件是它的完整参考读本。
