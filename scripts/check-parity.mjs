...
// Exported for the red-case tests in scripts/__tests__/check-parity.test.mjs.
export function actionExportNames(source) {
  const names = [];
...
// HTTP methods a route.ts exports, across all three legal shapes:
// `export async function GET`, `export const { GET, POST } = handlers`, and
// `export const GET = async` (SOL 复审 D-018⑤ — the third shape was a blind spot).
// Exported for the red-case tests in scripts/__tests__/check-parity.test.mjs.
export function apiRouteExportMethods(source) {
  const methods = new Set();
  for (const match of source.matchAll(/^export\s+async\s+function\s+([A-Z]+)\b/gm)) {
    if (allowedMethods.has(match[1])) methods.add(match[1]);
  }
  for (const match of source.matchAll(/^export\s+const\s+\{([^}]+)\}/gm)) {
    for (const raw of match[1].split(",")) {
      const name = raw.trim();
      if (allowedMethods.has(name)) methods.add(name);
    }
  }
  for (const match of source.matchAll(/^export\s+const\s+([A-Z]+)\s*(?::[^=]+)?=\s*async\b/gm)) {
    if (allowedMethods.has(match[1])) methods.add(match[1]);
  }
  return [...methods];
}
...
function exportedAsyncFunctions(file) {
  return actionExportNames(fs.readFileSync(file, "utf8"));
}
...
function discoverApiSurfaces() {
  ...
  for (const file of walk(apiRoot).filter((candidate) => candidate.endsWith(`${path.sep}route.ts`))) {
    ...
    for (const method of apiRouteExportMethods(source)) keys.add(`api:${route}.${method}`);
  }
  return keys;
}

function discoverReadSurfaces(readSurfaces) {
  const keys = new Set();
...
async function main() {
  ...
  const readKeys = discoverReadSurfaces(PARITY_READ_SURFACES);
