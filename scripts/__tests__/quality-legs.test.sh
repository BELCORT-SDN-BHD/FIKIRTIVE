...
# one character of one `run:` script — each is a different byte here. There is no
# face this misses, because it never looks at faces; if the runner would see
# anything differently, so does this comparison.
#
...
try_parser() {
  ...
  local canon
  canon="$(canonical_of "$out")" || return 0
  [[ -n "$canon" ]] || return 0
  parser_labels+=("$label")
  parser_canon+=("$canon")
