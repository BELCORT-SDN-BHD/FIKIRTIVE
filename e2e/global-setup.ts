 ...
 *
 *      IT IS ONLY HALF THE FENCE, and the other half cannot live here (#1052): Playwright starts
 *      the webServer BEFORE globalSetup runs, and `apps/web/.env.local` is loaded by `next start`
 *      where this process cannot see it. `support/env.ts`'s `appEnv()` carries the other half —
 *      an explicit empty value for every storage variable, which wins over both.
 ...
 */
import { e2eDatabaseUrl, offMachineCredentialsPresent } from "./support/env.js";
