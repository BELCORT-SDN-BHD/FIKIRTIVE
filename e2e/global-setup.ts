 ...
 *      for real and bills a merchant's workspace, or mails a real address, or files this suite's
 *      own deliberate failures as production errors. Checked here, before a browser exists, and
 *      fatal.
 ...
 */
import { e2eDatabaseUrl, OFF_MACHINE_CREDENTIAL_NAMES } from "./support/env.js";
...
export default async function globalSetup(): Promise<void> {
  const configured = OFF_MACHINE_CREDENTIAL_NAMES.filter(
    (name) => (process.env[name] ?? "").trim() !== "",
  );
  if (configured.length > 0) {
    throw new Error(
      `e2e: refusing to run with ${configured.join(", ")} in the environment. This suite must never be able to spend real money, send real mail, or report into a real error stream; unset them and run again.`,
    );
  }
