 ...
 *
 * #769 后续(Founder 重裁 2026-08-08,效果为王):**默认档要改成 Seedance 2.5,mini 降为
 * 可选「快速档」**。2.5 **还没上这张表**,不是漏了,是两个前置没到位 ——
 *   ① **没有价。** 2.5 的牌价是 $10.70/M(mini $3.50/M),现行价目表卖 2.5 会跌破 45%
 *      地板(整段参考视频那一格只剩 4.96% 毛利)。价必须 Founder 裁,裁完才谈上架 ——
 *      这正是上一段那条纪律本身:先有清得了地板的价,才有资格进 FLAT_PRICED_VIDEO_MODELS。
 *   ② **参数面没实测。** ModelArk 元数据只给了分辨率(480p/720p)、时长(4–30s)、
 *      帧率(24fps),**没给比例列表,也没给逐比例像素表** —— 而档位表和成本表两样都要。
 *      给 2.5 抄一份 2.0 的比例表进来,就是这个文件明令禁止的「给没核过的档位编数字」。
 *      mini 当初是实测过才敢逐字沿用 2.0 档位的(见下面档位表的注释),2.5 也得走同一遍。
 * 还有一处**结构**上的前置:`assertSpendableModel` 今天的判据是「必须等于唯一的在产模型」,
 * 也就是说系统结构上只能卖一台。「2.5 默认 + mini 快速档」= 同时可卖两台,那道闸要从
 * 「唯一模型」改成「菜单内 ∩ 已定价 ∩ 未被后台禁用」。它该和裁价一起落 ——
 * 「能选哪几台」与「每台多少钱」是同一个产品决定的两面。
 */
