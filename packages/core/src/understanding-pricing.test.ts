 ...
 *
 * 背景一句话:理解(看图 / 读文档 / 看视频)原本是平台自费、商家零触点;Founder
 * 2026-08-31 裁决「就是用户使用照算」,它变成一个真正的计费面。于是它第一次需要一个价,
 * 而这个文件钉的就是那个价的**来源**:
 ...
 * 本文件**不测**钱路接线(reserve/settle、快照、暂停态)—— 那是 worker 侧的验收,
 * `money-engine-acceptance.test.ts` 的 A9 行仍留着占位。
 */
