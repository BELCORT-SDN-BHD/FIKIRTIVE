/**
 * quote-version 的纯判词（规格 docs/specs/creation-engine.md §5 :170，FSE-012）。
 *
...
describe("creation §5 :170 FSE-012 报价版本指纹", () => {
  it("creation §5 :170 FSE-012 CREATE-A1 同一份报价算两次是同一串", () => {
    expect(cardQuoteVersion(CARD)).toBe(cardQuoteVersion({ ...CARD }));
  });

  it("creation §5 :170 FSE-012 CREATE-A1 决定价格的每一格变了都换一串", () => {
  ...
  it("creation §5 :170 FSE-012 CREATE-A1 与报价无关的那些格改了不换串", () => {
    const base = cardQuoteVersion(CARD);
  ...
  it("creation §5 :170 FSE-012 CREATE-A1 型号那一格不参与:DTO 会丢掉它,算进去等于刷新后批不动", () => {
  ...
  it("creation §5 :170 FSE-012 CREATE-A1 客户端解析过的那份与库里原始那份算出同一串", () => {
  ...
  it("creation §5 :170 FSE-012 CREATE-A1 读不懂的 payload 照样算得出一串,不抛", () => {
