 ...
 *
 * WHY IT LIVES IN `@fikirtive/core` (#602 r2, cross-family judge P1-3). T2c put it in `apps/web`
 * on the reasoning that "this is what a CARD says to a merchant". That held only while the web app
 * was the sole reader — and it was not: `@fikirtive/otto`'s canvas skill has its own copy of the
 * paid-card delete guard, because a package cannot import from an app. The two copies drifted the
 * moment the faces gained `queued`/`generating`, and Otto went back to being able to delete a
 * merchant's in-flight PAID card without refunding or stopping it. A vocabulary with two homes is
 * not a vocabulary, so this is now pure, dependency-free, and in the one place every reader can
 * reach; `apps/web/lib/canvas-card-status.ts` re-exports it verbatim.
 *
 ...
 *
 * `done` WITH NO MEDIA AND NO OUTPUT is `missing`, not `done` (#602 r2, judge P1-3). A row can say
 * done and have nothing to show — a library picture whose media no longer resolves, or an Otto
 * placement that never got one. Answering `done` there looked harmless until you follow it to the
 * card: the renderers fall back to the spinner for any face they cannot draw media for, so a
 * "done" card with no picture spun for ever. `missing` is the word the board already has for
 * exactly this, and it rests.
 */
export function canvasCardFace(input: {
  ...
  switch (input.rowStatus) {
    // Neither media nor a paid output reached the two branches above, so there is nothing to show.
    case "done": return "missing";
    case "failed": return "failed";
    case "cancelled": return "cancelled";
...
/**
 * A PAID GENERATION STILL IN FLIGHT — the one definition of "deleting this costs the merchant"
 * (#602 r2, judge P1-3).
 *
 * An image/video card with no media yet whose job may still be running. Deleting one does NOT
 * refund (its GenJob already reserved and will settle) and re-running starts a fresh paid action →
 * a SECOND charge. Both the human delete confirm (`useCanvasGen.isInFlightPaidGen`) and Otto's
 * `remove` refusal (`manageCanvas`) ask THIS function, because two copies of it is how Otto came
 * to not recognise `queued`/`generating` at all.
 *
 * It reads a card FACE, not a stored row word, and that is not a detail: both callers get their
 * input from a board read, and a board read returns faces (`CanvasNodeDTO.status`). Asking a row
 * word here would mean re-reading the database for something the caller is already holding.
 *
 * `timeout` counts alongside the in-flight faces because the job may still be running even though
 * the browser stopped watching. Every other resting face — failed, cancelled, missing, unknown —
 * is an answer, so the warning would be false.
 */
export function canvasCardIsInFlightPaid(card: {
  type: string;
  status?: string | null;
  url?: string | null;
}): boolean {
  if (card.type !== "image" && card.type !== "video") return false;
  if (card.url) return false;
  return isInFlightCardFace(card.status) || card.status === "timeout";
}
