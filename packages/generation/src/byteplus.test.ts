...
import { BytePlusProvider, IMAGE_MODEL_MAP, VIDEO_MODEL_MAP, readReceipt } from "./byteplus.js";
 ...
 *
 * 三件事在这里钉死:读得回来的**读回来**、读不回来的**留空**(绝不编)、以及读回执这条路
 * **永远不许**把一单已经成功的付费生成推翻成失败。第三条是本组里最贵的一条:图片路径上
 * `res.ok` 之后每一次抛出都会被翻成 chargedError 并终态失败 —— 一个记账字段读崩了就赔掉
 * 一单钱,是这次改动唯一能引入的新花钱风险。
 * ──────────────────────────────────────────────────────────────────────────── */
describe("#776 回执读回:图片", () => {
  it("usage.total_tokens 与 revised_prompt 都读回来", async () => {
    stubFetch((url) => {
      if (url.endsWith("/images/generations")) {
        return jsonRes({ data: [{ url: "https://tos/img1.png", revised_prompt: "a red apple on a wooden table, soft light" }], usage: { total_tokens: 16384 } });
      }
      return bytesRes();
    });
    const out = await new BytePlusProvider("ark-test").generate({ prompt: "an apple", inputImageUrls: [], count: 1, model: "seedream" });
    expect(out[0]!.receipt).toEqual({ finalPrompt: "a red apple on a wooden table, soft light", billedUnits: 16384 });
  });

  it("引擎只报了量、没报提示词 ⇒ 只带量,提示词**缺席**(不拿商家那句冒充)", async () => {
    stubFetch((url) => {
      if (url.endsWith("/images/generations")) return jsonRes({ data: [{ url: "https://tos/img1.png" }], usage: { total_tokens: 16384 } });
      return bytesRes();
    });
    const out = await new BytePlusProvider("ark-test").generate({ prompt: "an apple", inputImageUrls: [], count: 1, model: "seedream" });
    expect(out[0]!.receipt).toEqual({ billedUnits: 16384 });
    expect(out[0]!.receipt!.finalPrompt).toBeUndefined();
  });

  it("引擎什么都没报 ⇒ 整个回执缺席 = 未知", async () => {
    stubFetch((url) => {
      if (url.endsWith("/images/generations")) return jsonRes({ data: [{ url: "https://tos/img1.png" }] });
      return bytesRes();
    });
  ...
  it("每张图带自己那份回执 —— count 张 = count 次付费调用,不共享一个数", async () => {
    ...
    stubFetch((url) => {
      if (url.endsWith("/images/generations")) {
        n++;
        return jsonRes({ data: [{ url: `https://tos/img${n}.png`, revised_prompt: `rewrite ${n}` }], usage: { total_tokens: 1000 * n } });
      }
      return bytesRes();
    });
    const out = await new BytePlusProvider("ark-test").generate({ prompt: "an apple", inputImageUrls: [], count: 2, model: "seedream" });
    expect(out.map((o) => o.receipt)).toEqual([
      { finalPrompt: "rewrite 1", billedUnits: 1000 },
      { finalPrompt: "rewrite 2", billedUnits: 2000 },
    ]);
  });
  ...
  it("回执字段是垃圾也只是没读到 —— 一单已经付过钱的生成绝不因此变成失败", async () => {
    stubFetch((url) => {
      if (url.endsWith("/images/generations")) {
        // 0 / 负数 / 小数 / 非字符串:都不是引擎在报数,一律当没读到,而不是抛出去。
        return jsonRes({ data: [{ url: "https://tos/img1.png", revised_prompt: { not: "a string" } }], usage: { total_tokens: -1 } });
      }
      return bytesRes();
    });
    const out = await new BytePlusProvider("ark-test").generate({ prompt: "an apple", inputImageUrls: [], count: 1, model: "seedream" });
  ...
  it("空白提示词等于没报(空串不是一个答案)", async () => {
    stubFetch((url) => {
      if (url.endsWith("/images/generations")) return jsonRes({ data: [{ url: "https://tos/img1.png", revised_prompt: "   " }], usage: { total_tokens: 5 } });
      return bytesRes();
    });
    const out = await new BytePlusProvider("ark-test").generate({ prompt: "an apple", inputImageUrls: [], count: 1, model: "seedream" });
    expect(out[0]!.receipt).toEqual({ billedUnits: 5 });
  });
});

describe("#776 回执读回:视频", () => {
  beforeEach(() => vi.useFakeTimers());
  afterEach(() => vi.useRealTimers());

  it("从成功那一次 poll 上读走计费量与提示词", async () => {
    stubFetch((url, init) => {
      if (url.endsWith("/contents/generations/tasks") && init?.method === "POST") return jsonRes({ id: "cgt-r1" });
      if (url.includes("/tasks/cgt-r1")) {
        return jsonRes({ status: "succeeded", content: { video_url: "https://tos/v.mp4" }, usage: { total_tokens: 108900 }, revised_prompt: "slow push-in on the product, warm light" });
      }
      return bytesRes();
    });
    ...
    await vi.runAllTimersAsync();
    const out = await promise;
    expect(out.receipt).toEqual({ finalPrompt: "slow push-in on the product, warm light", billedUnits: 108900 });
  });

  it("引擎没报提示词 ⇒ 只有计费量", async () => {
    stubFetch((url, init) => {
      if (url.endsWith("/contents/generations/tasks") && init?.method === "POST") return jsonRes({ id: "cgt-r2" });
      if (url.includes("/tasks/cgt-r2")) return jsonRes({ status: "succeeded", content: { video_url: "https://tos/v.mp4" }, usage: { total_tokens: 108900 } });
      return bytesRes();
    });
    const promise = new BytePlusProvider("ark-test").generateVideo({ prompt: "roll", imageUrl: "", durationSeconds: 5, model: "seedance-2-mini" });
    await vi.runAllTimersAsync();
    expect((await promise).receipt).toEqual({ billedUnits: 108900 });
  });

  it("整份 usage 缺席 ⇒ 回执缺席,片子照样交付", async () => {
    stubFetch((url, init) => {
      if (url.endsWith("/contents/generations/tasks") && init?.method === "POST") return jsonRes({ id: "cgt-r3" });
      if (url.includes("/tasks/cgt-r3")) return jsonRes({ status: "succeeded", content: { video_url: "https://tos/v.mp4" } });
      return bytesRes();
    });
    const promise = new BytePlusProvider("ark-test").generateVideo({ prompt: "roll", imageUrl: "", durationSeconds: 5, model: "seedance-2-mini" });
    await vi.runAllTimersAsync();
...
describe("#776 readReceipt 本身:永不抛,永不编", () => {
  it.each([
    ["null", null],
    ["undefined", undefined],
    ["字符串", "not an object"],
    ...
    ["total_tokens 是字符串", { usage: { total_tokens: "16384" } }],
    ["total_tokens 是 0", { usage: { total_tokens: 0 } }],
    ["total_tokens 是小数", { usage: { total_tokens: 1.5 } }],
    ["total_tokens 是 NaN", { usage: { total_tokens: Number.NaN } }],
    ["total_tokens 是 Infinity", { usage: { total_tokens: Number.POSITIVE_INFINITY } }],
  ])("%s ⇒ undefined,不抛", (_label, payload) => {
    expect(() => readReceipt(payload)).not.toThrow();
    expect(readReceipt(payload)).toBeUndefined();
  });

  it("取值本身抛异常也只是没读到(getter 炸了不许炸掉一单生成)", () => {
    const hostile = { get usage(): unknown { throw new Error("boom"); } };
    expect(() => readReceipt(hostile)).not.toThrow();
    expect(readReceipt(hostile)).toBeUndefined();
  });

  it("超长提示词被截断,而不是被丢掉 —— 半句真话仍是真话", () => {
    const long = "x".repeat(9_000);
    const r = readReceipt({ usage: { total_tokens: 1 } }, { revised_prompt: long });
    expect(r!.finalPrompt!.length).toBe(4_000);
    expect(r!.finalPrompt!.startsWith("xxxx")).toBe(true);
  });

  it("逐项优先:data 项上的提示词压过顶层的(逐张不同时不许混成同一句)", () => {
    const r = readReceipt({ revised_prompt: "top level", usage: { total_tokens: 3 } }, { revised_prompt: "per item" });
    expect(r!.finalPrompt).toBe("per item");
  });
});
