 ...
 *
 * #1435(零排队)修订:视频任务**不再**按整个任务(提交 + 轮询全程)占一个位 —— 那是旧实现
 * 自己的代价(客户端在一次调用里原地等到终态),不是账户额度的性质。账户
 * `concurrent_requests` 管的是同时在途的**请求**,提交是一个请求,之后每次轮询各是它自己
 * 独立的一个请求(`byteplus.ts` 的 `submitVideo` / `pollVideo`,各自单独 `gate.run(...)`),
 * 与图片路径读这个闸门的方式完全同构。单次占位时长现在与一次图片 POST 同量级
 * (≤ `ARK_CONTROL_TIMEOUT_MS`,60s),不是 15 分钟。
 */
