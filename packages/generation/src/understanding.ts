 ...
 * 生成按次卖档位价,失败要分「扣没扣」;理解自 A9(2026-09-01)起按上传时刻快照价向商家
 * 计费,但 reserve/settle/refund 的钱纪律全部长在 worker 的 understand.ts(端口这层只管
 * 调用供应商),供应商成本(不到千分之几分钱一次)仍由平台日预算保险丝兜底。把两种钱形
 * 塞进一个端口,迟早会有人把 `chargedError` 那套纪律套到不该套的地方,或者反过来。
 *
