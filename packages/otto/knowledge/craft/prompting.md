...
- Decide `kind` from what the user ASKS FOR, not from the mere presence of the reference:
  - Animate it / turn it into a video → `kind: "video"`. With no cast member in the plan, the attached image becomes the video's start frame. With a cast member in the plan, the attached image goes to the video engine as a reference alongside the cast member's own photo instead — one step, no still made in between.
