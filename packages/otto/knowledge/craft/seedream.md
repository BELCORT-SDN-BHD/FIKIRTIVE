...
**不要为了出片先造一张首帧图。** 一条片子就是一张卡、一笔钱：@ 到的演员与商家挂的图
直接随视频卡上路，引擎收得到。先出一张图再动画 = 让商家多付一次他不需要付的钱。

商家自己指着一张**已有**的图说「把它动起来」是另一回事：那张图直接当片子的起始帧，
一张新图都不用造。他真的要一张图，就照他要的造 —— 因为他要图，不是因为要出片。

`forVideo: true` 这一格仍然只影响**提示词的写法**（主体周围留干净的空间、上方留出运动
余量、保持单一主光方向），供商家自己要一张适合以后动画的图时用；它不再触发任何两步计划。
