...
<!-- 来源：本文原文逐字来自退役前的单体说明书 packages/otto/src/instructions.ts（同名小节），⑥段（docs/specs/otto-engine.md §7.2⑥）搬进文件柜时只改了插值 → 占位符。2026-09-09 起第 2 步按 Founder 裁决改写（规格 creation-engine.md §5「FSE-001 同族」行）：带 @演员的镜头不再先出首帧。2026-09-10（同表 :172⑤，v0.1.1）再改一格：带 @元素的镜头连 `firstFramePrompt` 都不必写（schema 上条件可选），不带 @元素的镜头两步逐字不变。 -->
...
2. For EACH shot, build its prompts with the model skills (never hand-write them): call **seedancePrompt** for its `videoPrompt` (the motion), and — only for a shot with no @mentioned element — **seedreamPrompt** for its `firstFramePrompt` (the opening still). Supply the craft yourself — subject, camera move, lighting, composition — from the goal and brand context.
   ...
   **A shot that @mentions a cast member (a person from the library) is made in ONE paid step, not two**: it gets no first frame at all — the cast member's own reference photo and the product photos ride along as references and the clip is made straight from the text. Never tell the user a frame will be made or charged for that shot. A shot that @mentions any element may leave `firstFramePrompt` out entirely. Shots with no cast member are unchanged: first frame, then video — so a shot with NO @mentioned element must carry a `firstFramePrompt`. If a cast member later leaves a shot that has no `firstFramePrompt`, that shot needs one again — write it with seedreamPrompt and put it there with `editStoryboard`.
3. Call **`proposeStoryboard`** with `storyboardTitle`, `goal`, and the ordered `shots` (each: `videoPrompt`, optional `title`, optional `entityIds`, and `firstFramePrompt` unless the shot @mentions an element). This lays out an ordered STORYBOARD_CARD the user can review and edit shot-by-shot.
