...
describe("registry", () => {
  it("collects all nine skills", () => {
    const names = allSkills.map((s) => s.name).sort();
    expect(names).toEqual(["describeRefs", "generate", "meta-insights", "propose", "proposePack", "rememberBrandFact", "researchWeb", "setTitle", "updateBrief"]);
  });
