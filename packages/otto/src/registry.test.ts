...
describe("registry", () => {
  it("collects all thirty-four skills", () => {
    const names = allSkills.map((s) => s.name).sort();
    expect(names).toEqual(["approveScheduledPost", "cancelScheduledPost", "describeRefs", "editScheduledPost", "generate", "importMedia", "ingestProduct", "list-meta-pages", "listPublishTargets", "listScheduledPosts", "lookupProducts", "manageCanvas", "manageMedia", "meta-ad-performance", "meta-expert", "meta-insights", "meta-list-objects", "propose", "propose-ad-build", "propose-meta-action", "proposePack", "proposeResearch", "proposeStoryboard", "rememberBrandFact", "renderVideo", "researchWeb", "saveCustomerSegment", "saveOffer", "saveProduct", "schedulePosts", "seedancePrompt", "seedreamPrompt", "setTitle", "updateBrief"]);
  });
