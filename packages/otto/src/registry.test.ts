...
describe("registry", () => {
  it("collects all thirty skills", () => {
    const names = allSkills.map((s) => s.name).sort();
    expect(names).toEqual(["approveScheduledPost", "cancelScheduledPost", "describeRefs", "editScheduledPost", "generate", "ingestProduct", "list-meta-pages", "listPublishTargets", "listScheduledPosts", "lookupProducts", "meta-ad-performance", "meta-expert", "meta-insights", "meta-list-objects", "propose", "propose-ad-build", "propose-meta-action", "proposePack", "proposeResearch", "proposeStoryboard", "rememberBrandFact", "researchWeb", "saveCustomerSegment", "saveOffer", "saveProduct", "schedulePosts", "seedancePrompt", "seedreamPrompt", "setTitle", "updateBrief"]);
  });
