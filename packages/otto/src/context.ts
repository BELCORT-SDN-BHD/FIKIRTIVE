 ...
 * 数**,而不是已成功数:
 *
 *   打供应商之前  —— `taken >= 上限` ⇒ 当场拒绝(不打供应商、不收费);否则 `taken += 1`。
 ...
 * 抑制)。预扣按上限满额 hold、结算按 `succeeded`,见 runtime.ts `ottoBudgetArgsFor`。
 */
export type OttoSearchSlots = { taken: number; succeeded: number };
...
export type LibraryItemView = {
  ...
  /**
   * 商家自己那句提示词 —— 但**只在没有 `sentPrompt` 佐证时**才把它当「商家原话」引用。
   *
   * #914 r4 起口径写准:入队之前平台自己的拼装步骤动过手的那些单(cowork 的
   * `composePrompt` 给未配专属提示词技能的模型家族追加家族×模式指令词),这一列装的是
   * **拼装之后**的那句,不是商家逐字写下的那句。想说「我们实际送出的是什么」,读
   * `sentPrompt`,别拿这一列去替它作证。
   */
  prompt: string;
  /**
   * #776 —— 引擎自报**它真正跑的那句提示词**。
   *
   * Otto 解释「上次为什么长这样」时需要的正是这一句 —— 引擎实际执行的那句在这里,与
   * 我们送出的那句(`sentPrompt`)常常不同,而差别往往就是答案。
   *
   * 三种状态,**互不相同**,别混,而 `null` 这一种 #914 起按 kind 分家:
   *   · 字符串 = 引擎报的那句(已在 web 侧一处过白标过滤,可以直接说出口);
   *   · `null` 且 kind:"video" = 引擎**没报**(或回执落库前的历史行)= 未知 → 明说不知道,
   *     不得拿 `prompt` 冒充它;
   *   · `null` 且 kind:"image" = **恒为真**,是图片引擎结构上没有这个字段的能力限制,
   *     不是「这次没报」→ 别说「不知道」,manage-library.ts 的工具描述已经把这句讲给
   *     模型听(#914 r2);
  ...
  /**
   * #914 r4 —— **我们自己**交给引擎的那一句(不问引擎要,所以图片这条路也答得出来)。
   * 事实由 worker 在调用引擎那一刻记下,与商家详情面板同一条记录、同一次比对。
   *
   * 三种形状,别混:
   *   · `{ verbatim: true }`        = 我们送出的与商家写的**逐字**相同 → 可以说「就是你
   *     写的那句,一个字没动」;
   *   · `{ verbatim: false, text }` = 不同 → `text` 就是实际送出的全文(已过白标),照它
   *     说,别转述成「大概加了点东西」;
   *   · `null`                      = 这一条**不是一次引擎调用的产物**:早于这条记录的
   *     历史行,或者根本不是引擎产的(商家上传的图、裁剪出来的图也在图库里)→ **什么都
   *     别说**,既不说「原样送出」也不说「被改过」;
   *   · 键**缺席**                  = 这条路根本没查这一列(history 分页就是如此)。
   */
