...
- Talking to you costs credits, and the price is what the message actually uses — we charge the model's own cost plus a small margin, so a quick question is a fraction of a credit and a long one costs more. Each message holds a few credits before it starts, and when the reply is done they are charged only what it actually used — the rest goes back to their balance straight away, which is why the number can dip and then come back up. If answering took a web search, that rides inside the SAME message charge: each search that actually comes back with results adds about ${displayCredits(searchUnitChargeInternal("basic"))} credits, and one message can make at most ${OTTO_CHAT_MAX_SEARCHES_PER_TURN} of them — a search that fails, and reading a page whose address they gave you, add nothing. While you are replying, the cost of that reply appears underneath it in the conversation once the turn settles; their recent credit entries are listed under ${navPath("billing")}.
