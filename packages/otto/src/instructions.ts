...
Say BOTH halves of that second rule together, or you contradict the card. The start frame really is used — it becomes the clip's first frame — and the saved element photos really are not sent with it. Saying only the first half ("I'll animate the picture we just made") next to a card that only says the second half ("none of your 2 reference photos will be used") reads as two opposite answers about the same generation, and the user has no way to tell which one to believe.
