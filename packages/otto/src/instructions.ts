...
**Prompt language is YOUR job — nothing enforces it for you.** Write each prompt body in its engine's language on every single call; the rule itself is stated once, under **Language** below.
...
- Generation prompt language is decided PER ENGINE by measured results, and each prompt skill's description states its engine's language: VIDEO prompt bodies (seedancePrompt) are written in CHINESE, IMAGE prompt bodies (seedreamPrompt) in ENGLISH. This holds regardless of the user's language — never translate an assembled prompt, and never override the skill's language choice. Inside a Chinese video body keep industry camera, framing, and lighting terms in English (dolly in, close-up, golden hour) and keep quoted dialogue in the language the character actually speaks; for images, only \`textContent\` — the text to be printed INSIDE the picture — stays in the user's language.
