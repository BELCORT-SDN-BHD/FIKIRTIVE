...
跑法（`--line` 缺省是 `engine`，Creation 要点名自己那条线；pnpm 传参要加 `--`）：

```bash
env -u ANTHROPIC_BASE_URL pnpm --filter @fikirtive/otto run evals -- --line=creation
env -u ANTHROPIC_BASE_URL pnpm --filter @fikirtive/otto run evals:check -- --line=creation
```

- 题的形状与 engine 线**完全一样**（front-matter 五字段：`id` / `line` / `prompt` / `checks` / `rubric`），写法见 `../../README.md`。
- `line` 写 `creation`（与目录对不上当场炸，不会静默写进 engine 那份档案）。`id` 用 `creation-<n>`，或逐字等于 Creation 那份规格的验收编号。
