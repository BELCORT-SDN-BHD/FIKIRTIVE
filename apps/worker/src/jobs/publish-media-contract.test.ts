...
// A minimal static-PNG prefix (signature + IHDR) so the 工单 F byte-gate in pass 1b confirms the
// png sanity control below as image/png. The video / empty-mime / mixed-carousel cases are refused at
// pass 1a (mime pre-filter) and never reach a read, so this default only feeds the png control.
