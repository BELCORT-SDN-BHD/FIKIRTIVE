...
describe("QUEUE-A1 / QUEUE-A2 — 4 条视频真的提交在飞时，施工位与账本都是空闲的，商家 B 的短任务立即开跑不等队", () => {
  ...
  // resolve **之前**就已经 DONE。诚实边界(判官修正轮实测钉死):本 harness 里 A、B 各自
  // 直接调 handleGen,promise 链彼此独立,不存在共享施工位或并发上限——所以这条时序断言在
  // 旧的原地轮询实现下**照样绿**(判官用真旧实现对照验证过),它不能单独证伪「B 等了 A」。
  // 本文件证明的是投递结构(handleGen 在终态前放手,见上面的结构断言,旧实现下那条才红);
  // A1 那句「B 立即开跑」的观测证据,由规格自己要求的第三轮真机走查兑现——两个账号同时上,
  // 一边连发 4 条长视频、一边发一张图,图要在几秒内出来。
  it(
