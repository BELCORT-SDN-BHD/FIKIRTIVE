 ...
 * existed. A timeout abandons the WAIT, not the work: whatever the store had already begun may
 * still land (the writes are idempotent — the Asset is keyed by owner+contentHash and the
 * pointer write is a conditional updateMany), it simply stops being something DONE waits for.
 */
  ...
  try {
    await Promise.race([
      (async () => {
        ...
        await prisma.genJob.updateMany({ where: { id: jobId, ownerId }, data: { lastFrameAssetId: asset.id } });
      })(),
