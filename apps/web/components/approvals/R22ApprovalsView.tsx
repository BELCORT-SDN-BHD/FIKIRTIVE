...
export function R22ApprovalsView({ fixture = false, fixtureState = "ready", fixtureOutcome = "success" }: { fixture?: boolean; fixtureState?: "ready" | "loading" | "error" | "permission" | "empty" | "unknown"; fixtureOutcome?: "success" | "error" | "permission" | "unknown" }) {
  ...
  if (readState === "loading") return <main className="r22-approvals" data-r22-approvals data-state="loading"><header><div><h1>Approvals</h1><p>Everything waiting on your decision, in one list.</p></div></header><section className="r22-approvals-unavailable" aria-busy="true"><h2>Loading workspace approvals…</h2><p>Nothing is guessed while this loads.</p></section></main>;
  if (readState === "error" || readState === "permission" || readState === "unknown") return <main className="r22-approvals" data-r22-approvals data-state={readState}><header><div><h1>Approvals</h1><p>Everything waiting on your decision, in one list.</p></div></header><section className="r22-approvals-unavailable" role={readState === "error" ? "alert" : "status"}><AlertCircle aria-hidden="true" /><h2>{readState === "error" ? "Approvals could not load" : readState === "permission" ? "Approvals are not available to this member" : "Approval read outcome is unknown"}</h2><p>{readState === "error" ? "The unified fixture read failed. Nothing is guessed in its place." : readState === "permission" ? "No titles, counts or approval costs are exposed without the required capability." : "The read may still finish. Retry the same read instead of treating it as empty."}</p>{readState !== "permission" ? <Button unstyled type="button" onClick={() => { setItems(FIXTURE_ITEMS); setReadState("ready"); setRestored(true); }}>Retry</Button> : null}</section></main>;
  ...
  function fixWithOtto(item: ApprovalItem) {
    ...
    otto?.openPanel();
    setNotice(`${opened}. What needs fixing: ${item.blocker.fixContext}. Fixture state only — no cap, batch or cr was changed.`);
  }
  ...
  return <main className="r22-approvals" data-r22-approvals data-fixture>
    ...
    {visible.length ? <section className="r22-approvals-list">{tab === "waiting" ? groupedWaiting.map((entry) => <div className="r22-approvals-group" key={entry.group.id}><header><h2>{entry.group.label}</h2>{entry.group.time ? <span>{entry.group.time}</span> : null}</header>{entry.groupItems.map(renderCard)}</div>) : <div className="r22-approvals-group"><header><h2>Last 7 days</h2><span>times in GMT+8</span></header>{visible.map(renderCard)}</div>}</section> : <section className="r22-approvals-empty"><h2>{filter !== "all" ? "Nothing here matches this filter" : tab === "waiting" ? "Nothing needs your review" : tab === "approved" ? "Nothing approved in the last 7 days" : "Nothing sent back yet"}</h2><p>{filter !== "all"
      ? filter === "otto" ? "No approvals from Otto in this list right now." : "No approvals that cost cr in this list right now."
