        ...
        {error ? (
          ...
          <ul className="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-2">
            {items.map((item) => {
              ...
              // E2E-CRE-PAV-006 / LIB-STG-P2-005 (2026-09-04 Codex staging 审计): the button's
              // accessible name is a CONCISE title + media type (lib/library-item-a11y.ts), not
              // the raw prompt — a paragraph-length aria-label exhausts screen-reader navigation
              // and defeats voice-control targeting. The full prompt still reaches sighted users
              // via `title` below and the caption span, neither of which this line touches.
