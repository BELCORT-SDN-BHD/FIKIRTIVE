...
import { isInFlightPaidGen } from "../../lib/canvas-gen";
...
export default function FlowCanvas({ projectId, entities = [], activeThreadId = null, activity, skin }: { projectId: string; entities?: EntityDTO[]; activeThreadId?: string | null; activity?: Set<string>; skin?: "gb" }) {
  ...
  // Is the card awaiting delete a PAID generation still in flight? If so the
  // confirm must warn that removing won't refund and re-running charges again —
  // this is what actually stops the "delete a stuck-looking paid card → reclick →
  // second charge" vector (deleteKeyCode is null, so the ✕→confirm is the only
  // delete path). See isInFlightPaidGen.
