  ...
  if (state !== "ready") {
    ...
    return <main className="r22-notifications" data-r22-notifications data-state={state}>
      ...
      <section className="r22-notifications-state" role={state === "error" ? "alert" : undefined}>
        ...
        <p>{permission ? "Ask a workspace administrator for the capability to read this feed." : state === "loading" ? "The current workspace feed is being read. Nothing is guessed while this loads." : state === "error" ? "Nothing is guessed in its place. Retry the same workspace read." : state === "unknown" ? "The read may still finish. Nothing is guessed in its place." : "This frontend is ready for the server-backed feed, unread count and read actions. Until that adapter exists, Fikirtive will not invent events or say that everything is read."}</p>
