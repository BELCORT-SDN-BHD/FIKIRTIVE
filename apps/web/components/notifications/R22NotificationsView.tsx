  ...
  if (state !== "ready") {
    ...
    return <main className="r22-notifications" data-r22-notifications data-state={state}>
      ...
      <section className="r22-notifications-state" role={state === "error" ? "alert" : undefined}>
        ...
        <h2>{permission ? "Notifications are not available to this member" : state === "loading" ? "Loading notifications…" : state === "error" ? "Notifications could not be loaded" : state === "unknown" ? "Notification read outcome is unknown" : "Notification delivery is not connected yet"}</h2>
        <p>{permission ? "Ask a workspace administrator for the capability to read this feed." : state === "loading" ? "The current workspace feed is being read. No empty history is inferred while it loads." : state === "error" ? "No empty feed was inferred. Retry the same workspace read." : state === "unknown" ? "The read may still finish. No unread count, title or empty history was inferred." : "This frontend is ready for the server-backed feed, unread count and read actions. Until that adapter exists, Fikirtive will not invent events or say that everything is read."}</p>
