...
export function R22HelpView({ fixture = false, state = fixture ? "ready" : "unavailable", initialArticleId, supportOutcome = "success", initialSupportPhase }: { fixture?: boolean; state?: HelpState; initialArticleId?: string; supportOutcome?: SupportOutcome; initialSupportPhase?: Exclude<SupportPhase, "draft" | "submitting" | "error" | "unknown"> }) {
  ...
  const context = [includeRoute ? `Page: ${pathname}` : "", includeWorkspace ? "Please include my current workspace after re-authorizing it." : ""].filter(Boolean).join("\n");
  ...
  return <main className="r22-help" data-r22-help data-fixture={fixture || undefined}>
    ...
    <div className="r22-help-grid">
      <section className="r22-help-articles" aria-labelledby="help-articles-heading">
        ...
        {state !== "ready" ? <div className="r22-help-unavailable" role={state === "error" ? "alert" : "status"}><b>{state === "loading" ? "Loading verified product help…" : state === "error" ? "Product help could not be loaded" : state === "permission" ? "Product help is not available to this member" : state === "unknown" ? "Product help read outcome is unknown" : "The verified help library is not connected yet"}</b><p>{state === "loading" ? "No empty help library is inferred while the current workspace corpus is read." : state === "error" ? "The product article read failed. No policy page was substituted as help." : state === "permission" ? "Ask a workspace administrator for access, or use the support exit without attaching workspace context." : state === "unknown" ? "The corpus read may still finish. No article or empty result was inferred; retry this same read." : "Fikirtive will not relabel policy pages as product guidance. Use the real support exit while the versioned article corpus is being connected."}</p>{state === "error" || state === "unknown" ? <Link href={fixture ? "/help?fixture=r22" : "/help"}>Retry</Link> : null}</div> : selectedArticle ? <article className="r22-help-article-detail"><Button unstyled type="button" onClick={closeArticle}><ArrowLeft data-icon="inline-start" aria-hidden="true" /> Back to results</Button><small>{selectedArticle.category}</small><h3>{selectedArticle.title}</h3><p>{selectedArticle.summary}</p><p>This deterministic article is present for R22 interaction QA only. Production remains unavailable until a versioned help corpus is connected.</p></article> : visible.length ? <ul>{visible.map((article) => <li key={article.id}><Button unstyled type="button" onClick={() => openArticle(article)}><small>{article.category}</small><b>{article.title}</b><span>{article.summary}</span></Button></li>)}</ul> : <div className="r22-help-unavailable"><b>No matching article</b><p>Try a broader task name or contact support.</p></div>}
      </section>
      <section className="r22-help-contact" aria-labelledby="help-contact-heading">
        ...
        {fixture ? <>
          {supportPhase === "draft" ? <div className="r22-help-support-form">
            ...
            <fieldset><legend>Optional context</legend><label><Checkbox checked={includeRoute} onCheckedChange={(checked) => setIncludeRoute(checked === true)} /><span><b>Current page</b><small>Include this route, without form contents.</small></span></label><label><Checkbox checked={includeWorkspace} onCheckedChange={(checked) => setIncludeWorkspace(checked === true)} /><span><b>Workspace</b><small>Ask support to re-authorize the current workspace.</small></span></label></fieldset>
            <p className="r22-help-boundary">No conversation, files or browser logs are attached. Nothing is submitted until the next review confirms it.</p>
            ...
            <b>{supportPhase === "error" ? "Request was not submitted" : supportPhase === "unknown" ? "Submission outcome is unknown" : supportPhase === "queued" ? "Request queued" : supportPhase === "waiting" ? "Waiting for a support reply" : "Request closed"}</b>
        ...
        </> : <><fieldset><legend>Optional context</legend><label><Checkbox checked={includeRoute} onCheckedChange={(checked) => setIncludeRoute(checked === true)} /><span><b>Current page</b><small>Include this route, without form contents.</small></span></label><label><Checkbox checked={includeWorkspace} onCheckedChange={(checked) => setIncludeWorkspace(checked === true)} /><span><b>Workspace</b><small>Ask support to re-authorize the current workspace.</small></span></label></fieldset><p className="r22-help-boundary">No conversation, files or browser logs are attached. Opening the email app does not mean a request was sent.</p><a href={mailto}>Review email request <ExternalLink aria-hidden="true" /></a></>}
      </section>
    </div>
