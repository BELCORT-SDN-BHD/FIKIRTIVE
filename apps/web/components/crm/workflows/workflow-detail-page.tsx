      ...
      <div className="mx-auto max-w-[1280px]">
        ...
        <header className="mt-4 flex flex-col gap-4 border-b border-border pb-6 sm:flex-row sm:items-start sm:justify-between sm:gap-8">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><Badge variant={status.variant}>{status.label}</Badge><Badge variant="outline">{definition.definitionKind === "journey" ? "Contact journey" : "Rule"}</Badge>{routineReadError ? <Badge variant="outline">Routine status unavailable</Badge> : activeRoutineCount > 0 ? <Badge variant="success">{activeRoutineCount} active {activeRoutineCount === 1 ? "Routine" : "Routines"}</Badge> : <Badge variant="outline">No active Routines</Badge>}</div>
        ...
        {readError ? <div className="mt-4 rounded-xl border border-warning/30 bg-warning-soft px-4 py-3 text-sm leading-6 text-warning-soft-foreground" data-error-code={readError}><p className="font-semibold">Some workflow data could not refresh</p><p className="mt-1">{readError === "NETWORK" ? "The request could not finish. Previously loaded data remains visible." : workflowErrorMessage(readError)}</p></div> : null}
            ...
            <div className="border-t border-border p-5">
              ...
              <div className="mt-4 grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
                <Card className="overflow-hidden p-0">
                  ...
                  <CardContent className="p-5">
                    <Textarea aria-label="Workflow rule file" spellCheck={false} disabled={archived} value={rulesSource} onChange={(event) => { setRulesSource(event.target.value); setCompilation(null); }} className="min-h-[500px] resize-y rounded-xl bg-[#111114] p-5 font-mono text-[13px] leading-6 text-[#F4F4F5] shadow-inner selection:bg-primary/35" />
              ...
              <Card className="mt-5">
                <CardContent>
                  ...
                  {revisions.length === 0 ? <p className="mt-4 rounded-xl border border-dashed border-border px-5 py-8 text-center text-sm text-muted-foreground">No revisions saved yet. Validate the starter rule, then save the first immutable revision.</p> : <div className="mt-4 grid grid-cols-2 gap-3">{revisions.map((revision) => { const revisionValidation = validationStatusPresentation(revision.validationState); const current = definition.currentRevision === revision.revision; const selected = selectedRevisionId === revision.id; return <Button key={revision.id} type="button" variant={selected ? "secondary" : "outline"} onClick={() => openRevision(revision)} className="h-auto w-full flex-col items-stretch justify-start whitespace-normal rounded-xl p-4 text-left shadow-none"><span className="flex items-center justify-between gap-3"><span className="flex items-center gap-2"><span className="font-semibold">Revision {revision.revision}</span>{current ? <Badge variant="info">Published</Badge> : null}</span><Badge variant={revisionValidation.variant}>{revisionValidation.label}</Badge></span><span className="mt-3 flex items-center gap-2 text-xs font-normal text-muted-foreground"><Clock3 className="size-3.5" />{dateTimeLabel(revision.createdAt)}</span><span className="mt-2 font-mono text-[11px] font-normal text-muted-foreground">{shortWorkflowId(revision.id)}</span></Button>; })}</div>}
                </CardContent>
              </Card>
            </div>
          </details>
