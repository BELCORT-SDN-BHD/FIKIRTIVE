      ...
      <div className="mt-5 grid grid-cols-2 gap-4">
        ...
        <Card>
          <CardContent>
            ...
            {listError ? <div className="mt-3 rounded-lg border border-warning/25 bg-warning-soft px-3 py-2 text-sm leading-6 text-warning-soft-foreground"><p>{listError === "NETWORK" ? "The request could not finish." : workflowErrorMessage(listError)} No time zone or schedule is inferred.</p><p className="mt-1 font-mono text-xs">Error code: {listError}</p></div> : policies.length === 0 ? <p className="mt-3 text-sm leading-6 text-muted-foreground">No business-hours policies exist.</p> : <div className="mt-3 grid gap-2">{policies.map((policy) => <div key={policy.id} className="rounded-xl border border-border bg-secondary/20 p-3"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold">{policy.name}</p><Badge variant="outline">{statusLabel(policy.status)}</Badge></div><p className="mt-1 text-xs text-muted-foreground">{policy.timeZone} · revision {policy.revision}</p><p className="mt-1 font-mono text-[11px] text-muted-foreground">{policy.policyKey}</p></div><Button type="button" size="sm" variant="secondary" disabled={busy !== null} onClick={() => void viewPolicy(policy.id)}>{busy === "detail" && selectedPolicyId === policy.id ? <LoaderCircle className="animate-spin" /> : null}{selectedPolicyId === policy.id && detail ? "Hide hours" : "View hours"}</Button></div>{selectedPolicyId === policy.id ? <div className="mt-3 border-t border-border pt-3">{detailError ? <div className="rounded-lg border border-warning/25 bg-warning-soft px-3 py-2 text-xs leading-5 text-warning-soft-foreground"><p>{detailError === "NETWORK" ? "The request could not finish." : workflowErrorMessage(detailError)} The schedule remains unavailable.</p><p className="mt-1 font-mono">Error code: {detailError}</p></div> : detail ? <div><p className="text-xs font-semibold">Exact weekly windows · {detail.timeZone}</p>{detail.weeklyWindows.length === 0 ? <p className="mt-2 text-xs text-muted-foreground">No open windows.</p> : <ul className="mt-2 grid gap-1 text-xs text-muted-foreground">{detail.weeklyWindows.map((window, index) => <li key={`${window.weekday}-${window.startMinute}-${index}`}>{WEEKDAYS[window.weekday]}: {minuteLabel(window.startMinute)}–{minuteLabel(window.endMinute)}</li>)}</ul>}<p className="mt-2 text-[11px] text-muted-foreground">Updated {dateTimeLabel(detail.updatedAt)}{detail.supersedesPolicyId ? ` · supersedes ${detail.supersedesPolicyId}` : ""}</p></div> : <p className="text-xs text-muted-foreground">Reading exact policy hours…</p>}</div> : null}</div>)}{cursor ? <Button type="button" size="sm" variant="secondary" disabled={busy !== null} onClick={() => void loadMore()}>{busy === "list" ? <LoaderCircle className="animate-spin" /> : null}Load more policies</Button> : null}</div>}
          </CardContent>
        </Card>
      </div>
