  ...
  return (
    <Dialog open={open} onOpenChange={(next) => next ? onOpenChange(true) : requestClose()}>
      <DialogContent className="r22-brand-voice-flow" showCloseButton={false}>
        ...
        </> : step === "generating" ? <div className="r22-brand-generating" aria-live="polite"><span /><DialogTitle>Generating Brand Voice</DialogTitle><DialogDescription>Jasper’s full flow pauses here while approved sources are reviewed. FIKIRTIVE does not show success until analysis finishes.</DialogDescription><article><b>{name}</b><small>Reviewing your sources</small><i /><i /><i /></article></div> : step === "review" ? <>
...
function AudienceFlow({ open, fixture, onOpenChange, onSaved }: {
  ...
  const requestClose = () => dirty ? setCancelOpen(true) : onOpenChange(false);
  const editableList = (label: string, values: string[], setValues: React.Dispatch<React.SetStateAction<string[]>>) => <section className="r22-audience-list"><h3>{label} <span>{values.length} / 8</span></h3>{values.map((value, index) => <div key={`${label}-${index}`}><Input unstyled value={value} onChange={(event) => setValues((current) => current.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} /><Button unstyled type="button" onClick={() => setValues((current) => current.filter((_, itemIndex) => itemIndex !== index))}>Remove</Button></div>)}<Button unstyled type="button" className="is-quiet" disabled={values.length >= 8} onClick={() => setValues((current) => [...current, ""])}>Add</Button></section>;
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => nextOpen ? onOpenChange(true) : requestClose()}><DialogContent className="r22-kb-flow r22-audience-flow" showCloseButton={false}>
    {status === "processing" ? <div className="r22-kb-processing" aria-live="polite"><span /><DialogTitle>Generating audience</DialogTitle><DialogDescription>Reviewing the details and approved source list. Nothing is saved until this finishes.</DialogDescription><i /></div> : status === "success" ? <div className="r22-brand-success" role="status"><ShieldCheck aria-hidden="true" /><DialogTitle>Audience saved</DialogTitle><DialogDescription>{name} is now approved {scope} context.</DialogDescription><Button unstyled className="is-primary" type="button" onClick={() => onOpenChange(false)}>Done</Button></div> : <>
...
function VisualGuidelineFlow({ open, fixture, onOpenChange, onSaved }: {
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => nextOpen ? onOpenChange(true) : requestClose()}><DialogContent className="r22-kb-flow r22-visual-flow" showCloseButton={false}>{status === "processing" ? <div className="r22-kb-processing" aria-live="polite"><span /><DialogTitle>Saving Visual Guideline</DialogTitle><DialogDescription>Keeping the declared colours, font slots, source and scope together.</DialogDescription><i /></div> : status === "success" ? <div className="r22-brand-success" role="status"><ShieldCheck aria-hidden="true" /><DialogTitle>Visual Guideline saved</DialogTitle><DialogDescription>{name} is now approved {scope} context.</DialogDescription><Button unstyled className="is-primary" type="button" onClick={() => onOpenChange(false)}>Done</Button></div> : <>
