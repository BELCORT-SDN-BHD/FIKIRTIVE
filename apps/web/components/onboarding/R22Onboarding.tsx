      ...
      <div className="r22-ob-stage">
        <section className="r22-ob-form">
          ...
          {step === "channel" ? <>
            ...
            {channelState === "unknown" ? <p className="r22-ob-error" role="alert">Connection status could not be read. Nothing is guessed in its place.</p> : null}
        ...
        <aside className="r22-ob-side" aria-label={step === "workspace" || step === "post" ? "Preview" : "Step preview"}>
          ...
          {step === "brand" ? <><div className="r22-ob-memory"><b>What Otto reads first</b><p>{brandContext || brandPhase === "result" ? "Logo, colours, products and a warm, plain voice — each with its page source." : "Nothing yet. Whatever you save here is the first thing Otto reads before drafting anything."}</p><small>Working in {workspaceName.trim() || "your workspace"}</small></div><p className="r22-ob-side-note">Every saved fact keeps the page it came from, so you can always ask where a claim came from.</p></> : null}
