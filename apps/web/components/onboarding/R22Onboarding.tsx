      ...
      <div className="r22-ob-stage">
        <section className="r22-ob-form">
          ...
          {step === "routine" ? <>
            ...
            <p className="r22-ob-fine">Otto stops for the week the moment it reaches the cap. It never asks for more.</p>
        ...
        <aside className="r22-ob-side" aria-label={step === "workspace" || step === "post" ? "Preview" : "Step preview"}>
          ...
          {step === "post" ? <><div className="r22-ob-preview"><div className="r22-ob-preview-head">Preview</div><div className="r22-ob-post-preview">{postPhase === "result" || postPhase === "approved" ? <Image src="/fixtures/r22-canvas/art-1.jpg" fill sizes="296px" alt="Fixture first post preview" /> : <b>{postPhase === "generating" ? "Drafting…" : "Nothing drafted yet"}</b>}</div><p>{postPhase === "result" || postPhase === "approved" ? prompt : "Nothing drafted yet. What Otto makes shows up here before it goes anywhere."}</p></div><p className="r22-ob-side-note">Otto will not make anything on its own after this. Set up a routine if you want it to.</p></> : null}
        </aside>
      </div>
    </main>
  );
