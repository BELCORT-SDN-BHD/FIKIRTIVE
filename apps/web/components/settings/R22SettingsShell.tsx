...
export function R22SettingsShell({ data, initialSection = "workspace", fixture = false, fixtureState = "ready", fixtureOutcome = "success" }: { data: R22SettingsData; initialSection?: R22SettingsSection; fixture?: boolean; fixtureState?: FixtureSurfaceState; fixtureOutcome?: "success" | "error" | "conflict" | "unknown" }) {
  ...
  const submitAction = () => {
    ...
    if (activeAction === "Invite member") {
      ...
      if (!Number.isInteger(Number(actionValue)) || Number(actionValue) <= 0) return setActionError("Choose a whole number of cr above 0.");
      ...
      if (workspaceDirectory.workspaces.some((workspace) => workspace.name.toLowerCase() === actionValue.trim().toLowerCase())) return setActionError("A workspace with this name is already in this preview.");
    ...
    if (!fixture) return setActionError(`${activeAction} is not switched on yet. Nothing changed.`);
    ...
    if (!fixtureActions.includes(activeAction) && !activeAction.startsWith("View ") && !activeAction.startsWith("Change role:") && !activeAction.startsWith("Remove member:") && !activeAction.startsWith("Verify domain:")) return setActionError(`${activeAction} is not ready in this preview, so nothing was changed.`);
    setActionBusy(true);
    window.setTimeout(() => {
      if (fixtureOutcome !== "success" && !actionFailedOnce) {
        setActionFailedOnce(true);
        setActionBusy(false);
        setActionError(fixtureOutcome === "conflict" ? "This setting changed elsewhere. Review the latest value and retry the same change." : fixtureOutcome === "unknown" ? "This setting outcome is unknown. Check this same change before starting another; nothing is assumed saved." : "This change was not confirmed. Nothing changed, so it is safe to try again.");
        return;
      }
  ...
  else if (activeAction === "Review sessions") dialogFields = <div className="r22-settings-session-list">{sessions.map((session) => <div key={session.id}><span><b>{session.label}</b><small>{session.detail}</small></span>{session.current ? <i className="r22-settings-pill">Current</i> : <Button unstyled type="button" onClick={() => { setSessions((current) => current.filter((item) => item.id !== session.id)); setNotice("Session signed out in this preview."); }}>Revoke</Button>}</div>)}</div>;
  ...
  else if (activeAction === "Top up") dialogFields = <label>Credits<SelectNative unstyled value={actionValue} onChange={(event) => setActionValue(event.target.value)}><option value="200">200 cr</option><option value="500">500 cr</option><option value="1000">1,000 cr</option></SelectNative></label>;
  ...
  else if (activeAction === "Create workspace") dialogFields = <><label>Workspace name<Input unstyled autoFocus value={actionValue} onChange={(event) => setActionValue(event.target.value)} placeholder="Client or brand name" /></label><Empty className="r22-settings-dialog-empty"><EmptyHeader><EmptyTitle>A separate workspace boundary will be created</EmptyTitle><EmptyDescription>Members, roles, channels, credits and settings all start empty. Nothing from {workspaceName} is copied across.</EmptyDescription></EmptyHeader></Empty></>;
  ...
  else if (destructiveAction || activeAction.startsWith("Connect ") || activeAction.startsWith("Verify domain:")) dialogFields = <Empty className="r22-settings-dialog-empty"><EmptyHeader><EmptyTitle>{destructiveAction ? "Review the consequence before continuing" : "This changes only the workspace you have open"}</EmptyTitle><EmptyDescription>{activeAction.startsWith("Disconnect ") ? "Scheduled work stays visible and moves to held-connection. No provider or live account is changed." : activeAction.startsWith("Remove member:") ? "They lose access to this workspace only. Their personal account is not deleted." : "No outside service, domain record or live permission is changed."}</EmptyDescription></EmptyHeader></Empty>;
