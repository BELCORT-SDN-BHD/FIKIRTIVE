...
export function R22SettingsShell({ data, initialSection = "workspace", fixture = false, fixtureState = "ready", fixtureOutcome = "success" }: { data: R22SettingsData; initialSection?: R22SettingsSection; fixture?: boolean; fixtureState?: FixtureSurfaceState; fixtureOutcome?: "success" | "error" | "conflict" | "unknown" }) {
  ...
  else if (activeAction === "View invoices") dialogFields = <div className="r22-settings-dialog-empty"><b>No invoices yet</b><p>Invoices appear here once billing is switched on for this workspace.</p></div>;
  ...
  else if (destructiveAction || activeAction.startsWith("Connect ") || activeAction.startsWith("Verify domain:")) dialogFields = <div className="r22-settings-dialog-empty"><b>{destructiveAction ? "Review the consequence before continuing" : "This changes only the workspace you have open"}</b><p>{activeAction.startsWith("Disconnect ") ? "Scheduled work stays visible and moves to held-connection. No provider or live account is changed." : activeAction.startsWith("Remove member:") ? "They lose access to this workspace only. Their personal account is not deleted." : "No outside service, domain record or live permission is changed."}</p></div>;
