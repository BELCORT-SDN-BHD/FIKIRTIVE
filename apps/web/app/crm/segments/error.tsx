...
export default function CustomerSegmentsError({
  error,
  unstable_retry,
}: {
  ...
  return (
    <main className="min-h-dvh bg-background px-4 py-10 text-foreground sm:px-6">
      <section className="mx-auto max-w-xl rounded-[var(--radius-card)] border border-border bg-card p-6 shadow-[var(--shadow-sm)] sm:p-8">
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight">Segments could not load</h1>
        <p className="mt-3 text-sm leading-6 text-muted-foreground">
          Your saved segments were not changed. Retry the owner-scoped read, or return to Otto.
        </p>
        {error.digest ? (
