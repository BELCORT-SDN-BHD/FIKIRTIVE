...
const alertVariants = cva(
  "relative grid w-full items-start gap-y-0.5 rounded-[var(--radius-card)] border text-sm grid-cols-[0_1fr] has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] has-[>svg]:gap-x-3 [&>svg]:size-4 [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
...
function Alert({
  ...
  return <div data-slot="alert" className={cn(alertVariants({ variant, density }), className)} {...props} />
}
