 ...
 *   - fullscreen —— 同一条 conversation 的专用全屏视图;主内容暂时 inert,退出后继续使用。
 *
...
export interface OttoPanelProps {
  ...
  expanded: boolean;
  /** Cloudflare-style dedicated conversation surface. It covers the app but keeps this panel mounted. */
  fullscreen?: boolean;
  ...
  /** Reference / product-specific header composition. The panel still owns its frame and geometry. */
  ...
  return (
    <TooltipProvider>
      ...
      <aside
        aria-label="Otto"
        data-otto-panel=""
        data-otto-panel-mode={fullscreen ? "fullscreen" : state.mode}
        ...
        className={cn(
          ...
          fullscreen
            ? "fixed inset-0 h-dvh w-dvw border-0"
            : floating
      ...
      >
        ...
        <header
          ...
          onPointerDown={fullscreen ? undefined : startHeaderDrag}
          className={cn(
            "flex shrink-0 items-center gap-1.5 border-b border-border px-3 py-2.5 select-none",
            fullscreen ? "cursor-default" : drag && drag.kind !== "resize-docked" ? "cursor-grabbing" : "cursor-grab",
          )}
        >
          {header ?? (
            <>
