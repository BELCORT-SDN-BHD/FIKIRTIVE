...
function rowClass(collapsed: boolean, active: boolean): string {
  return cn(
    "flex h-9 w-full items-center gap-2.5 rounded-lg text-[13px] outline-none transition-colors",
...
function RailLink({
  ...
  return (
    <Link
      href={hrefOverrides?.[link.key] ?? link.href}
...
/** 导轨只需要余额；身份与 sign-out 由全局 utility bar 统一负责。 */
...
export interface NavigationRailProps {
  ...
  /** Review surfaces may redirect a canonical destination into an equivalent local fixture. */
  hrefOverrides?: Readonly<Record<string, string>>;
  /** Review surfaces may render the frozen IA without mutating production navigation first. */
  visibleTopLevelKeys?: readonly string[];
  /** Review surfaces may preview a future top-level destination while runtime still stores it as a group. */
  flattenedGroupKeys?: readonly string[];
  /** Review surfaces may keep the credits shortcut inside the same local experience. */
  creditsHref?: string;
}

export function NavigationRail({ pathname, account, hrefOverrides, visibleTopLevelKeys, flattenedGroupKeys, creditsHref }: NavigationRailProps) {
  ...
  const visibleKeys = visibleTopLevelKeys ? new Set(visibleTopLevelKeys) : null;
  const flatKeys = flattenedGroupKeys ? new Set(flattenedGroupKeys) : null;
  const renderedTree = visibleKeys ? RAIL_TREE.filter((node) => visibleKeys.has(node.key)) : RAIL_TREE;
  ...
  return (
    <nav
      ...
      className="flex h-dvh shrink-0 flex-col border-r border-border bg-muted/35 text-foreground"
    >
      <div className={cn("flex items-center gap-1.5 px-3 pt-3", collapsed && "flex-col gap-1 px-2")}>
        <Link
          href={hrefOverrides?.home ?? SHELL_ROUTES.home}
          aria-label="Fikirtive home"
          className={cn(
            "flex h-10 items-center gap-2 rounded-lg text-[15px] font-bold tracking-[-0.025em] text-foreground outline-none",
            "focus-visible:ring-[3px] focus-visible:ring-ring/40",
            collapsed ? "w-10 justify-center" : "flex-1 px-2",
          )}
        >
          <FikirtiveMark size={26} />
          {!collapsed && <span>fikirtive</span>}
        </Link>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-3 pb-3 pt-4">
        ...
        {!collapsed && (
          <span className="mb-1 px-2.5 text-[11px] font-medium text-muted-foreground">Workspace</span>
        )}
        <div className="flex flex-col gap-1">
          {renderedTree.map((node) => {
            if (isRailGroupNode(node) && flatKeys?.has(node.key)) {
              const firstChild = node.items[0];
              if (!firstChild) return null;
              const flattenedLink: RailLinkNode = {
                ...firstChild,
                key: node.key,
                label: node.label,
                href: hrefOverrides?.[node.key] ?? firstChild.href,
                icon: node.icon,
              };
              return (
                <RailLink
                  key={node.key}
                  link={flattenedLink}
                  active={isGroupActive(node, pathname)}
                  collapsed={collapsed}
                  hrefOverrides={hrefOverrides}
                />
              );
            }
            return isRailGroupNode(node) ? (
              <RailGroup key={node.key} group={node} pathname={pathname} collapsed={collapsed} hrefOverrides={hrefOverrides} />
            ) : (
              <RailLink key={node.href} link={node} active={winner === node.href} collapsed={collapsed} hrefOverrides={hrefOverrides} />
            );
          })}
        </div>
