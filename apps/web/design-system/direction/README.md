...
> Founder 2026-08-20 设计专场定案。本目录负责视觉方向，完整权威地图在
> [`../README.md`](../README.md)。`../references/legacy-v3/`、Claude Design prototype v2 与旧
> ui_kits 已被本方向取代，只作历史参考。
...
- **两件官方标识**:F 字标(公司)+ Otto 云朵(操盘手),规范全文在 `../brand/`。

## 目录

| 文件 | 内容 | 状态 |
|---|---|---|
| [`design-principles.md`](design-principles.md) | 《Fikirtive 设计原则》一页:18 条死规矩 + 逐条判法,品味判官对每张 UI PR 的判据 | 定稿(Founder 2026-08-20 逐条拍板 13/13 + 审计增补 4 条 + 钱路显著度第 18 条亲批) |
...
2. **components/设计系统场**:`../brand/colors.json` + guidelines 已翻译进
   `../foundations/globals.css`;正式 primitives 在 `../primitives/`。
...
- 本目录不含组件代码；实现一律走 `../foundations/`、`../primitives/` 与 `../patterns/`。
