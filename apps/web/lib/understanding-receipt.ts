 ...
 *   · **已到终态但净额为 0** —— 没有回执可说:要么这件素材根本不进理解(裁剪存下的新素材),
 *     要么那一笔失败/被退款(账本 RESERVE+REFUND 净额恒 0,与消费历史「Held, then refunded
 *     in full」同一个口径)。既不编一句「Understood · 0 credits」,也不留一行空回执 ——
 *     调用方据此整行不渲染,与详情面其余几块回执同一条纪律(有则显示、无则整行不出现)。
 */
