...
export async function startGen(raw: unknown): Promise<StartGenResult> {
  ...
  return runAsUser(principal, async (): Promise<StartGenResult> => {
    ...
    const parsed = genRequest.safeParse(resolvePublicModelAlias(raw));
    if (!parsed.success) return { error: "That generation request is out of bounds." };
    const { projectId, shotId, sourceGenerationId, tailGenerationId, referenceVideoGenerationId, prompt, requestedPrompt, entityIds, count, kind, model, durationSeconds, resolution, aspectRatio, fps, audio, idempotencyKey, variantSel, threadId, approvedEntities, coherentSet } = parsed.data;
