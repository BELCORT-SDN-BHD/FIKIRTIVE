 ...
 *
 * Wraps the SAME owner-gated Project server actions the human sidebar uses (actions.ts). Identity is
 ...
 * delete any Project that still contains a live Generation (deletedAt null count > 0) and directs the
 * user to the UI's by-hand confirm. Only an EMPTY Project may be deleted through Otto — aligned with
...
export function makeOttoProjectsPort(ownerId: string) {
  return {
    ...
    remove: async (projectId: string): Promise<{ ok: true } | { error: string }> => {
      // Deterministic pre-gate (see header): only an EMPTY Project may be deleted via Otto.
