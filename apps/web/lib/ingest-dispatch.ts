...
/**
 * 上传落行之后把 ingest 派出去 —— **`apps/web` 里唯一**的那一处(家规 §7.3 单一真相)。
 *
 * 说准一点(2026-09-18 复审):唯一的是**产品这一侧**。worker 侧的清道夫
 * `redispatchLostIngest`(`apps/worker/src/index.ts`)自己也送一次单,那一处**有意保留** ——
 * 它补的正是「这里发出去失败了」,不可能由 apps/web 代劳。围栏扫的也正是 `apps/web/lib`
 * 这一棵树(`lib/__tests__/ingest-dispatch-single-authority.test.ts`)。
 *
 ...
 * 让每一条上传入口调**同一个**函数。谁将来再加一条上传入口,也只有这一个函数可调。
 *
 * 2026-09-18 复审回修:那句裁的是**这一类**设计,不是画布那一条入口。于是同根的另外两处 ——
 * `actions.ts` 的 `createEntity`(Library「新建元素」的参考图)与 `asset-actions.ts` 的
 * `saveCroppedGeneration`(素材详情的裁剪保存)—— 随同一票一起改成当场调这里(家规
 * 「修根不修表、关类不补例」)。今天四条上传入口全在这一个函数上,围栏按**函数**粒度守着。
 *
