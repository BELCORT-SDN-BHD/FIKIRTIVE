...
// 租户围栏收尾片（#464）：thread-activity.ts 现在从这个模块拿 `resolveUserPrincipal` 建帧;
// 单测 mock 掉了整个 `../auth-guard`，补上 #464 B1 既有的共享 stub——与其它对着真库的围栏
// 测试（tenant-action-cross-tenant-slice2.test.ts 等）同款用法：Prisma 是真的，principal
// 的解析不需要再多打一次数据库。
