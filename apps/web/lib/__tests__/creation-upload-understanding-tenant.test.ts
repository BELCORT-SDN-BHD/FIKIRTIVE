...
describe("creation §5 :169 FSE-009 上传理解费的租户边界(真库)", () => {
  it("creation §5 :169 FSE-009 真库双租户:B 拿 A 的 generation 去问费用合计,折出 0 —— 看不到 A 那一笔", async () => {
    ...
    const asOwner = await loadUploadUnderstandingCredits(a.ownerId, [
      { id: a.generationId, assetId: a.assetId, mime: "image/png", source: "UPLOAD", deletedAt: null, width: 800, height: 600, durationS: null },
    ]);
    ...
    const asOtherTenant = await loadUploadUnderstandingCredits(b.ownerId, [
      { id: a.generationId, assetId: a.assetId, mime: "image/png", source: "UPLOAD", deletedAt: null, width: 800, height: 600, durationS: null },
    ]);
