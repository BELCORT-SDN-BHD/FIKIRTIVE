...
describe("登录门规格 · 17 条验收的落点地图", () => {
  ...
  //                   fail-closed 分支（FOR SHARE 那次读失败、读不到 ba_user 邮箱行），外加第 8 轮
  //                   七条故障注入（告警自己抛错时两条调用路各自抛的仍是原来那个错、删失败重试一次
  //                   的四条读数、首登兜底那处 tag 的告警与不告警各一条））、
  //                   better-auth-oauth-session-gate.test.ts（会话闸的库内接线，既有）

  // ⑤ 端到端旅程（issue #1320）—— 最后一行 it.todo 换成了真旅程：
  //    · SIGNIN-A12 → e2e/journeys/26-two-doors-one-workspace.spec.ts（码门首登 → 生成一张图
