...
function row(id: string, ext: string, createdAtIso: string, favorite = false) {
  return {
    id, projectId: `p-${id}`, assetId: `asset-${id}`, promptText: `p-${id}`, favorite, createdAt: new Date(createdAtIso),
...
describe("getGenerationHistory — paging & mapping", () => {
  it("derives kind from ext, resolves url, maps fields", async () => {
    ...
    expect(res.items[0]).toEqual({ id: "a", projectId: "p-a", assetId: "asset-a", url: "https://cdn/u1/h-a.mp4", kind: "video", prompt: "p-a", favorite: true, createdAt: "2026-01-03T00:00:00.000Z" });
