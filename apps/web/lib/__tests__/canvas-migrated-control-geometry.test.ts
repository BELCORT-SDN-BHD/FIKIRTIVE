 ...
 * 写死尺寸的旧配方类。这就意味着:**把那份来源也一并删掉**的改动,能从闸下走过去。
 * 具体说,如果有人把 GeneratingBody 那份 inline `style` 换成纯 Tailwind、同时把 `h-auto`
 * 也删了,第①闸不会响 —— 它已经不认为那是个「几何来自别处」的调用点了。
 *
 * 判官核过现实:今天**没有漏网的**,三个旧配方类与那份 inline style 都还在原地。所以
 * 这一条是**未来**的盲区,不是眼下的洞,本轮按判官意见只留档、不扩闸。
 *
 * 真要补的话,补法不是把闸写宽(「每个 Button 都要写高度」那条已经证明会误伤一堆正常
 * CTA),而是换一种判据:钉住这几个调用点**渲染出来的高度**,而不是钉它们的 class 字符串。
 * 那需要把 jsdom 换成一个真会算样式的环境(jsdom 不做层叠计算),是另一件工程,不在本车。
 */
