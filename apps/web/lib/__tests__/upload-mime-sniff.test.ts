...
describe("finalizeCandidateUploads — byte-derived mime (工单 F)", () => {
  ...
  it("a transient storage READ failure degrades to the ext mime (best-effort; worker gate re-verifies at publish)", async () => {
    ...
    expect(res).toMatchObject({ ok: true, count: 1 });
    expect(assetUpsert.mock.calls[0]?.[0]?.create?.mime).toBe("image/png"); // mimeOf("png") fallback
  });
});
