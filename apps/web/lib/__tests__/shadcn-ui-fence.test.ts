/**
 * #840 立法围栏 —— 商家可见面的交互原语,一律走 `@/components/ui` 唯一包装层。
 *
 * ── 法条更新 2026-08-28(《开发作业手册》落地 PR)────────────────────────────
 * Founder 2026-08-28 裁决:组件基座方向切换为 Base UI(碑:docs/specs/base-ui-switch.md;
 * 切换本身须走 S1 冻结后才动主干)。2026-08-11 常令中「shadcn/ui 唯一」的表述自此废止,
 * 由「@/components/ui 是唯一包装层」承接——这正是本围栏一直在机器上执行的规则:它管
 * 调用点必须走对位组件,不管包装件内部用 Radix 还是 Base UI。下文法理叙述与豁免板
 * 理由里凡提及 shadcn/Radix 处,均为 2026-08-11 立法时的历史原文,保留不改;围栏的
 * 机械规则与豁免棘轮原样生效,直到基座切换的 S1 冻结时按其「受影响围栏清单」修订。
 * ──────────────────────────────────────────────────────────────────────────
 *
