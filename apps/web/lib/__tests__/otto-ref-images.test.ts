...
vi.mock("@fikirtive/core", () => ({
  storageKey: () => "k/e/y",
  // 判据的**真身**照旧用真函数(而不是桩)—— 下面那条 CREATE-A2 断言要看的正是它产出的
  // where 里有没有混进 projectId。
  generationReferenceScope: (ownerId: string, exts: readonly string[]) => ({
    ownerId,
    deletedAt: null,
    asset: { ext: { in: [...exts] } },
  }),
...
describe("gatherReferenceImages", () => {
  it("returns [] when no sourceGenerationId", async () => {
    expect(await gatherReferenceImages("o", null)).toEqual([]);
  ...
  it("returns [] when vision disabled", async () => {
    ...
    expect(await gatherReferenceImages("o", "gen-1")).toEqual([]);
    expect(genFindFirst).not.toHaveBeenCalled();
  });
  ...
  it("returns a data URL on success", async () => {
    ...
    const out = await gatherReferenceImages("o", "gen-1");
  ...
  it("returns multiple data URLs for multiple sourceGenerationIds in input order", async () => {
    ...
    const out = await gatherReferenceImages("o", ["gen-a", "gen-b"]);
  ...
  it("CREATE-A2 Codex QA-CRE-FE9-013:Otto 的视觉也按租户取图 —— 两条 where 里都没有 projectId", async () => {
    genFindFirst.mockResolvedValue({ asset: { ownerId: "o", contentHash: "h", ext: "png", sizeBytes: 12 } });
    storageGet.mockResolvedValue(Buffer.from([0x01]));
    // 单件那一路
    await gatherReferenceImages("o", "gen-from-another-canvas");
    expect(JSON.stringify(genFindFirst.mock.calls[0]?.[0]?.where)).not.toContain("projectId");
    expect(genFindFirst.mock.calls[0]?.[0]?.where).toMatchObject({ ownerId: "o", deletedAt: null });

    // 多件那一路
    genFindMany.mockResolvedValue([]);
    await gatherReferenceImages("o", ["gen-a", "gen-b"]);
    expect(JSON.stringify(genFindMany.mock.calls[0]?.[0]?.where)).not.toContain("projectId");
  });

  it("never throws — returns [] on a storage error", async () => {
    genFindFirst.mockResolvedValue({ asset: { ownerId: "o", contentHash: "h", ext: "png", sizeBytes: 1234 } });
    storageGet.mockRejectedValue(new Error("s3 down"));
    await expect(gatherReferenceImages("o", "gen-1")).resolves.toEqual([]);
  });
});
